        ...
        match args[i].as_str() {
            ...
            "--role" if i + 1 < args.len() => {
                ...
                role = Role::parse(name).ok_or_else(|| {
                    format!(
                        "unknown role '{name}' (lab|explorer|formalizer|proof-searcher|falsifier|reviewer|auditor|replication-agent|empirical-analyst|numerical-verifier|provenance-auditor)"
                    )
                })?;
                i += 2;
            }
