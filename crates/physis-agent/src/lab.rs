...
mod tests {
    ...
    fn why_prints_first_class_identity_fields() {
        ...
        let a3 = lab
            .exec(Command::Why {
                claim: "gut.coupling-unification-interval".into(),
            })
            .text()
            .to_string();
        let a3b = why_theory_block(&a3, "su5-gut");
        assert!(
            a3b.contains("  datasets:") && a3b.contains("pdg-2022-alpha-s-mz"),
            "{a3b}"
        );
        assert!(
            a3b.contains("pdg-2024-sin2theta") && a3b.contains("pdg-2022-inv-alpha-em-mz"),
            "input listings must be on the identity: {a3b}"
        );
        assert!(a3b.contains("M_Z"), "{a3b}");
        assert!(a3b.contains("regimes:"), "{a3b}");
        assert!(
            !a3b.contains("not yet a machine-checked regime"),
    ...
    fn turning_on_susy_flips_gqw_weinberg_angle() {
        ...
        assert!(
            diffs
                .iter()
                .any(|d| d.claim == "gut.coupling-unification-interval"
                    && d.from == VerdictKind::Fails
                    && d.to == VerdictKind::Undecidable),
    ...
    fn susy_gqw_interval_is_insufficient_precision() {
        ...
        assert!(
            gaps.lines()
                .any(|l| l.contains("gut.coupling-unification-interval")
                    && l.contains("tighter-enclosure")),
            "{gaps}"
        );
    ...
    fn research_loop_proves_reviews_and_restores_knobs() {
        ...
        assert!(
            text.contains("enclose  gut.coupling-unification-interval"),
        ...
        assert!(
            text.contains("constant  ledger  58290942461f9847b63d0fbc97b2ead26afc2f8a6b13da64164ceeeb6a5493e7"),
    ...
    fn numerical_verifier_encloses_ratio_strings_and_cannot_prove() {
        ...
        let a3 = lab
            ...
            .text()
            .to_string();
        assert!(a3.contains("interval-certified"), "{a3}");
        assert!(a3.contains("not P3N"), "{a3}");
        assert!(a3.contains("not a kernel proof"), "{a3}");
        assert!(a3.contains("not P4"), "{a3}");
        assert!(!a3.contains("receipt"), "{a3}");
        assert!(!a3.contains("enclosure    [3/8, 3/8]"), "{a3}");
        let a3_id = numeric_certificate_id(&a3);
        assert_eq!(
            lab.store.get(a3_id).map(|n| n.kind),
            Some(NodeKind::NumericCertificate)
        );
        assert_ne!(
            a3_id.to_hex(),
            "0967e9f42ec9ff0fd8e29fecc5bb5a3ed9aba4974ac77b0e5217a4bb634ec202",
            "α_3 interval certificate is not the GUT-scale 3/8 pin"
        );
        assert_ne!(
            a3_id.to_hex(),
            "abb134fa6d8b112c92c0dfbefb789a4446cbed54aaeb83528658bd65d2b1ace3",
            "α_3 interval certificate is not the GQW interval pin"
        );
        assert_eq!(
            a3_id.to_hex(),
        ...
        assert!(
            a3.contains("11b15a7f8fbfaab08c38b773cfaac930a13e0d77b021194a7c07676a083e8825"),
        ...
        let why_a3 = lab
            ...
            .to_string();
        assert!(
            why_a3.contains(&format!("enclose:     {a3_id}")),
            "{why_a3}"
        );
        assert!(
            why_a3.contains("judgment:   statistical computed"),
            "{why_a3}"
        );
        assert!(!why_a3.contains("certified-numeric"), "{why_a3}");
        assert!(
            !why_a3
                .lines()
                .any(|l| l.contains("trust:") && l.contains("P3N")),
    ...
    fn provenance_auditor_rebuilds_versioned_constants_and_cannot_review() {
        ...
        let gmu = lab
            ...
            .text()
            .to_string();
        assert!(gmu.contains("constant  gmu  node "), "{gmu}");
        assert!(
            gmu.contains(
                "hash     0a3447871c2dc78e3fa0c69d9134b7ee2852cbf55a0c570452a9118d9c747ded"
            ),
            "{gmu}"
        );
        assert!(gmu.contains("kind     interval"), "{gmu}");
        assert!(gmu.contains("table    XXXI"), "{gmu}");
        assert!(gmu.contains("range    gmu = -2.0023318418(13)"), "{gmu}");
        assert!(gmu.contains("unit     1"), "{gmu}");
        assert!(
            gmu.contains("value    [-20023318431/10000000000, -4004663681/2000000000]"),
            "{gmu}"
        );
        assert!(gmu.contains("rebuild  ok"), "{gmu}");
        assert!(gmu.contains("not P3N"), "{gmu}");
        assert!(!gmu.contains("receipt"), "{gmu}");
        assert!(!gmu.contains("theorem"), "{gmu}");
        let gmu_id = constant_node_id(&gmu);
        assert_eq!(
            gmu_id.to_hex(),
            "a810ed959ec82be1763f525714739871f182303850a02507d163f4e73fcc5996",
            "journaling must not change the gmu constant payload"
        );
        assert_eq!(
            lab.store.get(gmu_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_mu_mup = lab
            .exec(Command::Constant {
                name: Some("mu_mu_mup".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_mu_mup.contains("constant  mu_mu_mup  node "),
            "{mu_mu_mup}"
        );
        assert!(
            mu_mu_mup.contains(
                "hash     495228cd39ae86738938efec2d2639f9a0c198fdc4cb75ed4968321672b7df84"
            ),
            "{mu_mu_mup}"
        );
        assert!(mu_mu_mup.contains("kind     interval"), "{mu_mu_mup}");
        assert!(mu_mu_mup.contains("table    XXXI"), "{mu_mu_mup}");
        assert!(
            mu_mu_mup.contains("range    mu_mu/mup = -3.183345142(71)"),
            "{mu_mu_mup}"
        );
        assert!(mu_mu_mup.contains("unit     1"), "{mu_mu_mup}");
        assert!(
            mu_mu_mup.contains("value    [-3183345213/1000000000, -3183345071/1000000000]"),
            "{mu_mu_mup}"
        );
        assert!(mu_mu_mup.contains("rebuild  ok"), "{mu_mu_mup}");
        assert!(mu_mu_mup.contains("not P3N"), "{mu_mu_mup}");
        assert!(!mu_mu_mup.contains("receipt"), "{mu_mu_mup}");
        assert!(!mu_mu_mup.contains("theorem"), "{mu_mu_mup}");
        let mu_mu_mup_id = constant_node_id(&mu_mu_mup);
        assert_eq!(
            mu_mu_mup_id.to_hex(),
            "6468f3033806b2fdcf78e2fbc7dfc7921b14f98b7a61b88127e4302c4d7fabff",
            "journaling must not change the mu_mu_mup constant payload"
        );
        assert_eq!(
            lab.store.get(mu_mu_mup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_p_u = lab
            .exec(Command::Constant {
                name: Some("m_p_u".into()),
            })
            .text()
            .to_string();
        assert!(m_p_u.contains("constant  m_p_u  node "), "{m_p_u}");
        assert!(
            m_p_u.contains(
                "hash     244a086710c746078b5de6d5f2c5f896dd01a8469448035eadbc63c49fff6435"
            ),
            "{m_p_u}"
        );
        assert!(m_p_u.contains("kind     interval"), "{m_p_u}");
        assert!(m_p_u.contains("table    XXXI"), "{m_p_u}");
        assert!(
            m_p_u.contains("range    mp_u = 1.007276466621(53)"),
            "{m_p_u}"
        );
        assert!(m_p_u.contains("unit     u"), "{m_p_u}");
        assert!(
            m_p_u.contains("value    [125909558321/125000000000, 503638233337/500000000000]"),
            "{m_p_u}"
        );
        assert!(m_p_u.contains("rebuild  ok"), "{m_p_u}");
        assert!(m_p_u.contains("not P3N"), "{m_p_u}");
        assert!(!m_p_u.contains("receipt"), "{m_p_u}");
        assert!(!m_p_u.contains("theorem"), "{m_p_u}");
        let m_p_u_id = constant_node_id(&m_p_u);
        assert_eq!(
            m_p_u_id.to_hex(),
            "b7d7c12458d2829801236ee857190822650a55625bc19c759e683874869f9b6c",
            "journaling must not change the m_p_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_p_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_p_c2 = lab
            .exec(Command::Constant {
                name: Some("m_p_c2".into()),
            })
            .text()
            .to_string();
        assert!(m_p_c2.contains("constant  m_p_c2  node "), "{m_p_c2}");
        assert!(
            m_p_c2.contains(
                "hash     9bdab3205a64c45d2c413626db03cdf6452e021df65c420170cd7ffb163990f3"
            ),
            "{m_p_c2}"
        );
        assert!(m_p_c2.contains("kind     interval"), "{m_p_c2}");
        assert!(m_p_c2.contains("table    XXXI"), "{m_p_c2}");
        assert!(
            m_p_c2.contains("range    mpc2 = 1.50327761598(46)e-10"),
            "{m_p_c2}"
        );
        assert!(m_p_c2.contains("unit     J"), "{m_p_c2}");
        assert!(
            m_p_c2.contains(
                "value    [9395485097/62500000000000000000, 37581940411/250000000000000000000]"
            ),
            "{m_p_c2}"
        );
        assert!(m_p_c2.contains("rebuild  ok"), "{m_p_c2}");
        assert!(m_p_c2.contains("not P3N"), "{m_p_c2}");
        assert!(!m_p_c2.contains("receipt"), "{m_p_c2}");
        assert!(!m_p_c2.contains("theorem"), "{m_p_c2}");
        let m_p_c2_id = constant_node_id(&m_p_c2);
        assert_eq!(
            m_p_c2_id.to_hex(),
            "e242f8c1e541d797801325311916546d420c7a71b42303c94a0a7d060b8adde6",
            "journaling must not change the m_p_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_p_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_p_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_p_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_p_c2_mev.contains("constant  m_p_c2_MeV  node "),
            "{m_p_c2_mev}"
        );
        assert!(
            m_p_c2_mev.contains(
                "hash     fe91682af8608f3a6117790109cc0cbb09c709fb7cc1a778d6c6be39efea1c5e"
            ),
            "{m_p_c2_mev}"
        );
        assert!(m_p_c2_mev.contains("kind     interval"), "{m_p_c2_mev}");
        assert!(m_p_c2_mev.contains("table    XXXI"), "{m_p_c2_mev}");
        assert!(
            m_p_c2_mev.contains("range    mpc2_MeV = 938.27208816(29)"),
            "{m_p_c2_mev}"
        );
        assert!(m_p_c2_mev.contains("unit     MeV"), "{m_p_c2_mev}");
        assert!(
            m_p_c2_mev.contains("value    [93827208787/100000000, 18765441769/20000000]"),
            "{m_p_c2_mev}"
        );
        assert!(m_p_c2_mev.contains("rebuild  ok"), "{m_p_c2_mev}");
        assert!(m_p_c2_mev.contains("not P3N"), "{m_p_c2_mev}");
        assert!(!m_p_c2_mev.contains("receipt"), "{m_p_c2_mev}");
        assert!(!m_p_c2_mev.contains("theorem"), "{m_p_c2_mev}");
        let m_p_c2_mev_id = constant_node_id(&m_p_c2_mev);
        assert_eq!(
            m_p_c2_mev_id.to_hex(),
            "c44790b16220afd067ff78b6174045801f94d8412541b829412bccf223891629",
            "journaling must not change the m_p_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_p_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mp_me = lab
            .exec(Command::Constant {
                name: Some("mp_me".into()),
            })
            .text()
            .to_string();
        assert!(mp_me.contains("constant  mp_me  node "), "{mp_me}");
        assert!(
            mp_me.contains(
                "hash     8b8047d3ebeca5e157da8e85248892e5731d87114960e63335d74a49631f27b6"
            ),
            "{mp_me}"
        );
        assert!(mp_me.contains("kind     interval"), "{mp_me}");
        assert!(mp_me.contains("table    XXXI"), "{mp_me}");
        assert!(
            mp_me.contains("range    mp/me = 1836.15267343(11)"),
            "{mp_me}"
        );
        assert!(mp_me.contains("unit     1"), "{mp_me}");
        assert!(
            mp_me.contains("value    [45903816833/25000000, 91807633677/50000000]"),
            "{mp_me}"
        );
        assert!(mp_me.contains("rebuild  ok"), "{mp_me}");
        assert!(mp_me.contains("not P3N"), "{mp_me}");
        assert!(!mp_me.contains("receipt"), "{mp_me}");
        assert!(!mp_me.contains("theorem"), "{mp_me}");
        let mp_me_id = constant_node_id(&mp_me);
        assert_eq!(
            mp_me_id.to_hex(),
            "b26d54036d2d6b10bf50cda5ef46746fba7a7a0edeb71569428e895ee0a81dc0",
            "journaling must not change the mp_me constant payload"
        );
        assert_eq!(
            lab.store.get(mp_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mp_mmu = lab
            .exec(Command::Constant {
                name: Some("mp_mmu".into()),
            })
            .text()
            .to_string();
        assert!(mp_mmu.contains("constant  mp_mmu  node "), "{mp_mmu}");
        assert!(
            mp_mmu.contains(
                "hash     f94b9bc09239d4e20d3f7e4b9f07f39cc5f820c334fe269635d2f80c0dd88bc3"
            ),
            "{mp_mmu}"
        );
        assert!(mp_mmu.contains("kind     interval"), "{mp_mmu}");
        assert!(mp_mmu.contains("table    XXXI"), "{mp_mmu}");
        assert!(
            mp_mmu.contains("range    mp/mmu = 8.88024337(20)"),
            "{mp_mmu}"
        );
        assert!(mp_mmu.contains("unit     1"), "{mp_mmu}");
        assert!(
            mp_mmu.contains("value    [888024317/100000000, 888024357/100000000]"),
            "{mp_mmu}"
        );
        assert!(mp_mmu.contains("rebuild  ok"), "{mp_mmu}");
        assert!(mp_mmu.contains("not P3N"), "{mp_mmu}");
        assert!(!mp_mmu.contains("receipt"), "{mp_mmu}");
        assert!(!mp_mmu.contains("theorem"), "{mp_mmu}");
        let mp_mmu_id = constant_node_id(&mp_mmu);
        assert_eq!(
            mp_mmu_id.to_hex(),
            "25ce5aab6c87db67ab59a27319ed6dc0e5958ade65afad70a977c9fa43049634",
            "journaling must not change the mp_mmu constant payload"
        );
        assert_eq!(
            lab.store.get(mp_mmu_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mp_mn = lab
            .exec(Command::Constant {
                name: Some("mp_mn".into()),
            })
            .text()
            .to_string();
        assert!(mp_mn.contains("constant  mp_mn  node "), "{mp_mn}");
        assert!(
            mp_mn.contains(
                "hash     fd6d15f0f9cd99a1889486d78f316d0e3299c3b9ff8db13d6429bcb47cccb465"
            ),
            "{mp_mn}"
        );
        assert!(mp_mn.contains("kind     interval"), "{mp_mn}");
        assert!(mp_mn.contains("table    XXXI"), "{mp_mn}");
        assert!(
            mp_mn.contains("range    mp/mn = 0.99862347812(49)"),
            "{mp_mn}"
        );
        assert!(mp_mn.contains("unit     1"), "{mp_mn}");
        assert!(
            mp_mn.contains("value    [99862347763/100000000000, 99862347861/100000000000]"),
            "{mp_mn}"
        );
        assert!(mp_mn.contains("rebuild  ok"), "{mp_mn}");
        assert!(mp_mn.contains("not P3N"), "{mp_mn}");
        assert!(!mp_mn.contains("receipt"), "{mp_mn}");
        assert!(!mp_mn.contains("theorem"), "{mp_mn}");
        let mp_mn_id = constant_node_id(&mp_mn);
        assert_eq!(
            mp_mn_id.to_hex(),
            "569fded02422616274c374100f8e423cedece58684b6bd0521c559301a74577e",
            "journaling must not change the mp_mn constant payload"
        );
        assert_eq!(
            lab.store.get(mp_mn_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let e_mp = lab
            ...
            .text()
            .to_string();
        assert!(e_mp.contains("constant  e_mp  node "), "{e_mp}");
        assert!(
            e_mp.contains(
                "hash     e0404fef33540ecdef40b36cb1e14112b9e91713cccab65bb6a920fa44106611"
            ),
            "{e_mp}"
        );
        assert!(e_mp.contains("kind     interval"), "{e_mp}");
        assert!(e_mp.contains("table    XXXI"), "{e_mp}");
        assert!(
            e_mp.contains("range    e/mp = 9.5788331560(29)e7"),
            "{e_mp}"
        );
        assert!(e_mp.contains("unit     C kg^{-1}"), "{e_mp}");
        assert!(
            e_mp.contains("value    [95788331531/1000, 95788331589/1000]"),
            "{e_mp}"
        );
        assert!(e_mp.contains("rebuild  ok"), "{e_mp}");
        assert!(e_mp.contains("not P3N"), "{e_mp}");
        assert!(!e_mp.contains("receipt"), "{e_mp}");
        assert!(!e_mp.contains("theorem"), "{e_mp}");
        let e_mp_id = constant_node_id(&e_mp);
        assert_eq!(
            e_mp_id.to_hex(),
            "297019b19da4904272c51a8e04b72e02d384fc15dd567e0d888d9edbc3ac5097",
            "journaling must not change the e_mp constant payload"
        );
        assert_eq!(
            lab.store.get(e_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_p_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_p_molar.contains("constant  M_p  node "), "{m_p_molar}");
        assert!(
            m_p_molar.contains(
                "hash     6ca2722d15970d11783522598ee8879e560019865477f1735041e1c9c8180149"
            ),
            "{m_p_molar}"
        );
        assert!(m_p_molar.contains("kind     interval"), "{m_p_molar}");
        assert!(m_p_molar.contains("table    XXXI"), "{m_p_molar}");
        assert!(
            m_p_molar.contains("range    Mp = 1.00727646627(31)e-3"),
            "{m_p_molar}"
        );
        assert!(m_p_molar.contains("unit     kg mol^{-1}"), "{m_p_molar}");
        assert!(
            m_p_molar.contains("value    [25181911649/25000000000000, 50363823329/50000000000000]"),
            "{m_p_molar}"
        );
        assert!(m_p_molar.contains("rebuild  ok"), "{m_p_molar}");
        assert!(m_p_molar.contains("not P3N"), "{m_p_molar}");
        assert!(!m_p_molar.contains("receipt"), "{m_p_molar}");
        assert!(!m_p_molar.contains("theorem"), "{m_p_molar}");
        let m_p_molar_id = constant_node_id(&m_p_molar);
        assert_eq!(
            m_p_molar_id.to_hex(),
            "aa03e8a73d0ec0d8805ac00559d7c6db336f50fc102dc2d7f295adadfb291162",
            "journaling must not change the M_p constant payload"
        );
        assert_eq!(
            lab.store.get(m_p_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let lambda_c_p = lab
            .exec(Command::Constant {
                name: Some("lambda_C_p".into()),
            })
            .text()
            .to_string();
        assert!(
            lambda_c_p.contains("constant  lambda_C_p  node "),
            "{lambda_c_p}"
        );
        assert!(
            lambda_c_p.contains(
                "hash     439c5267d7664a8bd5c359b40f5a2291b0b364e0a915c57378abd6787b9d5a08"
            ),
            "{lambda_c_p}"
        );
        assert!(lambda_c_p.contains("kind     interval"), "{lambda_c_p}");
        assert!(lambda_c_p.contains("table    XXXI"), "{lambda_c_p}");
        assert!(
            lambda_c_p.contains("range    lambda_C_p = 1.32140985539(40)e-15"),
            "{lambda_c_p}"
        );
        assert!(lambda_c_p.contains("unit     m"), "{lambda_c_p}");
        assert!(
            lambda_c_p.contains(
                "value    [132140985499/100000000000000000000000000, 132140985579/100000000000000000000000000]"
            ),
            "{lambda_c_p}"
        );
        assert!(lambda_c_p.contains("rebuild  ok"), "{lambda_c_p}");
        assert!(lambda_c_p.contains("not P3N"), "{lambda_c_p}");
        assert!(!lambda_c_p.contains("receipt"), "{lambda_c_p}");
        assert!(!lambda_c_p.contains("theorem"), "{lambda_c_p}");
        let lambda_c_p_id = constant_node_id(&lambda_c_p);
        assert_eq!(
            lambda_c_p_id.to_hex(),
            "7c05dfef72a29e8b2ede826332febcd0331436b3be2f6b97f83f2489d1ee6840",
            "journaling must not change the lambda_C_p constant payload"
        );
        assert_eq!(
            lab.store.get(lambda_c_p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let rp = lab
            ...
            .text()
            .to_string();
        assert!(rp.contains("constant  rp  node "), "{rp}");
        assert!(
            rp.contains(
                "hash     722e45d219142c882b4475408333d68c48c5952af08680c0a16bed5af1bfc944"
            ),
            "{rp}"
        );
        assert!(rp.contains("kind     interval"), "{rp}");
        assert!(rp.contains("table    XXXI"), "{rp}");
        assert!(rp.contains("range    rp = 8.414(19)e-16"), "{rp}");
        assert!(rp.contains("unit     m"), "{rp}");
        assert!(
            rp.contains("value    [1679/2000000000000000000, 8433/10000000000000000000]"),
            "{rp}"
        );
        assert!(rp.contains("rebuild  ok"), "{rp}");
        assert!(rp.contains("not P3N"), "{rp}");
        assert!(!rp.contains("receipt"), "{rp}");
        assert!(!rp.contains("theorem"), "{rp}");
        let rp_id = constant_node_id(&rp);
        assert_eq!(
            rp_id.to_hex(),
            "1cae30455d7ad0da2a7cee5aea88b53749b93936a1d209c3e4bb514f1d9c3058",
            "journaling must not change the rp constant payload"
        );
        assert_eq!(
            lab.store.get(rp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_p = lab
            ...
            .text()
            .to_string();
        assert!(mu_p.contains("constant  mu_p  node "), "{mu_p}");
        assert!(
            mu_p.contains(
                "hash     bf987c5fccc4ef40691f126024092320bc335b7942323fa90675a28a250e304c"
            ),
            "{mu_p}"
        );
        assert!(mu_p.contains("kind     interval"), "{mu_p}");
        assert!(mu_p.contains("table    XXXI"), "{mu_p}");
        assert!(
            mu_p.contains("range    mu_p = 1.41060679736(60)e-26"),
            "{mu_p}"
        );
        assert!(mu_p.contains("unit     J T^{-1}"), "{mu_p}");
        assert!(
            mu_p.contains(
                "value    [35265169919/2500000000000000000000000000000000000, 35265169949/2500000000000000000000000000000000000]"
            ),
            "{mu_p}"
        );
        assert!(mu_p.contains("rebuild  ok"), "{mu_p}");
        assert!(mu_p.contains("not P3N"), "{mu_p}");
        assert!(!mu_p.contains("receipt"), "{mu_p}");
        assert!(!mu_p.contains("theorem"), "{mu_p}");
        let mu_p_id = constant_node_id(&mu_p);
        assert_eq!(
            mu_p_id.to_hex(),
            "05785ed691b20e86ee5246df2dad1d6a0f90fe6bb85bd35f06c8561a6194622e",
            "journaling must not change the mu_p constant payload"
        );
        assert_eq!(
            lab.store.get(mu_p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_p_mu_b = lab
            .exec(Command::Constant {
                name: Some("mu_p_muB".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_p_mu_b.contains("constant  mu_p_muB  node "),
            "{mu_p_mu_b}"
        );
        assert!(
            mu_p_mu_b.contains(
                "hash     cadc896f8c2b6f960aa051bb05a70efb8b2e58bc36b7230787148191227cff3a"
            ),
            "{mu_p_mu_b}"
        );
        assert!(mu_p_mu_b.contains("kind     interval"), "{mu_p_mu_b}");
        assert!(mu_p_mu_b.contains("table    XXXI"), "{mu_p_mu_b}");
        assert!(
            mu_p_mu_b.contains("range    mu_p/muB = 1.52103220230(46)e-3"),
            "{mu_p_mu_b}"
        );
        assert!(mu_p_mu_b.contains("unit     1"), "{mu_p_mu_b}");
        assert!(
            mu_p_mu_b.contains("value    [19012902523/12500000000000, 38025805069/25000000000000]"),
            "{mu_p_mu_b}"
        );
        assert!(mu_p_mu_b.contains("rebuild  ok"), "{mu_p_mu_b}");
        assert!(mu_p_mu_b.contains("not P3N"), "{mu_p_mu_b}");
        assert!(!mu_p_mu_b.contains("receipt"), "{mu_p_mu_b}");
        assert!(!mu_p_mu_b.contains("theorem"), "{mu_p_mu_b}");
        let mu_p_mu_b_id = constant_node_id(&mu_p_mu_b);
        assert_eq!(
            mu_p_mu_b_id.to_hex(),
            "c5a6302bb2a038bac321db0f3ab362024dfed4b37cf3bc42c1a2850ec255af2a",
            "journaling must not change the mu_p_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_p_mu_b_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_p_mu_n = lab
            .exec(Command::Constant {
                name: Some("mu_p_muN".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_p_mu_n.contains("constant  mu_p_muN  node "),
            "{mu_p_mu_n}"
        );
        assert!(
            mu_p_mu_n.contains(
                "hash     3d1337d5d8845bcc477feee0bef86a648b907c20087669165db91902a1f14fd8"
            ),
            "{mu_p_mu_n}"
        );
        assert!(mu_p_mu_n.contains("kind     interval"), "{mu_p_mu_n}");
        assert!(mu_p_mu_n.contains("table    XXXI"), "{mu_p_mu_n}");
        assert!(
            mu_p_mu_n.contains("range    mu_p/muN = 2.79284734463(82)"),
            "{mu_p_mu_n}"
        );
        assert!(mu_p_mu_n.contains("unit     1"), "{mu_p_mu_n}");
        assert!(
            mu_p_mu_n.contains("value    [279284734381/100000000000, 55856946909/20000000000]"),
            "{mu_p_mu_n}"
        );
        assert!(mu_p_mu_n.contains("rebuild  ok"), "{mu_p_mu_n}");
        assert!(mu_p_mu_n.contains("not P3N"), "{mu_p_mu_n}");
        assert!(!mu_p_mu_n.contains("receipt"), "{mu_p_mu_n}");
        assert!(!mu_p_mu_n.contains("theorem"), "{mu_p_mu_n}");
        let mu_p_mu_n_id = constant_node_id(&mu_p_mu_n);
        assert_eq!(
            mu_p_mu_n_id.to_hex(),
            "4a808f05f3bd65df18072a9c679ee49155dd2f621824f039a0b83990ae14cc65",
            "journaling must not change the mu_p_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_p_mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gp = lab
            ...
            .text()
            .to_string();
        assert!(gp.contains("constant  gp  node "), "{gp}");
        assert!(
            gp.contains(
                "hash     9a1a482bd1adcc3258834dd9275ce119d29903b398307609f17788e5f4a6874d"
            ),
            "{gp}"
        );
        assert!(gp.contains("kind     interval"), "{gp}");
        assert!(gp.contains("table    XXXI"), "{gp}");
        assert!(gp.contains("range    gp = 5.5856946893(16)"), "{gp}");
        assert!(gp.contains("unit     1"), "{gp}");
        assert!(
            gp.contains("value    [55856946877/10000000000, 55856946909/10000000000]"),
            "{gp}"
        );
        assert!(gp.contains("rebuild  ok"), "{gp}");
        assert!(gp.contains("not P3N"), "{gp}");
        assert!(!gp.contains("receipt"), "{gp}");
        assert!(!gp.contains("theorem"), "{gp}");
        let gp_id = constant_node_id(&gp);
        assert_eq!(
            gp_id.to_hex(),
            "ae15d6d01a97e5ff6a9e8fdab3c5e3e3e262e41aab803a85ee3a8f8f7e471169",
            "journaling must not change the gp constant payload"
        );
        assert_eq!(
            lab.store.get(gp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_p_mun = lab
            ...
            .text()
            .to_string();
        assert!(mu_p_mun.contains("constant  mu_p_mun  node "), "{mu_p_mun}");
        assert!(
            mu_p_mun.contains(
                "hash     962ad1f4f5a18ff54eb9bb0acff22f5fa8b96537291568c387b82f8bd41abd98"
            ),
            "{mu_p_mun}"
        );
        assert!(mu_p_mun.contains("kind     interval"), "{mu_p_mun}");
        assert!(mu_p_mun.contains("table    XXXI"), "{mu_p_mun}");
        assert!(
            mu_p_mun.contains("range    mu_p/mun = -1.45989805(34)"),
            "{mu_p_mun}"
        );
        assert!(mu_p_mun.contains("unit     1"), "{mu_p_mun}");
        assert!(
            mu_p_mun.contains("value    [-145989839/100000000, -145989771/100000000]"),
            "{mu_p_mun}"
        );
        assert!(mu_p_mun.contains("rebuild  ok"), "{mu_p_mun}");
        assert!(mu_p_mun.contains("not P3N"), "{mu_p_mun}");
        assert!(!mu_p_mun.contains("receipt"), "{mu_p_mun}");
        assert!(!mu_p_mun.contains("theorem"), "{mu_p_mun}");
        let mu_p_mun_id = constant_node_id(&mu_p_mun);
        assert_eq!(
            mu_p_mun_id.to_hex(),
            "865524beff711d79dbcb0c655d6494882c2a63f8988d1476a15c03d34daa29ce",
            "journaling must not change the mu_p_mun constant payload"
        );
        assert_eq!(
            lab.store.get(mu_p_mun_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu0p = lab
            ...
            .text()
            .to_string();
        assert!(mu0p.contains("constant  mu0p  node "), "{mu0p}");
        assert!(
            mu0p.contains(
                "hash     e515a5f9475cddcdcb01f653117e63e25dbe9e3d63d7024a3b522ffe362679cb"
            ),
            "{mu0p}"
        );
        assert!(mu0p.contains("kind     interval"), "{mu0p}");
        assert!(mu0p.contains("table    XXXI"), "{mu0p}");
        assert!(
            mu0p.contains("range    mu0p = 1.410570560(15)e-26"),
            "{mu0p}"
        );
        assert!(mu0p.contains("unit     J T^{-1}"), "{mu0p}");
        assert!(mu0p.contains("value    [282114109/20000000000000000000000000000000000, 56422823/4000000000000000000000000000000000]"), "{mu0p}");
        assert!(mu0p.contains("rebuild  ok"), "{mu0p}");
        assert!(mu0p.contains("not P3N"), "{mu0p}");
        assert!(!mu0p.contains("receipt"), "{mu0p}");
        assert!(!mu0p.contains("theorem"), "{mu0p}");
        let mu0p_id = constant_node_id(&mu0p);
        assert_eq!(
            mu0p_id.to_hex(),
            "06e2160c2ed44fd38fe9766274f2de71e2ec60d4e4c705763ba3446f07164739",
            "journaling must not change the mu0p constant payload"
        );
        assert_eq!(
            lab.store.get(mu0p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0p_mu_b = lab
            .exec(Command::Constant {
                name: Some("mu0p_muB".into()),
            })
            .text()
            .to_string();
        assert!(
            mu0p_mu_b.contains("constant  mu0p_muB  node "),
            "{mu0p_mu_b}"
        );
        assert!(
            mu0p_mu_b.contains(
                "hash     6ff346294d2cdeefe14d9611bc3db150e51438fb3a864406768858faee733791"
            ),
            "{mu0p_mu_b}"
        );
        assert!(mu0p_mu_b.contains("kind     interval"), "{mu0p_mu_b}");
        assert!(mu0p_mu_b.contains("table    XXXI"), "{mu0p_mu_b}");
        assert!(
            mu0p_mu_b.contains("range    mu0p/muB = 1.520993128(17)e-3"),
            "{mu0p_mu_b}"
        );
        assert!(mu0p_mu_b.contains("unit     1"), "{mu0p_mu_b}");
        assert!(
            mu0p_mu_b.contains("value    [1520993111/1000000000000, 304198629/200000000000]"),
            "{mu0p_mu_b}"
        );
        assert!(mu0p_mu_b.contains("rebuild  ok"), "{mu0p_mu_b}");
        assert!(mu0p_mu_b.contains("not P3N"), "{mu0p_mu_b}");
        assert!(!mu0p_mu_b.contains("receipt"), "{mu0p_mu_b}");
        assert!(!mu0p_mu_b.contains("theorem"), "{mu0p_mu_b}");
        let mu0p_mu_b_id = constant_node_id(&mu0p_mu_b);
        assert_eq!(
            mu0p_mu_b_id.to_hex(),
            "6204531acecb1bcba0d4e881586e346d94f771bf66c19c8c43148c5de99cbbb6",
            "journaling must not change the mu0p_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu0p_mu_b_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0p_mu_n = lab
            .exec(Command::Constant {
                name: Some("mu0p_muN".into()),
            })
            .text()
            .to_string();
        assert!(
            mu0p_mu_n.contains("constant  mu0p_muN  node "),
            "{mu0p_mu_n}"
        );
        assert!(
            mu0p_mu_n.contains(
                "hash     9f7913d246532a470d0c2dfe8ccfc7613aaafedaf22f42f0e5eb72d46fdfb9ed"
            ),
            "{mu0p_mu_n}"
        );
        assert!(mu0p_mu_n.contains("kind     interval"), "{mu0p_mu_n}");
        assert!(mu0p_mu_n.contains("table    XXXI"), "{mu0p_mu_n}");
        assert!(
            mu0p_mu_n.contains("range    mu0p/muN = 2.792775599(30)"),
            "{mu0p_mu_n}"
        );
        assert!(mu0p_mu_n.contains("unit     1"), "{mu0p_mu_n}");
        assert!(
            mu0p_mu_n.contains("value    [2792775569/1000000000, 2792775629/1000000000]"),
            "{mu0p_mu_n}"
        );
        assert!(mu0p_mu_n.contains("rebuild  ok"), "{mu0p_mu_n}");
        assert!(mu0p_mu_n.contains("not P3N"), "{mu0p_mu_n}");
        assert!(!mu0p_mu_n.contains("receipt"), "{mu0p_mu_n}");
        assert!(!mu0p_mu_n.contains("theorem"), "{mu0p_mu_n}");
        let mu0p_mu_n_id = constant_node_id(&mu0p_mu_n);
        assert_eq!(
            mu0p_mu_n_id.to_hex(),
            "842df9c30e0e80c22044f56801b4c741110707db286770da4a3f2d3ff25bf24a",
            "journaling must not change the mu0p_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu0p_mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let sigma0p = lab
            .exec(Command::Constant {
                name: Some("sigma0p".into()),
            })
            .text()
            .to_string();
        assert!(sigma0p.contains("constant  sigma0p  node "), "{sigma0p}");
        assert!(
            sigma0p.contains(
                "hash     2bd71c8a0c870e0f41ca7ec9ceada123c78583e8af24c97e5ad1918069bf1bd1"
            ),
            "{sigma0p}"
        );
        assert!(sigma0p.contains("kind     interval"), "{sigma0p}");
        assert!(sigma0p.contains("table    XXXI"), "{sigma0p}");
        assert!(
            sigma0p.contains("range    sigma0p = 2.5689(11)e-5"),
            "{sigma0p}"
        );
        assert!(sigma0p.contains("unit     1"), "{sigma0p}");
        assert!(
            sigma0p.contains("value    [12839/500000000, 257/10000000]"),
            "{sigma0p}"
        );
        assert!(sigma0p.contains("rebuild  ok"), "{sigma0p}");
        assert!(sigma0p.contains("not P3N"), "{sigma0p}");
        assert!(!sigma0p.contains("receipt"), "{sigma0p}");
        assert!(!sigma0p.contains("theorem"), "{sigma0p}");
        let sigma0p_id = constant_node_id(&sigma0p);
        assert_eq!(
            sigma0p_id.to_hex(),
            "5750e14f1d96e1d0a29d15f26fab1bc146ff8fbcc345daaea980d499198050b3",
            "journaling must not change the sigma0p constant payload"
        );
        assert_eq!(
            lab.store.get(sigma0p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_n = lab
            ...
            .text()
            .to_string();
        assert!(m_n.contains("constant  m_n  node "), "{m_n}");
        assert!(
            m_n.contains(
                "hash     a03f5c1c27081122d30ec7656df798ad4ad99413bb03790a40b3da58b76d1a0f"
            ),
            "{m_n}"
        );
        assert!(m_n.contains("kind     interval"), "{m_n}");
        assert!(m_n.contains("table    XXXI"), "{m_n}");
        assert!(m_n.contains("range    mn = 1.67492749804(95)e-27"), "{m_n}");
        assert!(m_n.contains("unit     kg"), "{m_n}");
        assert!(
            m_n.contains("value    [167492749709/100000000000000000000000000000000000000, 167492749899/100000000000000000000000000000000000000]"),
            "{m_n}"
        );
        assert!(m_n.contains("rebuild  ok"), "{m_n}");
        assert!(m_n.contains("not P3N"), "{m_n}");
        assert!(!m_n.contains("receipt"), "{m_n}");
        assert!(!m_n.contains("theorem"), "{m_n}");
        let m_n_id = constant_node_id(&m_n);
        assert_eq!(
            m_n_id.to_hex(),
            "226446ffad910bf91f0ec6f6a4ea8861e515ea82b319cbf2e077995edc051147",
            "journaling must not change the m_n constant payload"
        );
        assert_eq!(
            lab.store.get(m_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_n_u = lab
            .exec(Command::Constant {
                name: Some("m_n_u".into()),
            })
            .text()
            .to_string();
        assert!(m_n_u.contains("constant  m_n_u  node "), "{m_n_u}");
        assert!(
            m_n_u.contains(
                "hash     7b6d3f11b99b03358a438ae921f035e5e3b581b543c06680df7420e64dfa7241"
            ),
            "{m_n_u}"
        );
        assert!(m_n_u.contains("kind     interval"), "{m_n_u}");
        assert!(m_n_u.contains("table    XXXI"), "{m_n_u}");
        assert!(
            m_n_u.contains("range    mn_u = 1.00866491595(49)"),
            "{m_n_u}"
        );
        assert!(m_n_u.contains("unit     u"), "{m_n_u}");
        assert!(
            m_n_u.contains("value    [50433245773/50000000000, 25216622911/25000000000]"),
            "{m_n_u}"
        );
        assert!(m_n_u.contains("rebuild  ok"), "{m_n_u}");
        assert!(m_n_u.contains("not P3N"), "{m_n_u}");
        assert!(!m_n_u.contains("receipt"), "{m_n_u}");
        assert!(!m_n_u.contains("theorem"), "{m_n_u}");
        let m_n_u_id = constant_node_id(&m_n_u);
        assert_eq!(
            m_n_u_id.to_hex(),
            "97d71ba92203fee5acad8633da5489a51a8339cf70a47379866690bce701e6e9",
            "journaling must not change the m_n_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_n_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_n_c2 = lab
            .exec(Command::Constant {
                name: Some("m_n_c2".into()),
            })
            .text()
            .to_string();
        assert!(m_n_c2.contains("constant  m_n_c2  node "), "{m_n_c2}");
        assert!(
            m_n_c2.contains(
                "hash     6e677c893f10770fdedc46ec68d0c6de2321e9d5fd03819ca0bc0054308ebff4"
            ),
            "{m_n_c2}"
        );
        assert!(m_n_c2.contains("kind     interval"), "{m_n_c2}");
        assert!(m_n_c2.contains("table    XXXI"), "{m_n_c2}");
        assert!(
            m_n_c2.contains("range    mnc2 = 1.50534976287(86)e-10"),
            "{m_n_c2}"
        );
        assert!(m_n_c2.contains("unit     J"), "{m_n_c2}");
        assert!(
            m_n_c2.contains("value    [150534976201/1000000000000000000000, 150534976373/1000000000000000000000]"),
            "{m_n_c2}"
        );
        assert!(m_n_c2.contains("rebuild  ok"), "{m_n_c2}");
        assert!(m_n_c2.contains("not P3N"), "{m_n_c2}");
        assert!(!m_n_c2.contains("receipt"), "{m_n_c2}");
        assert!(!m_n_c2.contains("theorem"), "{m_n_c2}");
        let m_n_c2_id = constant_node_id(&m_n_c2);
        assert_eq!(
            m_n_c2_id.to_hex(),
            "d11ab471f5ca712d524882673709646b0e6480bd019d01af2813441bb1389825",
            "journaling must not change the m_n_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_n_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_n_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_n_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_n_c2_mev.contains("constant  m_n_c2_MeV  node "),
            "{m_n_c2_mev}"
        );
        assert!(
            m_n_c2_mev.contains(
                "hash     7f7aff06d346ee861dfaf56598a565600b09c0171deb1f46617ccc7a08aefef8"
            ),
            "{m_n_c2_mev}"
        );
        assert!(m_n_c2_mev.contains("kind     interval"), "{m_n_c2_mev}");
        assert!(m_n_c2_mev.contains("table    XXXI"), "{m_n_c2_mev}");
        assert!(
            m_n_c2_mev.contains("range    mnc2_MeV = 939.56542052(54)"),
            "{m_n_c2_mev}"
        );
        assert!(m_n_c2_mev.contains("unit     MeV"), "{m_n_c2_mev}");
        assert!(
            m_n_c2_mev.contains("value    [46978270999/50000000, 46978271053/50000000]"),
            "{m_n_c2_mev}"
        );
        assert!(m_n_c2_mev.contains("rebuild  ok"), "{m_n_c2_mev}");
        assert!(m_n_c2_mev.contains("not P3N"), "{m_n_c2_mev}");
        assert!(!m_n_c2_mev.contains("receipt"), "{m_n_c2_mev}");
        assert!(!m_n_c2_mev.contains("theorem"), "{m_n_c2_mev}");
        let m_n_c2_mev_id = constant_node_id(&m_n_c2_mev);
        assert_eq!(
            m_n_c2_mev_id.to_hex(),
            "8e291e13da25fdb5817f4ce46a62918d72e82a1a023efbac440e986768ccc034",
            "journaling must not change the m_n_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_n_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_me = lab
            .exec(Command::Constant {
                name: Some("mn_me".into()),
            })
            .text()
            .to_string();
        assert!(mn_me.contains("constant  mn_me  node "), "{mn_me}");
        assert!(
            mn_me.contains(
                "hash     024275bcd4128d6844efaf9740b16e69a8cfbb349d257465ad96a3cbada32b00"
            ),
            "{mn_me}"
        );
        assert!(mn_me.contains("kind     interval"), "{mn_me}");
        assert!(mn_me.contains("table    XXXI"), "{mn_me}");
        assert!(
            mn_me.contains("range    mn/me = 1838.68366173(89)"),
            "{mn_me}"
        );
        assert!(mn_me.contains("unit     1"), "{mn_me}");
        assert!(
            mn_me.contains("value    [45967091521/25000000, 91934183131/50000000]"),
            "{mn_me}"
        );
        assert!(mn_me.contains("rebuild  ok"), "{mn_me}");
        assert!(mn_me.contains("not P3N"), "{mn_me}");
        assert!(!mn_me.contains("receipt"), "{mn_me}");
        assert!(!mn_me.contains("theorem"), "{mn_me}");
        let mn_me_id = constant_node_id(&mn_me);
        assert_eq!(
            mn_me_id.to_hex(),
            "785b3da762cf12d86766853b74f872bc4c328111748be893a1ca4b39fef77fbb",
            "journaling must not change the mn_me constant payload"
        );
        assert_eq!(
            lab.store.get(mn_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_mmu = lab
            .exec(Command::Constant {
                name: Some("mn_mmu".into()),
            })
            .text()
            .to_string();
        assert!(mn_mmu.contains("constant  mn_mmu  node "), "{mn_mmu}");
        assert!(
            mn_mmu.contains(
                "hash     ab9106af98fa15acd9352bb71a5874aa8f151d4530172a7f704bbb8cc96e52b4"
            ),
            "{mn_mmu}"
        );
        assert!(mn_mmu.contains("kind     interval"), "{mn_mmu}");
        assert!(mn_mmu.contains("table    XXXI"), "{mn_mmu}");
        assert!(
            mn_mmu.contains("range    mn/mmu = 8.89248406(20)"),
            "{mn_mmu}"
        );
        assert!(mn_mmu.contains("unit     1"), "{mn_mmu}");
        assert!(
            mn_mmu.contains("value    [444624193/50000000, 444624213/50000000]"),
            "{mn_mmu}"
        );
        assert!(mn_mmu.contains("rebuild  ok"), "{mn_mmu}");
        assert!(mn_mmu.contains("not P3N"), "{mn_mmu}");
        assert!(!mn_mmu.contains("receipt"), "{mn_mmu}");
        assert!(!mn_mmu.contains("theorem"), "{mn_mmu}");
        let mn_mmu_id = constant_node_id(&mn_mmu);
        assert_eq!(
            mn_mmu_id.to_hex(),
            "cf2fb2b1ded7aec6b2512de342860fb4bc3a9f7be2f2d8ce37fd0ef180047f12",
            "journaling must not change the mn_mmu constant payload"
        );
        assert_eq!(
            lab.store.get(mn_mmu_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_mp = lab
            .exec(Command::Constant {
                name: Some("mn_mp".into()),
            })
            .text()
            .to_string();
        assert!(mn_mp.contains("constant  mn_mp  node "), "{mn_mp}");
        assert!(
            mn_mp.contains(
                "hash     4a1cc9e1870d573594b4de7b64f6ec1667f13da26f6cb8b72ae351eed5c89eb4"
            ),
            "{mn_mp}"
        );
        assert!(mn_mp.contains("kind     interval"), "{mn_mp}");
        assert!(mn_mp.contains("table    XXXI"), "{mn_mp}");
        assert!(
            mn_mp.contains("range    mn/mp = 1.00137841931(49)"),
            "{mn_mp}"
        );
        assert!(mn_mp.contains("unit     1"), "{mn_mp}");
        assert!(
            mn_mp.contains("value    [50068920941/50000000000, 5006892099/5000000000]"),
            "{mn_mp}"
        );
        assert!(mn_mp.contains("rebuild  ok"), "{mn_mp}");
        assert!(mn_mp.contains("not P3N"), "{mn_mp}");
        assert!(!mn_mp.contains("receipt"), "{mn_mp}");
        assert!(!mn_mp.contains("theorem"), "{mn_mp}");
        let mn_mp_id = constant_node_id(&mn_mp);
        assert_eq!(
            mn_mp_id.to_hex(),
            "945a9d7cd8a8baf4c852e73d2b86cb4cca450eeee92775737e6b50877abce21d",
            "journaling must not change the mn_mp constant payload"
        );
        assert_eq!(
            lab.store.get(mn_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_minus_mp = lab
            .exec(Command::Constant {
                name: Some("mn_minus_mp".into()),
            })
            .text()
            .to_string();
        assert!(
            mn_minus_mp.contains("constant  mn_minus_mp  node "),
            "{mn_minus_mp}"
        );
        assert!(
            mn_minus_mp.contains(
                "hash     16a765afe5d22205b54dcb17568b53b031f246b75748ece1624d4e553dbca66a"
            ),
            "{mn_minus_mp}"
        );
        assert!(mn_minus_mp.contains("kind     interval"), "{mn_minus_mp}");
        assert!(mn_minus_mp.contains("table    XXXI"), "{mn_minus_mp}");
        assert!(
            mn_minus_mp.contains("range    mn-mp = 2.30557435(82)e-30"),
            "{mn_minus_mp}"
        );
        assert!(mn_minus_mp.contains("unit     kg"), "{mn_minus_mp}");
        assert!(
            mn_minus_mp.contains(
                "value    [230557353/100000000000000000000000000000000000000, 230557517/100000000000000000000000000000000000000]"
            ),
            "{mn_minus_mp}"
        );
        assert!(mn_minus_mp.contains("rebuild  ok"), "{mn_minus_mp}");
        assert!(mn_minus_mp.contains("not P3N"), "{mn_minus_mp}");
        assert!(!mn_minus_mp.contains("receipt"), "{mn_minus_mp}");
        assert!(!mn_minus_mp.contains("theorem"), "{mn_minus_mp}");
        let mn_minus_mp_id = constant_node_id(&mn_minus_mp);
        assert_eq!(
            mn_minus_mp_id.to_hex(),
            "c02b5d5688fa618f7e226612267689b8a0f1428e6b1f7ead92f7b5a5959789da",
            "journaling must not change the mn_minus_mp constant payload"
        );
        assert_eq!(
            lab.store.get(mn_minus_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_minus_mp_u = lab
            .exec(Command::Constant {
                name: Some("mn_minus_mp_u".into()),
            })
            .text()
            .to_string();
        assert!(
            mn_minus_mp_u.contains("constant  mn_minus_mp_u  node "),
            "{mn_minus_mp_u}"
        );
        assert!(
            mn_minus_mp_u.contains(
                "hash     3e84ef4f23bdc6d6c4b0f17c5ee3535eb8352184eba5836d253750fabdcbf122"
            ),
            "{mn_minus_mp_u}"
        );
        assert!(
            mn_minus_mp_u.contains("kind     interval"),
            "{mn_minus_mp_u}"
        );
        assert!(mn_minus_mp_u.contains("table    XXXI"), "{mn_minus_mp_u}");
        assert!(
            mn_minus_mp_u.contains("range    mn-mp_u = 1.38844933(49)e-3"),
            "{mn_minus_mp_u}"
        );
        assert!(mn_minus_mp_u.contains("unit     u"), "{mn_minus_mp_u}");
        assert!(
            mn_minus_mp_u.contains("value    [34711221/25000000000, 69422491/50000000000]"),
            "{mn_minus_mp_u}"
        );
        assert!(mn_minus_mp_u.contains("rebuild  ok"), "{mn_minus_mp_u}");
        assert!(mn_minus_mp_u.contains("not P3N"), "{mn_minus_mp_u}");
        assert!(!mn_minus_mp_u.contains("receipt"), "{mn_minus_mp_u}");
        assert!(!mn_minus_mp_u.contains("theorem"), "{mn_minus_mp_u}");
        let mn_minus_mp_u_id = constant_node_id(&mn_minus_mp_u);
        assert_eq!(
            mn_minus_mp_u_id.to_hex(),
            "393944c68f266ffe577f831ae5c03ecbccfc9f2ee336efd281ca912b89fbdb3c",
            "journaling must not change the mn_minus_mp_u constant payload"
        );
        assert_eq!(
            lab.store.get(mn_minus_mp_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_minus_mp_c2 = lab
            .exec(Command::Constant {
                name: Some("mn_minus_mp_c2".into()),
            })
            .text()
            .to_string();
        assert!(
            mn_minus_mp_c2.contains("constant  mn_minus_mp_c2  node "),
            "{mn_minus_mp_c2}"
        );
        assert!(
            mn_minus_mp_c2.contains(
                "hash     fa3bd81d58322a2d13a2e2c98b628fef9fb69bcdcf7c3335cd8e26d6b2fb2c45"
            ),
            "{mn_minus_mp_c2}"
        );
        assert!(
            mn_minus_mp_c2.contains("kind     interval"),
            "{mn_minus_mp_c2}"
        );
        assert!(mn_minus_mp_c2.contains("table    XXXI"), "{mn_minus_mp_c2}");
        assert!(
            mn_minus_mp_c2.contains("range    mn-mp_c2 = 2.07214689(74)e-13"),
            "{mn_minus_mp_c2}"
        );
        assert!(mn_minus_mp_c2.contains("unit     J"), "{mn_minus_mp_c2}");
        assert!(
            mn_minus_mp_c2.contains(
                "value    [41442923/200000000000000000000, 207214763/1000000000000000000000]"
            ),
            "{mn_minus_mp_c2}"
        );
        assert!(mn_minus_mp_c2.contains("rebuild  ok"), "{mn_minus_mp_c2}");
        assert!(mn_minus_mp_c2.contains("not P3N"), "{mn_minus_mp_c2}");
        assert!(!mn_minus_mp_c2.contains("receipt"), "{mn_minus_mp_c2}");
        assert!(!mn_minus_mp_c2.contains("theorem"), "{mn_minus_mp_c2}");
        let mn_minus_mp_c2_id = constant_node_id(&mn_minus_mp_c2);
        assert_eq!(
            mn_minus_mp_c2_id.to_hex(),
            "d80066ce06701b9bfe83a44cfb9aa8a34bacc23e1d1a3860666ec0b317be0e08",
            "journaling must not change the mn_minus_mp_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(mn_minus_mp_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mn_minus_mp_c2_mev = lab
            .exec(Command::Constant {
                name: Some("mn_minus_mp_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            mn_minus_mp_c2_mev.contains("constant  mn_minus_mp_c2_MeV  node "),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains(
                "hash     1049facdec3000d011eb1c003e5a3c4ee952917b78aaed18b503398a2627515a"
            ),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("kind     interval"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("table    XXXI"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("range    mn-mp_c2_MeV = 1.29333236(46)"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("unit     MeV"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("value    [12933319/10000000, 64666641/50000000]"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("rebuild  ok"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            mn_minus_mp_c2_mev.contains("not P3N"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            !mn_minus_mp_c2_mev.contains("receipt"),
            "{mn_minus_mp_c2_mev}"
        );
        assert!(
            !mn_minus_mp_c2_mev.contains("theorem"),
            "{mn_minus_mp_c2_mev}"
        );
        let mn_minus_mp_c2_mev_id = constant_node_id(&mn_minus_mp_c2_mev);
        assert_eq!(
            mn_minus_mp_c2_mev_id.to_hex(),
            "3755ecb6520d54be37dbc22d659304b2504065b84d4a3b41422db7efe9a2ec6d",
            "journaling must not change the mn_minus_mp_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(mn_minus_mp_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_n_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_n_molar.contains("constant  M_n  node "), "{m_n_molar}");
        assert!(
            m_n_molar.contains(
                "hash     503014b9a1cfa5be5f983c7cd8f477ec6fa601225d084f3acd22ab41b88151d5"
            ),
            "{m_n_molar}"
        );
        assert!(m_n_molar.contains("kind     interval"), "{m_n_molar}");
        assert!(m_n_molar.contains("table    XXXI"), "{m_n_molar}");
        assert!(
            m_n_molar.contains("range    Mn = 1.00866491560(57)e-3"),
            "{m_n_molar}"
        );
        assert!(m_n_molar.contains("unit     kg mol^{-1}"), "{m_n_molar}");
        assert!(
            m_n_molar
                .contains("value    [100866491503/100000000000000, 100866491617/100000000000000]"),
            "{m_n_molar}"
        );
        assert!(m_n_molar.contains("rebuild  ok"), "{m_n_molar}");
        assert!(m_n_molar.contains("not P3N"), "{m_n_molar}");
        assert!(!m_n_molar.contains("receipt"), "{m_n_molar}");
        assert!(!m_n_molar.contains("theorem"), "{m_n_molar}");
        let m_n_molar_id = constant_node_id(&m_n_molar);
        assert_eq!(
            m_n_molar_id.to_hex(),
            "8351e225509b669ee96bf0e8e63baff12e020575065270ef2d54cc77877ed1c0",
            "journaling must not change the M_n constant payload"
        );
        assert_eq!(
            lab.store.get(m_n_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let lambda_c_n = lab
            .exec(Command::Constant {
                name: Some("lambda_C_n".into()),
            })
            .text()
            .to_string();
        assert!(
            lambda_c_n.contains("constant  lambda_C_n  node "),
            "{lambda_c_n}"
        );
        assert!(
            lambda_c_n.contains(
                "hash     415d4c31f9aafa35bb84bf48212aa1669b889e9e3e147dec957a1d64a551a449"
            ),
            "{lambda_c_n}"
        );
        assert!(lambda_c_n.contains("kind     interval"), "{lambda_c_n}");
        assert!(lambda_c_n.contains("table    XXXI"), "{lambda_c_n}");
        assert!(
            lambda_c_n.contains("range    lambda_C_n = 1.31959090581(75)e-15"),
            "{lambda_c_n}"
        );
        assert!(lambda_c_n.contains("unit     m"), "{lambda_c_n}");
        assert!(
            lambda_c_n.contains(
                "value    [65979545253/50000000000000000000000000, 4123721583/3125000000000000000000000]"
            ),
            "{lambda_c_n}"
        );
        assert!(lambda_c_n.contains("rebuild  ok"), "{lambda_c_n}");
        assert!(lambda_c_n.contains("not P3N"), "{lambda_c_n}");
        assert!(!lambda_c_n.contains("receipt"), "{lambda_c_n}");
        assert!(!lambda_c_n.contains("theorem"), "{lambda_c_n}");
        let lambda_c_n_id = constant_node_id(&lambda_c_n);
        assert_eq!(
            lambda_c_n_id.to_hex(),
            "7e23a68e4206617cddff21229e106f6cb57a4a7f1eb41316ee4e2c251a2ac85a",
            "journaling must not change the lambda_C_n constant payload"
        );
        assert_eq!(
            lab.store.get(lambda_c_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_n = lab
            ...
            .text()
            .to_string();
        assert!(mu_n.contains("constant  mu_n  node "), "{mu_n}");
        assert!(
            mu_n.contains(
                "hash     c9a6a49c3c793cee8a4e3f31b1245f16c05c8b90c6e5fb1752fff1f2337b5f2c"
            ),
            "{mu_n}"
        );
        assert!(mu_n.contains("kind     interval"), "{mu_n}");
        assert!(mu_n.contains("table    XXXI"), "{mu_n}");
        assert!(
            mu_n.contains("range    mu_n = -9.6623651(23)e-27"),
            "{mu_n}"
        );
        assert!(mu_n.contains("unit     J T^{-1}"), "{mu_n}");
        assert!(
            mu_n.contains(
                "value    [-48311837/5000000000000000000000000000000000, -24155907/2500000000000000000000000000000000]"
            ),
            "{mu_n}"
        );
        assert!(mu_n.contains("rebuild  ok"), "{mu_n}");
        assert!(mu_n.contains("not P3N"), "{mu_n}");
        assert!(!mu_n.contains("receipt"), "{mu_n}");
        assert!(!mu_n.contains("theorem"), "{mu_n}");
        let mu_n_id = constant_node_id(&mu_n);
        assert_eq!(
            mu_n_id.to_hex(),
            "f1a990d6d1f6c6abdae0834a0153935da5673e4979a29d6988e4ba563c90aa1d",
            "journaling must not change the mu_n constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_n_mub = lab
            .exec(Command::Constant {
                name: Some("mu_n_muB".into()),
            })
            .text()
            .to_string();
        assert!(mu_n_mub.contains("constant  mu_n_muB  node "), "{mu_n_mub}");
        assert!(
            mu_n_mub.contains(
                "hash     1e8fe1ba579ca229293ab7fd77a791d19df2f97296117a66bdf20339b4c0f45d"
            ),
            "{mu_n_mub}"
        );
        assert!(mu_n_mub.contains("kind     interval"), "{mu_n_mub}");
        assert!(mu_n_mub.contains("table    XXXI"), "{mu_n_mub}");
        assert!(
            mu_n_mub.contains("range    mu_n/muB = -1.04187563(25)e-3"),
            "{mu_n_mub}"
        );
        assert!(mu_n_mub.contains("unit     1"), "{mu_n_mub}");
        assert!(
            mu_n_mub.contains("value    [-26046897/25000000000, -52093769/50000000000]"),
            "{mu_n_mub}"
        );
        assert!(mu_n_mub.contains("rebuild  ok"), "{mu_n_mub}");
        assert!(mu_n_mub.contains("not P3N"), "{mu_n_mub}");
        assert!(!mu_n_mub.contains("receipt"), "{mu_n_mub}");
        assert!(!mu_n_mub.contains("theorem"), "{mu_n_mub}");
        let mu_n_mub_id = constant_node_id(&mu_n_mub);
        assert_eq!(
            mu_n_mub_id.to_hex(),
            "c4f9861a3802f769b5127e1c0e6308c19920c5baacb3eae95802b855984c9d1e",
            "journaling must not change the mu_n_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_mub_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_n_mun = lab
            .exec(Command::Constant {
                name: Some("mu_n_muN".into()),
            })
            .text()
            .to_string();
        assert!(mu_n_mun.contains("constant  mu_n_muN  node "), "{mu_n_mun}");
        assert!(
            mu_n_mun.contains(
                "hash     7f589089b63d3ad32bd59d8c16b542704a4dab7185cae50e8ed4179011ebb7f2"
            ),
            "{mu_n_mun}"
        );
        assert!(mu_n_mun.contains("kind     interval"), "{mu_n_mun}");
        assert!(mu_n_mun.contains("table    XXXI"), "{mu_n_mun}");
        assert!(
            mu_n_mun.contains("range    mu_n/muN = -1.91304273(45)"),
            "{mu_n_mun}"
        );
        assert!(mu_n_mun.contains("unit     1"), "{mu_n_mun}");
        assert!(
            mu_n_mun.contains("value    [-95652159/50000000, -47826057/25000000]"),
            "{mu_n_mun}"
        );
        assert!(mu_n_mun.contains("rebuild  ok"), "{mu_n_mun}");
        assert!(mu_n_mun.contains("not P3N"), "{mu_n_mun}");
        assert!(!mu_n_mun.contains("receipt"), "{mu_n_mun}");
        assert!(!mu_n_mun.contains("theorem"), "{mu_n_mun}");
        let mu_n_mun_id = constant_node_id(&mu_n_mun);
        assert_eq!(
            mu_n_mun_id.to_hex(),
            "32dc6a7e69b86f074f85c31946033fa8267e4d48df5b511a2bd371f813c383ed",
            "journaling must not change the mu_n_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_mun_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gn = lab
            ...
            .text()
            .to_string();
        assert!(gn.contains("constant  gn  node "), "{gn}");
        assert!(
            gn.contains(
                "hash     745d414efe1b217af8239787ffd1bb6d0d820fd8753d8aeaedac10c87658aac9"
            ),
            "{gn}"
        );
        assert!(gn.contains("kind     interval"), "{gn}");
        assert!(gn.contains("table    XXXI"), "{gn}");
        assert!(gn.contains("range    gn = -3.82608545(90)"), "{gn}");
        assert!(gn.contains("unit     1"), "{gn}");
        assert!(
            gn.contains("value    [-76521727/20000000, -76521691/20000000]"),
            "{gn}"
        );
        assert!(gn.contains("rebuild  ok"), "{gn}");
        assert!(gn.contains("not P3N"), "{gn}");
        assert!(!gn.contains("receipt"), "{gn}");
        assert!(!gn.contains("theorem"), "{gn}");
        let gn_id = constant_node_id(&gn);
        assert_eq!(
            gn_id.to_hex(),
            "5e266c7126aeed691a87f41410dd736298097a6a81760313b2d47517c7a3ebad",
            "journaling must not change the gn constant payload"
        );
        assert_eq!(
            lab.store.get(gn_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_n_mue = lab
            .exec(Command::Constant {
                name: Some("mu_n_mue".into()),
            })
            .text()
            .to_string();
        assert!(mu_n_mue.contains("constant  mu_n_mue  node "), "{mu_n_mue}");
        assert!(
            mu_n_mue.contains(
                "hash     6373ed28fe84cda2c7b3eb096b4db0662e477f295ff05247eafedc9aa61707bc"
            ),
            "{mu_n_mue}"
        );
        assert!(mu_n_mue.contains("kind     interval"), "{mu_n_mue}");
        assert!(mu_n_mue.contains("table    XXXI"), "{mu_n_mue}");
        assert!(
            mu_n_mue.contains("range    mu_n/mue = 1.04066882(25)e-3"),
            "{mu_n_mue}"
        );
        assert!(mu_n_mue.contains("unit     1"), "{mu_n_mue}");
        assert!(
            mu_n_mue.contains("value    [104066857/100000000000, 104066907/100000000000]"),
            "{mu_n_mue}"
        );
        assert!(mu_n_mue.contains("rebuild  ok"), "{mu_n_mue}");
        assert!(mu_n_mue.contains("not P3N"), "{mu_n_mue}");
        assert!(!mu_n_mue.contains("receipt"), "{mu_n_mue}");
        assert!(!mu_n_mue.contains("theorem"), "{mu_n_mue}");
        let mu_n_mue_id = constant_node_id(&mu_n_mue);
        assert_eq!(
            mu_n_mue_id.to_hex(),
            "2116e88a6a333c70afbbac409f03add69be63c505641359e0311ddb16f98bff0",
            "journaling must not change the mu_n_mue constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_mue_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_n_mup = lab
            .exec(Command::Constant {
                name: Some("mu_n_mup".into()),
            })
            .text()
            .to_string();
        assert!(mu_n_mup.contains("constant  mu_n_mup  node "), "{mu_n_mup}");
        assert!(
            mu_n_mup.contains(
                "hash     75b137c205d9297127a1955dc686acc1c0206fb92d7a7e4751a63addbef18942"
            ),
            "{mu_n_mup}"
        );
        assert!(mu_n_mup.contains("kind     interval"), "{mu_n_mup}");
        assert!(mu_n_mup.contains("table    XXXI"), "{mu_n_mup}");
        assert!(
            mu_n_mup.contains("range    mu_n/mup = -0.68497934(16)"),
            "{mu_n_mup}"
        );
        assert!(mu_n_mup.contains("unit     1"), "{mu_n_mup}");
        assert!(
            mu_n_mup.contains("value    [-1369959/2000000, -34248959/50000000]"),
            "{mu_n_mup}"
        );
        assert!(mu_n_mup.contains("rebuild  ok"), "{mu_n_mup}");
        assert!(mu_n_mup.contains("not P3N"), "{mu_n_mup}");
        assert!(!mu_n_mup.contains("receipt"), "{mu_n_mup}");
        assert!(!mu_n_mup.contains("theorem"), "{mu_n_mup}");
        let mu_n_mup_id = constant_node_id(&mu_n_mup);
        assert_eq!(
            mu_n_mup_id.to_hex(),
            "677f9e0d701a23bb4a86c03666f3e9af0850affa168ef263f4170843cb91f2ad",
            "journaling must not change the mu_n_mup constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_mup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_n_mu0p = lab
            .exec(Command::Constant {
                name: Some("mu_n_mu0p".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_n_mu0p.contains("constant  mu_n_mu0p  node "),
            "{mu_n_mu0p}"
        );
        assert!(
            mu_n_mu0p.contains(
                "hash     c691ce436f1d907625b45c49330e4f5481fab606584c677a69291b1897580550"
            ),
            "{mu_n_mu0p}"
        );
        assert!(mu_n_mu0p.contains("kind     interval"), "{mu_n_mu0p}");
        assert!(mu_n_mu0p.contains("table    XXXI"), "{mu_n_mu0p}");
        assert!(
            mu_n_mu0p.contains("range    mu_n/mu0p = -0.68499694(16)"),
            "{mu_n_mu0p}"
        );
        assert!(mu_n_mu0p.contains("unit     1"), "{mu_n_mu0p}");
        assert!(
            mu_n_mu0p.contains("value    [-6849971/10000000, -34249839/50000000]"),
            "{mu_n_mu0p}"
        );
        assert!(mu_n_mu0p.contains("rebuild  ok"), "{mu_n_mu0p}");
        assert!(mu_n_mu0p.contains("not P3N"), "{mu_n_mu0p}");
        assert!(!mu_n_mu0p.contains("receipt"), "{mu_n_mu0p}");
        assert!(!mu_n_mu0p.contains("theorem"), "{mu_n_mu0p}");
        let mu_n_mu0p_id = constant_node_id(&mu_n_mu0p);
        assert_eq!(
            mu_n_mu0p_id.to_hex(),
            "eb647725813c11763a6c554d40c2e6cb7c085fe4383c225ebe809024b9904620",
            "journaling must not change the mu_n_mu0p constant payload"
        );
        assert_eq!(
            lab.store.get(mu_n_mu0p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_d = lab
            ...
            .text()
            .to_string();
        assert!(m_d.contains("constant  m_d  node "), "{m_d}");
        assert!(
            m_d.contains(
                "hash     0710831944a3d44d75fd1e63f10fd9f06edc9b0d93028dd57749f807b1a37432"
            ),
            "{m_d}"
        );
        assert!(m_d.contains("kind     interval"), "{m_d}");
        assert!(m_d.contains("table    XXXI"), "{m_d}");
        assert!(m_d.contains("range    md = 3.3435837724(10)e-27"), "{m_d}");
        assert!(m_d.contains("unit     kg"), "{m_d}");
        assert!(
            m_d.contains(
                "value    [16717918857/5000000000000000000000000000000000000, 16717918867/5000000000000000000000000000000000000]"
            ),
            "{m_d}"
        );
        assert!(m_d.contains("rebuild  ok"), "{m_d}");
        assert!(m_d.contains("not P3N"), "{m_d}");
        assert!(!m_d.contains("receipt"), "{m_d}");
        assert!(!m_d.contains("theorem"), "{m_d}");
        let m_d_id = constant_node_id(&m_d);
        assert_eq!(
            m_d_id.to_hex(),
            "f4182bcd2231bc769f766746198bf60aa496cf0ae1b9ca7726cfc78c775eeaab",
            "journaling must not change the m_d constant payload"
        );
        assert_eq!(
            lab.store.get(m_d_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_d_u = lab
            .exec(Command::Constant {
                name: Some("m_d_u".into()),
            })
            .text()
            .to_string();
        assert!(m_d_u.contains("constant  m_d_u  node "), "{m_d_u}");
        assert!(
            m_d_u.contains(
                "hash     7a2afd4043689b99d9f043af14347050cf8d4f6b774886642c256c6ab0f2abbe"
            ),
            "{m_d_u}"
        );
        assert!(m_d_u.contains("kind     interval"), "{m_d_u}");
        assert!(m_d_u.contains("table    XXXI"), "{m_d_u}");
        assert!(
            m_d_u.contains("range    md_u = 2.013553212745(40)"),
            "{m_d_u}"
        );
        assert!(m_d_u.contains("unit     u"), "{m_d_u}");
        assert!(
            m_d_u.contains("value    [402710642541/200000000000, 402710642557/200000000000]"),
            "{m_d_u}"
        );
        assert!(m_d_u.contains("rebuild  ok"), "{m_d_u}");
        assert!(m_d_u.contains("not P3N"), "{m_d_u}");
        assert!(!m_d_u.contains("receipt"), "{m_d_u}");
        assert!(!m_d_u.contains("theorem"), "{m_d_u}");
        let m_d_u_id = constant_node_id(&m_d_u);
        assert_eq!(
            m_d_u_id.to_hex(),
            "d5f940659bb086ee0fe58faea4e01c168cf873d12a762de08d154892ce348373",
            "journaling must not change the m_d_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_d_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_d_c2 = lab
            .exec(Command::Constant {
                name: Some("m_d_c2".into()),
            })
            .text()
            .to_string();
        assert!(m_d_c2.contains("constant  m_d_c2  node "), "{m_d_c2}");
        assert!(
            m_d_c2.contains(
                "hash     dfe04d95b7f5a00ad95da03903d7218c1ba7e38d88d89a3e1644018a94e65868"
            ),
            "{m_d_c2}"
        );
        assert!(m_d_c2.contains("kind     interval"), "{m_d_c2}");
        assert!(m_d_c2.contains("table    XXXI"), "{m_d_c2}");
        assert!(
            m_d_c2.contains("range    mdc2 = 3.00506323102(91)e-10"),
            "{m_d_c2}"
        );
        assert!(m_d_c2.contains("unit     J"), "{m_d_c2}");
        assert!(
            m_d_c2.contains(
                "value    [300506323011/1000000000000000000000, 300506323193/1000000000000000000000]"
            ),
            "{m_d_c2}"
        );
        assert!(m_d_c2.contains("rebuild  ok"), "{m_d_c2}");
        assert!(m_d_c2.contains("not P3N"), "{m_d_c2}");
        assert!(!m_d_c2.contains("receipt"), "{m_d_c2}");
        assert!(!m_d_c2.contains("theorem"), "{m_d_c2}");
        let m_d_c2_id = constant_node_id(&m_d_c2);
        assert_eq!(
            m_d_c2_id.to_hex(),
            "98f021436a9b05709710fb44e4d4302f2559a000c80632c589d0ce35a60a310f",
            "journaling must not change the m_d_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_d_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_d_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_d_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_d_c2_mev.contains("constant  m_d_c2_MeV  node "),
            "{m_d_c2_mev}"
        );
        assert!(
            m_d_c2_mev.contains(
                "hash     798ca780f4396ff95cc3582e796b9830baa21f69cf653160827d0d0f80502a0d"
            ),
            "{m_d_c2_mev}"
        );
        assert!(m_d_c2_mev.contains("kind     interval"), "{m_d_c2_mev}");
        assert!(m_d_c2_mev.contains("table    XXXI"), "{m_d_c2_mev}");
        assert!(
            m_d_c2_mev.contains("range    mdc2_MeV = 1875.61294257(57)"),
            "{m_d_c2_mev}"
        );
        assert!(m_d_c2_mev.contains("unit     MeV"), "{m_d_c2_mev}");
        assert!(
            m_d_c2_mev.contains("value    [937806471/500000, 93780647157/50000000]"),
            "{m_d_c2_mev}"
        );
        assert!(m_d_c2_mev.contains("rebuild  ok"), "{m_d_c2_mev}");
        assert!(m_d_c2_mev.contains("not P3N"), "{m_d_c2_mev}");
        assert!(!m_d_c2_mev.contains("receipt"), "{m_d_c2_mev}");
        assert!(!m_d_c2_mev.contains("theorem"), "{m_d_c2_mev}");
        let m_d_c2_mev_id = constant_node_id(&m_d_c2_mev);
        assert_eq!(
            m_d_c2_mev_id.to_hex(),
            "1fd451a0f9e39fa6f32963caf5375259d716af30de6dc462c39eadfac63a644a",
            "journaling must not change the m_d_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_d_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let md_me = lab
            .exec(Command::Constant {
                name: Some("md_me".into()),
            })
            .text()
            .to_string();
        assert!(md_me.contains("constant  md_me  node "), "{md_me}");
        assert!(
            md_me.contains(
                "hash     9c0f207cbd339c3665d3cc96b38c98fddbf31894f385c84fa5197f61a212236d"
            ),
            "{md_me}"
        );
        assert!(md_me.contains("kind     interval"), "{md_me}");
        assert!(md_me.contains("table    XXXI"), "{md_me}");
        assert!(
            md_me.contains("range    md/me = 3670.48296788(13)"),
            "{md_me}"
        );
        assert!(md_me.contains("unit     1"), "{md_me}");
        assert!(
            md_me.contains("value    [14681931871/4000000, 367048296801/100000000]"),
            "{md_me}"
        );
        assert!(md_me.contains("rebuild  ok"), "{md_me}");
        assert!(md_me.contains("not P3N"), "{md_me}");
        assert!(!md_me.contains("receipt"), "{md_me}");
        assert!(!md_me.contains("theorem"), "{md_me}");
        let md_me_id = constant_node_id(&md_me);
        assert_eq!(
            md_me_id.to_hex(),
            "861d04500921d545595f7454950d748d589778b8e15b0633af2e6b2b89af6641",
            "journaling must not change the md_me constant payload"
        );
        assert_eq!(
            lab.store.get(md_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let md_mp = lab
            .exec(Command::Constant {
                name: Some("md_mp".into()),
            })
            .text()
            .to_string();
        assert!(md_mp.contains("constant  md_mp  node "), "{md_mp}");
        assert!(
            md_mp.contains(
                "hash     a1c84e01de3c4fb4e5eb0c9de98edd0d13aefc51c32e1c95c2d4203d6144a919"
            ),
            "{md_mp}"
        );
        assert!(md_mp.contains("kind     interval"), "{md_mp}");
        assert!(md_mp.contains("table    XXXI"), "{md_mp}");
        assert!(
            md_mp.contains("range    md/mp = 1.99900750139(11)"),
            "{md_mp}"
        );
        assert!(md_mp.contains("unit     1"), "{md_mp}");
        assert!(
            md_mp.contains("value    [12493796883/6250000000, 3998015003/2000000000]"),
            "{md_mp}"
        );
        assert!(md_mp.contains("rebuild  ok"), "{md_mp}");
        assert!(md_mp.contains("not P3N"), "{md_mp}");
        assert!(!md_mp.contains("receipt"), "{md_mp}");
        assert!(!md_mp.contains("theorem"), "{md_mp}");
        let md_mp_id = constant_node_id(&md_mp);
        assert_eq!(
            md_mp_id.to_hex(),
            "76438ea152e9bce3c9733ba0635291b65794e143615119234ed8777508b0f61d",
            "journaling must not change the md_mp constant payload"
        );
        assert_eq!(
            lab.store.get(md_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_d_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_d_molar.contains("constant  M_d  node "), "{m_d_molar}");
        assert!(
            m_d_molar.contains(
                "hash     cd2742c648825c389159209b4b9ab8105b81bbb4696f9cbd103883f371d3b50a"
            ),
            "{m_d_molar}"
        );
        assert!(m_d_molar.contains("kind     interval"), "{m_d_molar}");
        assert!(m_d_molar.contains("table    XXXI"), "{m_d_molar}");
        assert!(
            m_d_molar.contains("range    Md = 2.01355321205(61)e-3"),
            "{m_d_molar}"
        );
        assert!(m_d_molar.contains("unit     kg mol^{-1}"), "{m_d_molar}");
        assert!(
            m_d_molar
                .contains("value    [25169415143/12500000000000, 100677660633/50000000000000]"),
            "{m_d_molar}"
        );
        assert!(m_d_molar.contains("rebuild  ok"), "{m_d_molar}");
        assert!(m_d_molar.contains("not P3N"), "{m_d_molar}");
        assert!(!m_d_molar.contains("receipt"), "{m_d_molar}");
        assert!(!m_d_molar.contains("theorem"), "{m_d_molar}");
        let m_d_molar_id = constant_node_id(&m_d_molar);
        assert_eq!(
            m_d_molar_id.to_hex(),
            "29960ff017c63127f01463751240e48b884a25734f16f442f05e4b42806d3a70",
            "journaling must not change the M_d constant payload"
        );
        assert_eq!(
            lab.store.get(m_d_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let rd = lab
            ...
            .text()
            .to_string();
        assert!(rd.contains("constant  rd  node "), "{rd}");
        assert!(
            rd.contains(
                "hash     972906db24ea49fb88e62cf390508d5970373c0b9252f4f0ef50c68818c8ecdf"
            ),
            "{rd}"
        );
        assert!(rd.contains("kind     interval"), "{rd}");
        assert!(rd.contains("table    XXXI"), "{rd}");
        assert!(rd.contains("range    rd = 2.12799(74)e-15"), "{rd}");
        assert!(rd.contains("unit     m"), "{rd}");
        assert!(
            rd.contains("value    [8509/4000000000000000000, 212873/100000000000000000000]"),
            "{rd}"
        );
        assert!(rd.contains("rebuild  ok"), "{rd}");
        assert!(rd.contains("not P3N"), "{rd}");
        assert!(!rd.contains("receipt"), "{rd}");
        assert!(!rd.contains("theorem"), "{rd}");
        let rd_id = constant_node_id(&rd);
        assert_eq!(
            rd_id.to_hex(),
            "0203790131852cb3c7f69d05f48d29347c4955cfbbd096d3906aab205127ede5",
            "journaling must not change the rd constant payload"
        );
        assert_eq!(
            lab.store.get(rd_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_d = lab
            ...
            .text()
            .to_string();
        assert!(mu_d.contains("constant  mu_d  node "), "{mu_d}");
        assert!(
            mu_d.contains(
                "hash     c13c3bf94350f4bfc43806e523fa562c2309d7b6cf3bd2a0d0cca1017e5d3cbf"
            ),
            "{mu_d}"
        );
        assert!(mu_d.contains("kind     interval"), "{mu_d}");
        assert!(mu_d.contains("table    XXXI"), "{mu_d}");
        assert!(
            mu_d.contains("range    mu_d = 4.330735094(11)e-27"),
            "{mu_d}"
        );
        assert!(mu_d.contains("unit     J T^{-1}"), "{mu_d}");
        assert!(
            mu_d.contains(
                "value    [4330735083/1000000000000000000000000000000000000, 866147021/200000000000000000000000000000000000]"
            ),
            "{mu_d}"
        );
        assert!(mu_d.contains("rebuild  ok"), "{mu_d}");
        assert!(mu_d.contains("not P3N"), "{mu_d}");
        assert!(!mu_d.contains("receipt"), "{mu_d}");
        assert!(!mu_d.contains("theorem"), "{mu_d}");
        let mu_d_id = constant_node_id(&mu_d);
        assert_eq!(
            mu_d_id.to_hex(),
            "27fa5e1fad0d9768c160ff694e634905908467dc52156c39fb642d7406045835",
            "journaling must not change the mu_d constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_d_mub = lab
            .exec(Command::Constant {
                name: Some("mu_d_muB".into()),
            })
            .text()
            .to_string();
        assert!(mu_d_mub.contains("constant  mu_d_muB  node "), "{mu_d_mub}");
        assert!(
            mu_d_mub.contains(
                "hash     309b4366be950e8638826c987a8f2bfde2d5773bba5896727fb2f4ae859208e1"
            ),
            "{mu_d_mub}"
        );
        assert!(mu_d_mub.contains("kind     interval"), "{mu_d_mub}");
        assert!(mu_d_mub.contains("table    XXXI"), "{mu_d_mub}");
        assert!(
            mu_d_mub.contains("range    mu_d/muB = 4.669754570(12)e-4"),
            "{mu_d_mub}"
        );
        assert!(mu_d_mub.contains("unit     1"), "{mu_d_mub}");
        assert!(
            mu_d_mub.contains("value    [2334877279/5000000000000, 2334877291/5000000000000]"),
            "{mu_d_mub}"
        );
        assert!(mu_d_mub.contains("rebuild  ok"), "{mu_d_mub}");
        assert!(mu_d_mub.contains("not P3N"), "{mu_d_mub}");
        assert!(!mu_d_mub.contains("receipt"), "{mu_d_mub}");
        assert!(!mu_d_mub.contains("theorem"), "{mu_d_mub}");
        let mu_d_mub_id = constant_node_id(&mu_d_mub);
        assert_eq!(
            mu_d_mub_id.to_hex(),
            "1cb11f5a26cf543d181459b2c769e9a610d5573825e81d6bfcebd41085521ae0",
            "journaling must not change the mu_d_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_mub_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_d_to_mu_n = lab
            .exec(Command::Constant {
                name: Some("mu_d_muN".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_d_to_mu_n.contains("constant  mu_d_muN  node "),
            "{mu_d_to_mu_n}"
        );
        assert!(
            mu_d_to_mu_n.contains(
                "hash     823bcbc76a3eb38e75a8c40931b5eff4b84ad60d667b3bf87e09ed5700ddcf96"
            ),
            "{mu_d_to_mu_n}"
        );
        assert!(mu_d_to_mu_n.contains("kind     interval"), "{mu_d_to_mu_n}");
        assert!(mu_d_to_mu_n.contains("table    XXXI"), "{mu_d_to_mu_n}");
        assert!(
            mu_d_to_mu_n.contains("range    mu_d/muN = 0.8574382338(22)"),
            "{mu_d_to_mu_n}"
        );
        assert!(mu_d_to_mu_n.contains("unit     1"), "{mu_d_to_mu_n}");
        assert!(
            mu_d_to_mu_n.contains("value    [2143595579/2500000000, 214359559/250000000]"),
            "{mu_d_to_mu_n}"
        );
        assert!(mu_d_to_mu_n.contains("rebuild  ok"), "{mu_d_to_mu_n}");
        assert!(mu_d_to_mu_n.contains("not P3N"), "{mu_d_to_mu_n}");
        assert!(!mu_d_to_mu_n.contains("receipt"), "{mu_d_to_mu_n}");
        assert!(!mu_d_to_mu_n.contains("theorem"), "{mu_d_to_mu_n}");
        let mu_d_to_mu_n_id = constant_node_id(&mu_d_to_mu_n);
        assert_eq!(
            mu_d_to_mu_n_id.to_hex(),
            "20dd35057706c81f9f847cfe546030855559e0d520dee37b415d14357eab1d0e",
            "journaling must not change the mu_d_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_to_mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gd = lab
            ...
            .text()
            .to_string();
        assert!(gd.contains("constant  gd  node "), "{gd}");
        assert!(
            gd.contains(
                "hash     2abb89db8e1e22310c2a7f61a8afee935dfe5e73a9034a17537dd58927215d84"
            ),
            "{gd}"
        );
        assert!(gd.contains("kind     interval"), "{gd}");
        assert!(gd.contains("table    XXXI"), "{gd}");
        assert!(gd.contains("range    gd = 0.8574382338(22)"), "{gd}");
        assert!(gd.contains("unit     1"), "{gd}");
        assert!(
            gd.contains("value    [2143595579/2500000000, 214359559/250000000]"),
            "{gd}"
        );
        assert!(gd.contains("rebuild  ok"), "{gd}");
        assert!(gd.contains("not P3N"), "{gd}");
        assert!(!gd.contains("receipt"), "{gd}");
        assert!(!gd.contains("theorem"), "{gd}");
        let gd_id = constant_node_id(&gd);
        assert_eq!(
            gd_id.to_hex(),
            "db4f985bb0b58a1e8c23a9d093544983d9b1a9b7d7e29f04b25bb16be0c75f97",
            "journaling must not change the gd constant payload"
        );
        assert_eq!(
            lab.store.get(gd_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_d_mue = lab
            .exec(Command::Constant {
                name: Some("mu_d_mue".into()),
            })
            .text()
            .to_string();
        assert!(mu_d_mue.contains("constant  mu_d_mue  node "), "{mu_d_mue}");
        assert!(
            mu_d_mue.contains(
                "hash     c14a0b1e4aa0447ff0fe66724f0fe3058a2feec3dc134d41351dd57726d1bdb4"
            ),
            "{mu_d_mue}"
        );
        assert!(mu_d_mue.contains("kind     interval"), "{mu_d_mue}");
        assert!(mu_d_mue.contains("table    XXXI"), "{mu_d_mue}");
        assert!(
            mu_d_mue.contains("range    mu_d/mue = -4.664345551(12)e-4"),
            "{mu_d_mue}"
        );
        assert!(mu_d_mue.contains("unit     1"), "{mu_d_mue}");
        assert!(
            mu_d_mue.contains("value    [-4664345563/10000000000000, -4664345539/10000000000000]"),
            "{mu_d_mue}"
        );
        assert!(mu_d_mue.contains("rebuild  ok"), "{mu_d_mue}");
        assert!(mu_d_mue.contains("not P3N"), "{mu_d_mue}");
        assert!(!mu_d_mue.contains("receipt"), "{mu_d_mue}");
        assert!(!mu_d_mue.contains("theorem"), "{mu_d_mue}");
        let mu_d_mue_id = constant_node_id(&mu_d_mue);
        assert_eq!(
            mu_d_mue_id.to_hex(),
            "b7e3f8d483fffdd0e0214b0066e04342242e5d6b9e15c6833c7946aaf8153ca8",
            "journaling must not change the mu_d_mue constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_mue_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_d_mup = lab
            .exec(Command::Constant {
                name: Some("mu_d_mup".into()),
            })
            .text()
            .to_string();
        assert!(mu_d_mup.contains("constant  mu_d_mup  node "), "{mu_d_mup}");
        assert!(
            mu_d_mup.contains(
                "hash     714492efa58a8f73eec0856f5a3587db0ba4eeddaf47fef8867ce1f1e5e52120"
            ),
            "{mu_d_mup}"
        );
        assert!(mu_d_mup.contains("kind     interval"), "{mu_d_mup}");
        assert!(mu_d_mup.contains("table    XXXI"), "{mu_d_mup}");
        assert!(
            mu_d_mup.contains("range    mu_d/mup = 0.30701220939(79)"),
            "{mu_d_mup}"
        );
        assert!(mu_d_mup.contains("unit     1"), "{mu_d_mup}");
        assert!(
            mu_d_mup.contains("value    [1535061043/5000000000, 15350610509/50000000000]"),
            "{mu_d_mup}"
        );
        assert!(mu_d_mup.contains("rebuild  ok"), "{mu_d_mup}");
        assert!(mu_d_mup.contains("not P3N"), "{mu_d_mup}");
        assert!(!mu_d_mup.contains("receipt"), "{mu_d_mup}");
        assert!(!mu_d_mup.contains("theorem"), "{mu_d_mup}");
        let mu_d_mup_id = constant_node_id(&mu_d_mup);
        assert_eq!(
            mu_d_mup_id.to_hex(),
            "5d500b4b42060fbd1225eb945e6d95322456a99148415d18d11e04039c43be7a",
            "journaling must not change the mu_d_mup constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_mup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_d_mun = lab
            ...
            .text()
            .to_string();
        assert!(mu_d_mun.contains("constant  mu_d_mun  node "), "{mu_d_mun}");
        assert!(
            mu_d_mun.contains(
                "hash     4bd463c23a25971e81f82ac0351265971ebedc2e0a3f37ca76935aaf486b4da3"
            ),
            "{mu_d_mun}"
        );
        assert!(mu_d_mun.contains("kind     interval"), "{mu_d_mun}");
        assert!(mu_d_mun.contains("table    XXXI"), "{mu_d_mun}");
        assert!(
            mu_d_mun.contains("range    mu_d/mun = -0.44820653(11)"),
            "{mu_d_mun}"
        );
        assert!(mu_d_mun.contains("unit     1"), "{mu_d_mun}");
        assert!(
            mu_d_mun.contains("value    [-5602583/12500000, -22410321/50000000]"),
            "{mu_d_mun}"
        );
        assert!(mu_d_mun.contains("rebuild  ok"), "{mu_d_mun}");
        assert!(mu_d_mun.contains("not P3N"), "{mu_d_mun}");
        assert!(!mu_d_mun.contains("receipt"), "{mu_d_mun}");
        assert!(!mu_d_mun.contains("theorem"), "{mu_d_mun}");
        let mu_d_mun_id = constant_node_id(&mu_d_mun);
        assert_eq!(
            mu_d_mun_id.to_hex(),
            "1b59d33d5a68a24febbefa7ab88b5800bee2a0d0994ec769f9d147f6d1f6a184",
            "journaling must not change the mu_d_mun constant payload"
        );
        assert_eq!(
            lab.store.get(mu_d_mun_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_t = lab
            ...
            .text()
            .to_string();
        assert!(m_t.contains("constant  m_t  node "), "{m_t}");
        assert!(
            m_t.contains(
                "hash     8f7874deeb241abcd6ab910a824ee8badc1c135ca735604a9e4116b86d6255bc"
            ),
            "{m_t}"
        );
        assert!(m_t.contains("kind     interval"), "{m_t}");
        assert!(m_t.contains("table    XXXI"), "{m_t}");
        assert!(m_t.contains("range    mt = 5.0073567446(15)e-27"), "{m_t}");
        assert!(m_t.contains("unit     kg"), "{m_t}");
        assert!(
            m_t.contains(
                "value    [50073567431/10000000000000000000000000000000000000, 50073567461/10000000000000000000000000000000000000]"
            ),
            "{m_t}"
        );
        assert!(m_t.contains("rebuild  ok"), "{m_t}");
        assert!(m_t.contains("not P3N"), "{m_t}");
        assert!(!m_t.contains("receipt"), "{m_t}");
        assert!(!m_t.contains("theorem"), "{m_t}");
        let m_t_id = constant_node_id(&m_t);
        assert_eq!(
            m_t_id.to_hex(),
            "1f3c547a59756969902848bb496c7d19fddbdb577efca55e9e22074dd3f9ccf7",
            "journaling must not change the m_t constant payload"
        );
        assert_eq!(
            lab.store.get(m_t_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_t_u = lab
            .exec(Command::Constant {
                name: Some("m_t_u".into()),
            })
            .text()
            .to_string();
        assert!(m_t_u.contains("constant  m_t_u  node "), "{m_t_u}");
        assert!(
            m_t_u.contains(
                "hash     60799f1d95e3ac37f9505743bac8b6a7437b1707963516078da90c73d276e43a"
            ),
            "{m_t_u}"
        );
        assert!(m_t_u.contains("kind     interval"), "{m_t_u}");
        assert!(m_t_u.contains("table    XXXI"), "{m_t_u}");
        assert!(
            m_t_u.contains("range    mt_u = 3.01550071621(12)"),
            "{m_t_u}"
        );
        assert!(m_t_u.contains("unit     u"), "{m_t_u}");
        assert!(
            m_t_u.contains("value    [301550071609/100000000000, 301550071633/100000000000]"),
            "{m_t_u}"
        );
        assert!(m_t_u.contains("rebuild  ok"), "{m_t_u}");
        assert!(m_t_u.contains("not P3N"), "{m_t_u}");
        assert!(!m_t_u.contains("receipt"), "{m_t_u}");
        assert!(!m_t_u.contains("theorem"), "{m_t_u}");
        let m_t_u_id = constant_node_id(&m_t_u);
        assert_eq!(
            m_t_u_id.to_hex(),
            "40b6aff75bbd01dd42f219fff535af9cd774cf38082fa837bcec84ede61ae9a4",
            "journaling must not change the m_t_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_t_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_t_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_t_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_t_c2_mev.contains("constant  m_t_c2_MeV  node "),
            "{m_t_c2_mev}"
        );
        assert!(
            m_t_c2_mev.contains(
                "hash     818597e9129aeea9aa601ed37421b0cdba5f257cddc2437fd22d635d8a205136"
            ),
            "{m_t_c2_mev}"
        );
        assert!(m_t_c2_mev.contains("kind     interval"), "{m_t_c2_mev}");
        assert!(m_t_c2_mev.contains("table    XXXI"), "{m_t_c2_mev}");
        assert!(
            m_t_c2_mev.contains("range    mtc2_MeV = 2808.92113298(85)"),
            "{m_t_c2_mev}"
        );
        assert!(m_t_c2_mev.contains("unit     MeV"), "{m_t_c2_mev}");
        assert!(
            m_t_c2_mev.contains("value    [280892113213/100000000, 280892113383/100000000]"),
            "{m_t_c2_mev}"
        );
        assert!(m_t_c2_mev.contains("rebuild  ok"), "{m_t_c2_mev}");
        assert!(m_t_c2_mev.contains("not P3N"), "{m_t_c2_mev}");
        assert!(!m_t_c2_mev.contains("receipt"), "{m_t_c2_mev}");
        assert!(!m_t_c2_mev.contains("theorem"), "{m_t_c2_mev}");
        let m_t_c2_mev_id = constant_node_id(&m_t_c2_mev);
        assert_eq!(
            m_t_c2_mev_id.to_hex(),
            "1332df022f8b2d564b2ec9f8760debb91a5260fe0c1691c4919ccffc9c410c99",
            "journaling must not change the m_t_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_t_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mt_me = lab
            .exec(Command::Constant {
                name: Some("mt_me".into()),
            })
            .text()
            .to_string();
        assert!(mt_me.contains("constant  mt_me  node "), "{mt_me}");
        assert!(
            mt_me.contains(
                "hash     93547acb180d081377858fe38ebb143d9f8c1309a209cb635157b8d2627aed58"
            ),
            "{mt_me}"
        );
        assert!(mt_me.contains("kind     interval"), "{mt_me}");
        assert!(mt_me.contains("table    XXXI"), "{mt_me}");
        assert!(
            mt_me.contains("range    mt/me = 5496.92153573(27)"),
            "{mt_me}"
        );
        assert!(mt_me.contains("unit     1"), "{mt_me}");
        assert!(
            mt_me.contains("value    [274846076773/50000000, 85889399/15625]"),
            "{mt_me}"
        );
        assert!(mt_me.contains("rebuild  ok"), "{mt_me}");
        assert!(mt_me.contains("not P3N"), "{mt_me}");
        assert!(!mt_me.contains("receipt"), "{mt_me}");
        assert!(!mt_me.contains("theorem"), "{mt_me}");
        let mt_me_id = constant_node_id(&mt_me);
        assert_eq!(
            mt_me_id.to_hex(),
            "e204ed8ecdc5afb1b11914f6ba7980f0931aaabbf7ff74d0b8d2fca75025bd3a",
            "journaling must not change the mt_me constant payload"
        );
        assert_eq!(
            lab.store.get(mt_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mt_mp = lab
            .exec(Command::Constant {
                name: Some("mt_mp".into()),
            })
            .text()
            .to_string();
        assert!(mt_mp.contains("constant  mt_mp  node "), "{mt_mp}");
        assert!(
            mt_mp.contains(
                "hash     de2806f05b0502127a1c6e32a40eb5fde57e2dbbe06c02fbbd78e92899519595"
            ),
            "{mt_mp}"
        );
        assert!(mt_mp.contains("kind     interval"), "{mt_mp}");
        assert!(mt_mp.contains("table    XXXI"), "{mt_mp}");
        assert!(
            mt_mp.contains("range    mt/mp = 2.99371703414(15)"),
            "{mt_mp}"
        );
        assert!(mt_mp.contains("unit     1"), "{mt_mp}");
        assert!(
            mt_mp.contains("value    [299371703399/100000000000, 299371703429/100000000000]"),
            "{mt_mp}"
        );
        assert!(mt_mp.contains("rebuild  ok"), "{mt_mp}");
        assert!(mt_mp.contains("not P3N"), "{mt_mp}");
        assert!(!mt_mp.contains("receipt"), "{mt_mp}");
        assert!(!mt_mp.contains("theorem"), "{mt_mp}");
        let mt_mp_id = constant_node_id(&mt_mp);
        assert_eq!(
            mt_mp_id.to_hex(),
            "41ffc89be1c1822959ce00cee4dc8def69aa243b60dc61a9191747d600a7548f",
            "journaling must not change the mt_mp constant payload"
        );
        assert_eq!(
            lab.store.get(mt_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_t_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_t_molar.contains("constant  M_t  node "), "{m_t_molar}");
        assert!(
            m_t_molar.contains(
                "hash     c6c24c87f2920c72a840157ddeaa978adb4013e40c08e62168d27c940c0ff25b"
            ),
            "{m_t_molar}"
        );
        assert!(m_t_molar.contains("kind     interval"), "{m_t_molar}");
        assert!(m_t_molar.contains("table    XXXI"), "{m_t_molar}");
        assert!(
            m_t_molar.contains("range    Mt = 3.01550071517(92)e-3"),
            "{m_t_molar}"
        );
        assert!(m_t_molar.contains("unit     kg mol^{-1}"), "{m_t_molar}");
        assert!(
            m_t_molar
                .contains("value    [12062002857/4000000000000, 301550071609/100000000000000]"),
            "{m_t_molar}"
        );
        assert!(m_t_molar.contains("rebuild  ok"), "{m_t_molar}");
        assert!(m_t_molar.contains("not P3N"), "{m_t_molar}");
        assert!(!m_t_molar.contains("receipt"), "{m_t_molar}");
        assert!(!m_t_molar.contains("theorem"), "{m_t_molar}");
        let m_t_molar_id = constant_node_id(&m_t_molar);
        assert_eq!(
            m_t_molar_id.to_hex(),
            "98820fbeb58e46a4d5e31bc12f19bec979553c95500ffd5303811a5c7dd1edb0",
            "journaling must not change the M_t constant payload"
        );
        assert_eq!(
            lab.store.get(m_t_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_t = lab
            ...
            .text()
            .to_string();
        assert!(mu_t.contains("constant  mu_t  node "), "{mu_t}");
        assert!(
            mu_t.contains(
                "hash     f07f9b5d76ca552fd5126f5d9d3b2c21a68b0aec0da98c40394f95290766aa62"
            ),
            "{mu_t}"
        );
        assert!(mu_t.contains("kind     interval"), "{mu_t}");
        assert!(mu_t.contains("table    XXXI"), "{mu_t}");
        assert!(
            mu_t.contains("range    mu_t = 1.5046095202(30)e-26"),
            "{mu_t}"
        );
        assert!(mu_t.contains("unit     J T^{-1}"), "{mu_t}");
        assert!(
            mu_t.contains(
                "value    [3761523793/250000000000000000000000000000000000, 117547619/7812500000000000000000000000000000]"
            ),
            "{mu_t}"
        );
        assert!(mu_t.contains("rebuild  ok"), "{mu_t}");
        assert!(mu_t.contains("not P3N"), "{mu_t}");
        assert!(!mu_t.contains("receipt"), "{mu_t}");
        assert!(!mu_t.contains("theorem"), "{mu_t}");
        let mu_t_id = constant_node_id(&mu_t);
        assert_eq!(
            mu_t_id.to_hex(),
            "cf24b32cc388f7e447b4fd094f607dcb10e8982bd2251a0538b5a65d9fdf039c",
            "journaling must not change the mu_t constant payload"
        );
        assert_eq!(
            lab.store.get(mu_t_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_t_mub = lab
            .exec(Command::Constant {
                name: Some("mu_t_muB".into()),
            })
            .text()
            .to_string();
        assert!(mu_t_mub.contains("constant  mu_t_muB  node "), "{mu_t_mub}");
        assert!(
            mu_t_mub.contains(
                "hash     17fb6a9ef0fb6278581531a4a291899413f104dd27292cf4710f1903d1bdaefb"
            ),
            "{mu_t_mub}"
        );
        assert!(mu_t_mub.contains("kind     interval"), "{mu_t_mub}");
        assert!(mu_t_mub.contains("table    XXXI"), "{mu_t_mub}");
        assert!(
            mu_t_mub.contains("range    mu_t/muB = 1.6223936651(32)e-3"),
            "{mu_t_mub}"
        );
        assert!(mu_t_mub.contains("unit     1"), "{mu_t_mub}");
        assert!(
            mu_t_mub.contains("value    [16223936619/10000000000000, 16223936683/10000000000000]"),
            "{mu_t_mub}"
        );
        assert!(mu_t_mub.contains("rebuild  ok"), "{mu_t_mub}");
        assert!(mu_t_mub.contains("not P3N"), "{mu_t_mub}");
        assert!(!mu_t_mub.contains("receipt"), "{mu_t_mub}");
        assert!(!mu_t_mub.contains("theorem"), "{mu_t_mub}");
        let mu_t_mub_id = constant_node_id(&mu_t_mub);
        assert_eq!(
            mu_t_mub_id.to_hex(),
            "0165f8433e9823ce96f4b3758754f555006d0a3d074d2311d5b2424cb0101ec5",
            "journaling must not change the mu_t_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_t_mub_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_t_to_mu_n = lab
            .exec(Command::Constant {
                name: Some("mu_t_muN".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_t_to_mu_n.contains("constant  mu_t_muN  node "),
            "{mu_t_to_mu_n}"
        );
        assert!(
            mu_t_to_mu_n.contains(
                "hash     88f8acc9ba93fb7694b33fa92505cf9ade50157256c713a95b5802113dfe5c65"
            ),
            "{mu_t_to_mu_n}"
        );
        assert!(mu_t_to_mu_n.contains("kind     interval"), "{mu_t_to_mu_n}");
        assert!(mu_t_to_mu_n.contains("table    XXXI"), "{mu_t_to_mu_n}");
        assert!(
            mu_t_to_mu_n.contains("range    mu_t/muN = 2.9789624656(59)"),
            "{mu_t_to_mu_n}"
        );
        assert!(mu_t_to_mu_n.contains("unit     1"), "{mu_t_to_mu_n}");
        assert!(
            mu_t_to_mu_n.contains("value    [29789624597/10000000000, 5957924943/2000000000]"),
            "{mu_t_to_mu_n}"
        );
        assert!(mu_t_to_mu_n.contains("rebuild  ok"), "{mu_t_to_mu_n}");
        assert!(mu_t_to_mu_n.contains("not P3N"), "{mu_t_to_mu_n}");
        assert!(!mu_t_to_mu_n.contains("receipt"), "{mu_t_to_mu_n}");
        assert!(!mu_t_to_mu_n.contains("theorem"), "{mu_t_to_mu_n}");
        let mu_t_to_mu_n_id = constant_node_id(&mu_t_to_mu_n);
        assert_eq!(
            mu_t_to_mu_n_id.to_hex(),
            "499abd51eb8397c380382aaa5cdf4e713c71f2feb1ef36b0c8d3faa31a268266",
            "journaling must not change the mu_t_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_t_to_mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gt = lab
            ...
            .text()
            .to_string();
        assert!(gt.contains("constant  gt  node "), "{gt}");
        assert!(
            gt.contains(
                "hash     2f77c0ee28920e4866250d271d9288df2b68a2f82655588827e4678e5c0825ac"
            ),
            "{gt}"
        );
        assert!(gt.contains("kind     interval"), "{gt}");
        assert!(gt.contains("table    XXXI"), "{gt}");
        assert!(gt.contains("range    gt = 5.957924931(12)"), "{gt}");
        assert!(gt.contains("unit     1"), "{gt}");
        assert!(
            gt.contains("value    [5957924919/1000000000, 5957924943/1000000000]"),
            "{gt}"
        );
        assert!(gt.contains("rebuild  ok"), "{gt}");
        assert!(gt.contains("not P3N"), "{gt}");
        assert!(!gt.contains("receipt"), "{gt}");
        assert!(!gt.contains("theorem"), "{gt}");
        let gt_id = constant_node_id(&gt);
        assert_eq!(
            gt_id.to_hex(),
            "78102b0ca8a56bb2e5a99294daa66bce2d2899a0527102b56ffc0add1b2348f8",
            "journaling must not change the gt constant payload"
        );
        assert_eq!(
            lab.store.get(gt_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_h = lab
            ...
            .text()
            .to_string();
        assert!(m_h.contains("constant  m_h  node "), "{m_h}");
        assert!(
            m_h.contains(
                "hash     1a53756d23bdbbc188edb9cc55d1f3a9e5cc952d386bf885530507a4cded2492"
            ),
            "{m_h}"
        );
        assert!(m_h.contains("kind     interval"), "{m_h}");
        assert!(m_h.contains("table    XXXI"), "{m_h}");
        assert!(m_h.contains("range    mh = 5.0064127796(15)e-27"), "{m_h}");
        assert!(m_h.contains("unit     kg"), "{m_h}");
        assert!(
            m_h.contains("value    [50064127781/10000000000000000000000000000000000000, 50064127811/10000000000000000000000000000000000000]"),
            "{m_h}"
        );
        assert!(m_h.contains("rebuild  ok"), "{m_h}");
        assert!(m_h.contains("not P3N"), "{m_h}");
        assert!(!m_h.contains("receipt"), "{m_h}");
        assert!(!m_h.contains("theorem"), "{m_h}");
        let m_h_id = constant_node_id(&m_h);
        assert_eq!(
            m_h_id.to_hex(),
            "dd7fb3d8d0f506e48199cd8b575f47d9668d7968a53a4d82a9d40c0f62e4cb9a",
            "journaling must not change the m_h constant payload"
        );
        assert_eq!(
            lab.store.get(m_h_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_h_u = lab
            .exec(Command::Constant {
                name: Some("m_h_u".into()),
            })
            .text()
            .to_string();
        assert!(m_h_u.contains("constant  m_h_u  node "), "{m_h_u}");
        assert!(
            m_h_u.contains(
                "hash     652b05943359b759c6dd9125ca3fb791ebfe9272583a413f3d752ca742f5da51"
            ),
            "{m_h_u}"
        );
        assert!(m_h_u.contains("kind     interval"), "{m_h_u}");
        assert!(m_h_u.contains("table    XXXI"), "{m_h_u}");
        assert!(
            m_h_u.contains("range    mh_u = 3.014932247175(97)"),
            "{m_h_u}"
        );
        assert!(m_h_u.contains("unit     u"), "{m_h_u}");
        assert!(
            m_h_u.contains("value    [1507466123539/500000000000, 376866530909/125000000000]"),
            "{m_h_u}"
        );
        assert!(m_h_u.contains("rebuild  ok"), "{m_h_u}");
        assert!(m_h_u.contains("not P3N"), "{m_h_u}");
        assert!(!m_h_u.contains("receipt"), "{m_h_u}");
        assert!(!m_h_u.contains("theorem"), "{m_h_u}");
        let m_h_u_id = constant_node_id(&m_h_u);
        assert_eq!(
            m_h_u_id.to_hex(),
            "78d82dde163ee7a883b7e0748ee24f0af600a8d8feb762eaec4dccbce750e07a",
            "journaling must not change the m_h_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_h_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_h_c2 = lab
            .exec(Command::Constant {
                name: Some("m_h_c2".into()),
            })
            .text()
            .to_string();
        assert!(m_h_c2.contains("constant  m_h_c2  node "), "{m_h_c2}");
        assert!(
            m_h_c2.contains(
                "hash     33cd4c3d375911c13d541bf56f9664717286c73fa2ef1ebe83e2728a196a8a46"
            ),
            "{m_h_c2}"
        );
        assert!(m_h_c2.contains("kind     interval"), "{m_h_c2}");
        assert!(m_h_c2.contains("table    XXXI"), "{m_h_c2}");
        assert!(
            m_h_c2.contains("range    mhc2 = 4.4995394125(14)e-10"),
            "{m_h_c2}"
        );
        assert!(m_h_c2.contains("unit     J"), "{m_h_c2}");
        assert!(
            m_h_c2.contains(
                "value    [44995394111/100000000000000000000, 44995394139/100000000000000000000]"
            ),
            "{m_h_c2}"
        );
        assert!(m_h_c2.contains("rebuild  ok"), "{m_h_c2}");
        assert!(m_h_c2.contains("not P3N"), "{m_h_c2}");
        assert!(!m_h_c2.contains("receipt"), "{m_h_c2}");
        assert!(!m_h_c2.contains("theorem"), "{m_h_c2}");
        let m_h_c2_id = constant_node_id(&m_h_c2);
        assert_eq!(
            m_h_c2_id.to_hex(),
            "ca342c9a437d6127ac6cc8fa5219e366912504aadbd7d9c13ebea57feda513a2",
            "journaling must not change the m_h_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_h_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_h_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_h_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_h_c2_mev.contains("constant  m_h_c2_MeV  node "),
            "{m_h_c2_mev}"
        );
        assert!(
            m_h_c2_mev.contains(
                "hash     578eec087078da75a87dc84591b521cdf65581fda497b249d2e3e8e2bb6a4e8a"
            ),
            "{m_h_c2_mev}"
        );
        assert!(m_h_c2_mev.contains("kind     interval"), "{m_h_c2_mev}");
        assert!(m_h_c2_mev.contains("table    XXXI"), "{m_h_c2_mev}");
        assert!(
            m_h_c2_mev.contains("range    mhc2_MeV = 2808.39160743(85)"),
            "{m_h_c2_mev}"
        );
        assert!(m_h_c2_mev.contains("unit     MeV"), "{m_h_c2_mev}");
        assert!(
            m_h_c2_mev.contains("value    [140419580329/50000000, 70209790207/25000000]"),
            "{m_h_c2_mev}"
        );
        assert!(m_h_c2_mev.contains("rebuild  ok"), "{m_h_c2_mev}");
        assert!(m_h_c2_mev.contains("not P3N"), "{m_h_c2_mev}");
        assert!(!m_h_c2_mev.contains("receipt"), "{m_h_c2_mev}");
        assert!(!m_h_c2_mev.contains("theorem"), "{m_h_c2_mev}");
        let m_h_c2_mev_id = constant_node_id(&m_h_c2_mev);
        assert_eq!(
            m_h_c2_mev_id.to_hex(),
            "7b477dc07fbc9093483a14d474da85361c3f1b41df0815caf12c3c3c4c6750f8",
            "journaling must not change the m_h_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_h_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mh_me = lab
            .exec(Command::Constant {
                name: Some("mh_me".into()),
            })
            .text()
            .to_string();
        assert!(mh_me.contains("constant  mh_me  node "), "{mh_me}");
        assert!(
            mh_me.contains(
                "hash     2456ff97efcbfce853572c4f0bedf455455afedf9cec1628db183f23c2fe5368"
            ),
            "{mh_me}"
        );
        assert!(mh_me.contains("kind     interval"), "{mh_me}");
        assert!(mh_me.contains("table    XXXI"), "{mh_me}");
        assert!(
            mh_me.contains("range    mh/me = 5495.88528007(24)"),
            "{mh_me}"
        );
        assert!(mh_me.contains("unit     1"), "{mh_me}");
        assert!(
            mh_me.contains("value    [549588527983/100000000, 549588528031/100000000]"),
            "{mh_me}"
        );
        assert!(mh_me.contains("rebuild  ok"), "{mh_me}");
        assert!(mh_me.contains("not P3N"), "{mh_me}");
        assert!(!mh_me.contains("receipt"), "{mh_me}");
        assert!(!mh_me.contains("theorem"), "{mh_me}");
        let mh_me_id = constant_node_id(&mh_me);
        assert_eq!(
            mh_me_id.to_hex(),
            "ae4ffe1f19def746094bea82f05535eb48236670eb61d5eeaed0c821c42ce1eb",
            "journaling must not change the mh_me constant payload"
        );
        assert_eq!(
            lab.store.get(mh_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mh_mp = lab
            .exec(Command::Constant {
                name: Some("mh_mp".into()),
            })
            .text()
            .to_string();
        assert!(mh_mp.contains("constant  mh_mp  node "), "{mh_mp}");
        assert!(
            mh_mp.contains(
                "hash     46ae4686293f59d147df432b57e622e0883c8611813e0684326e4f2fd00d5c6d"
            ),
            "{mh_mp}"
        );
        assert!(mh_mp.contains("kind     interval"), "{mh_mp}");
        assert!(mh_mp.contains("table    XXXI"), "{mh_mp}");
        assert!(
            mh_mp.contains("range    mh/mp = 2.99315267167(13)"),
            "{mh_mp}"
        );
        assert!(mh_mp.contains("unit     1"), "{mh_mp}");
        assert!(
            mh_mp.contains("value    [149657633577/50000000000, 14965763359/5000000000]"),
            "{mh_mp}"
        );
        assert!(mh_mp.contains("rebuild  ok"), "{mh_mp}");
        assert!(mh_mp.contains("not P3N"), "{mh_mp}");
        assert!(!mh_mp.contains("receipt"), "{mh_mp}");
        assert!(!mh_mp.contains("theorem"), "{mh_mp}");
        let mh_mp_id = constant_node_id(&mh_mp);
        assert_eq!(
            mh_mp_id.to_hex(),
            "cd44c6dd61faf6bab9fc807f39873493ef1f0608501f15958cf18df3900b8e0e",
            "journaling must not change the mh_mp constant payload"
        );
        assert_eq!(
            lab.store.get(mh_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_h_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_h_molar.contains("constant  M_h  node "), "{m_h_molar}");
        assert!(
            m_h_molar.contains(
                "hash     a03524238032cd0ba44f2ed25ec8d2a62a5259d7b5353a00df939716d6efb7bf"
            ),
            "{m_h_molar}"
        );
        assert!(m_h_molar.contains("kind     interval"), "{m_h_molar}");
        assert!(m_h_molar.contains("table    XXXI"), "{m_h_molar}");
        assert!(
            m_h_molar.contains("range    Mh = 3.01493224613(91)e-3"),
            "{m_h_molar}"
        );
        assert!(m_h_molar.contains("unit     kg mol^{-1}"), "{m_h_molar}");
        assert!(
            m_h_molar.contains("value    [150746612261/50000000000000, 1177707909/390625000000]"),
            "{m_h_molar}"
        );
        assert!(m_h_molar.contains("rebuild  ok"), "{m_h_molar}");
        assert!(m_h_molar.contains("not P3N"), "{m_h_molar}");
        assert!(!m_h_molar.contains("receipt"), "{m_h_molar}");
        assert!(!m_h_molar.contains("theorem"), "{m_h_molar}");
        let m_h_molar_id = constant_node_id(&m_h_molar);
        assert_eq!(
            m_h_molar_id.to_hex(),
            "846bd5680670d73586466f859106f679a20e316bc8f379fd20a0f0198de56c89",
            "journaling must not change the M_h constant payload"
        );
        assert_eq!(
            lab.store.get(m_h_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_h = lab
            ...
            .text()
            .to_string();
        assert!(mu_h.contains("constant  mu_h  node "), "{mu_h}");
        assert!(
            mu_h.contains(
                "hash     c2cc411107459d5d4bbf914e1dc3eab7b9abb63d9a5189b497c5390169fe577f"
            ),
            "{mu_h}"
        );
        assert!(mu_h.contains("kind     interval"), "{mu_h}");
        assert!(mu_h.contains("table    XXXI"), "{mu_h}");
        assert!(
            mu_h.contains("range    mu_h = -1.074617532(13)e-26"),
            "{mu_h}"
        );
        assert!(mu_h.contains("unit     J T^{-1}"), "{mu_h}");
        assert!(
            mu_h.contains("value    [-214923509/20000000000000000000000000000000000, -1074617519/100000000000000000000000000000000000]"),
            "{mu_h}"
        );
        assert!(mu_h.contains("rebuild  ok"), "{mu_h}");
        assert!(mu_h.contains("not P3N"), "{mu_h}");
        assert!(!mu_h.contains("receipt"), "{mu_h}");
        assert!(!mu_h.contains("theorem"), "{mu_h}");
        let mu_h_id = constant_node_id(&mu_h);
        assert_eq!(
            mu_h_id.to_hex(),
            "d915995be231b6c53b6d71929708c0b6316d07b165cc70da16e043b6acc04ff5",
            "journaling must not change the mu_h constant payload"
        );
        assert_eq!(
            lab.store.get(mu_h_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_h_mub = lab
            .exec(Command::Constant {
                name: Some("mu_h_muB".into()),
            })
            .text()
            .to_string();
        assert!(mu_h_mub.contains("constant  mu_h_muB  node "), "{mu_h_mub}");
        assert!(
            mu_h_mub.contains(
                "hash     bffb76fa55e4a2baad9d666cebb1aa1df1bc5c500d58e081ee25fde1ba67fac5"
            ),
            "{mu_h_mub}"
        );
        assert!(mu_h_mub.contains("kind     interval"), "{mu_h_mub}");
        assert!(mu_h_mub.contains("table    XXXI"), "{mu_h_mub}");
        assert!(
            mu_h_mub.contains("range    mu_h/muB = -1.158740958(14)e-3"),
            "{mu_h_mub}"
        );
        assert!(mu_h_mub.contains("unit     1"), "{mu_h_mub}");
        assert!(
            mu_h_mub.contains("value    [-289685243/250000000000, -72421309/62500000000]"),
            "{mu_h_mub}"
        );
        assert!(mu_h_mub.contains("rebuild  ok"), "{mu_h_mub}");
        assert!(mu_h_mub.contains("not P3N"), "{mu_h_mub}");
        assert!(!mu_h_mub.contains("receipt"), "{mu_h_mub}");
        assert!(!mu_h_mub.contains("theorem"), "{mu_h_mub}");
        let mu_h_mub_id = constant_node_id(&mu_h_mub);
        assert_eq!(
            mu_h_mub_id.to_hex(),
            "3bb0b020d185e2a5a5fa6fd0448fca4a0c41a3cafa42e2e347e4f86d0dfacb32",
            "journaling must not change the mu_h_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_h_mub_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_h_to_mu_n = lab
            .exec(Command::Constant {
                name: Some("mu_h_muN".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_h_to_mu_n.contains("constant  mu_h_muN  node "),
            "{mu_h_to_mu_n}"
        );
        assert!(
            mu_h_to_mu_n.contains(
                "hash     b8daa265220c3de6776c9b0703e094ea1e56ce2e7d8f5e4c330f3bb38b513e3e"
            ),
            "{mu_h_to_mu_n}"
        );
        assert!(mu_h_to_mu_n.contains("kind     interval"), "{mu_h_to_mu_n}");
        assert!(mu_h_to_mu_n.contains("table    XXXI"), "{mu_h_to_mu_n}");
        assert!(
            mu_h_to_mu_n.contains("range    mu_h/muN = -2.127625307(25)"),
            "{mu_h_to_mu_n}"
        );
        assert!(mu_h_to_mu_n.contains("unit     1"), "{mu_h_to_mu_n}");
        assert!(
            mu_h_to_mu_n.contains("value    [-531906333/250000000, -1063812641/500000000]"),
            "{mu_h_to_mu_n}"
        );
        assert!(mu_h_to_mu_n.contains("rebuild  ok"), "{mu_h_to_mu_n}");
        assert!(mu_h_to_mu_n.contains("not P3N"), "{mu_h_to_mu_n}");
        assert!(!mu_h_to_mu_n.contains("receipt"), "{mu_h_to_mu_n}");
        assert!(!mu_h_to_mu_n.contains("theorem"), "{mu_h_to_mu_n}");
        let mu_h_to_mu_n_id = constant_node_id(&mu_h_to_mu_n);
        assert_eq!(
            mu_h_to_mu_n_id.to_hex(),
            "d5ec8eaa8842a4d6a63449e9c864686d2fe786bb47186ec5be6d6fe86c398e2e",
            "journaling must not change the mu_h_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_h_to_mu_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gh = lab
            ...
            .text()
            .to_string();
        assert!(gh.contains("constant  gh  node "), "{gh}");
        assert!(
            gh.contains(
                "hash     89764c03ec4774afa24862fca730205f559747ba9256e7378c64629720d31c4f"
            ),
            "{gh}"
        );
        assert!(gh.contains("kind     interval"), "{gh}");
        assert!(gh.contains("table    XXXI"), "{gh}");
        assert!(gh.contains("range    gh = -4.255250615(50)"), "{gh}");
        assert!(gh.contains("unit     1"), "{gh}");
        assert!(
            gh.contains("value    [-851050133/200000000, -851050113/200000000]"),
            "{gh}"
        );
        assert!(gh.contains("rebuild  ok"), "{gh}");
        assert!(gh.contains("not P3N"), "{gh}");
        assert!(!gh.contains("receipt"), "{gh}");
        assert!(!gh.contains("theorem"), "{gh}");
        let gh_id = constant_node_id(&gh);
        assert_eq!(
            gh_id.to_hex(),
            "89d787cea08d18f680faa8fa0e4e2bc9e25ed8d4c4f649ba7bfe47ed4096e90f",
            "journaling must not change the gh constant payload"
        );
        assert_eq!(
            lab.store.get(gh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu0h = lab
            ...
            .text()
            .to_string();
        assert!(mu0h.contains("constant  mu0h  node "), "{mu0h}");
        assert!(
            mu0h.contains(
                "hash     f207205c27290f0b85017413fd3cd47593d77a3ad71a7d4337c96d0bff8ff559"
            ),
            "{mu0h}"
        );
        assert!(mu0h.contains("kind     interval"), "{mu0h}");
        assert!(mu0h.contains("table    XXXI"), "{mu0h}");
        assert!(
            mu0h.contains("range    mu0h = -1.074553090(13)e-26"),
            "{mu0h}"
        );
        assert!(mu0h.contains("unit     J T^{-1}"), "{mu0h}");
        assert!(
            mu0h.contains("value    [-1074553103/100000000000000000000000000000000000, -1074553077/100000000000000000000000000000000000]"),
            "{mu0h}"
        );
        assert!(mu0h.contains("rebuild  ok"), "{mu0h}");
        assert!(mu0h.contains("not P3N"), "{mu0h}");
        assert!(!mu0h.contains("receipt"), "{mu0h}");
        assert!(!mu0h.contains("theorem"), "{mu0h}");
        let mu0h_id = constant_node_id(&mu0h);
        assert_eq!(
            mu0h_id.to_hex(),
            "1292784df9b775d6115c93d72a320c32327fccef404f1e628513353025312cc3",
            "journaling must not change the mu0h constant payload"
        );
        assert_eq!(
            lab.store.get(mu0h_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0h_mub = lab
            .exec(Command::Constant {
                name: Some("mu0h_muB".into()),
            })
            .text()
            .to_string();
        assert!(mu0h_mub.contains("constant  mu0h_muB  node "), "{mu0h_mub}");
        assert!(
            mu0h_mub.contains(
                "hash     9265606a87f0abee2603a308bba4322bd8fc1ac40513d8e8bb441cbeaa3e7c91"
            ),
            "{mu0h_mub}"
        );
        assert!(mu0h_mub.contains("kind     interval"), "{mu0h_mub}");
        assert!(mu0h_mub.contains("table    XXXI"), "{mu0h_mub}");
        assert!(
            mu0h_mub.contains("range    mu0h/muB = -1.158671471(14)e-3"),
            "{mu0h_mub}"
        );
        assert!(mu0h_mub.contains("unit     1"), "{mu0h_mub}");
        assert!(
            mu0h_mub.contains("value    [-231734297/200000000000, -1158671457/1000000000000]"),
            "{mu0h_mub}"
        );
        assert!(mu0h_mub.contains("rebuild  ok"), "{mu0h_mub}");
        assert!(mu0h_mub.contains("not P3N"), "{mu0h_mub}");
        assert!(!mu0h_mub.contains("receipt"), "{mu0h_mub}");
        assert!(!mu0h_mub.contains("theorem"), "{mu0h_mub}");
        let mu0h_mub_id = constant_node_id(&mu0h_mub);
        assert_eq!(
            mu0h_mub_id.to_hex(),
            "418169685f940c3a11d1df4c4a98825558ff51c7aa59ff97872428511b1db9e2",
            "journaling must not change the mu0h_muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu0h_mub_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0h_mun = lab
            .exec(Command::Constant {
                name: Some("mu0h_muN".into()),
            })
            .text()
            .to_string();
        assert!(mu0h_mun.contains("constant  mu0h_muN  node "), "{mu0h_mun}");
        assert!(
            mu0h_mun.contains(
                "hash     ee1cdad515f45e0be5c09c52afd3fbe0f4e5aab9b26d25218e5d09de60f2b4b5"
            ),
            "{mu0h_mun}"
        );
        assert!(mu0h_mun.contains("kind     interval"), "{mu0h_mun}");
        assert!(mu0h_mun.contains("table    XXXI"), "{mu0h_mun}");
        assert!(
            mu0h_mun.contains("range    mu0h/muN = -2.127497719(25)"),
            "{mu0h_mun}"
        );
        assert!(mu0h_mun.contains("unit     1"), "{mu0h_mun}");
        assert!(
            mu0h_mun.contains("value    [-132968609/62500000, -1063748847/500000000]"),
            "{mu0h_mun}"
        );
        assert!(mu0h_mun.contains("rebuild  ok"), "{mu0h_mun}");
        assert!(mu0h_mun.contains("not P3N"), "{mu0h_mun}");
        assert!(!mu0h_mun.contains("receipt"), "{mu0h_mun}");
        assert!(!mu0h_mun.contains("theorem"), "{mu0h_mun}");
        let mu0h_mun_id = constant_node_id(&mu0h_mun);
        assert_eq!(
            mu0h_mun_id.to_hex(),
            "b9f0dcbb3f7f4a7d7a965e00f7d25cf6f8229b62aaa1ac187f07c85387046b47",
            "journaling must not change the mu0h_muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu0h_mun_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0h_mup = lab
            .exec(Command::Constant {
                name: Some("mu0h_mup".into()),
            })
            .text()
            .to_string();
        assert!(mu0h_mup.contains("constant  mu0h_mup  node "), "{mu0h_mup}");
        assert!(
            mu0h_mup.contains(
                "hash     2a8e0963d82d303bc3cc41ef13d266a3e6aa732e724579beb1361a7cf849c80f"
            ),
            "{mu0h_mup}"
        );
        assert!(mu0h_mup.contains("kind     interval"), "{mu0h_mup}");
        assert!(mu0h_mup.contains("table    XXXI"), "{mu0h_mup}");
        assert!(
            mu0h_mup.contains("range    mu0h/mup = -0.7617665618(89)"),
            "{mu0h_mup}"
        );
        assert!(mu0h_mup.contains("unit     1"), "{mu0h_mup}");
        assert!(
            mu0h_mup.contains("value    [-7617665707/10000000000, -7617665529/10000000000]"),
            "{mu0h_mup}"
        );
        assert!(mu0h_mup.contains("rebuild  ok"), "{mu0h_mup}");
        assert!(mu0h_mup.contains("not P3N"), "{mu0h_mup}");
        assert!(!mu0h_mup.contains("receipt"), "{mu0h_mup}");
        assert!(!mu0h_mup.contains("theorem"), "{mu0h_mup}");
        let mu0h_mup_id = constant_node_id(&mu0h_mup);
        assert_eq!(
            mu0h_mup_id.to_hex(),
            "12d5e03dfa5101a9f92901c642bd9870cbe5340890fab192a12a0e18a36d9d97",
            "journaling must not change the mu0h_mup constant payload"
        );
        assert_eq!(
            lab.store.get(mu0h_mup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu0h_mu0p = lab
            .exec(Command::Constant {
                name: Some("mu0h_mu0p".into()),
            })
            .text()
            .to_string();
        assert!(
            mu0h_mu0p.contains("constant  mu0h_mu0p  node "),
            "{mu0h_mu0p}"
        );
        assert!(
            mu0h_mu0p.contains(
                "hash     18c53bbc40a6c0c95dc9af5019cf6a9482b3ae1918cd6381c949250e95bebd90"
            ),
            "{mu0h_mu0p}"
        );
        assert!(mu0h_mu0p.contains("kind     interval"), "{mu0h_mu0p}");
        assert!(mu0h_mu0p.contains("table    XXXI"), "{mu0h_mu0p}");
        assert!(
            mu0h_mu0p.contains("range    mu0h/mu0p = -0.7617861313(33)"),
            "{mu0h_mu0p}"
        );
        assert!(mu0h_mu0p.contains("unit     1"), "{mu0h_mu0p}");
        assert!(
            mu0h_mu0p.contains("value    [-3808930673/5000000000, -47611633/62500000]"),
            "{mu0h_mu0p}"
        );
        assert!(mu0h_mu0p.contains("rebuild  ok"), "{mu0h_mu0p}");
        assert!(mu0h_mu0p.contains("not P3N"), "{mu0h_mu0p}");
        assert!(!mu0h_mu0p.contains("receipt"), "{mu0h_mu0p}");
        assert!(!mu0h_mu0p.contains("theorem"), "{mu0h_mu0p}");
        let mu0h_mu0p_id = constant_node_id(&mu0h_mu0p);
        assert_eq!(
            mu0h_mu0p_id.to_hex(),
            "b92f8c9af39f5e7287562555b7531498bab3028db3f4bf11816d29ec3b323ef6",
            "journaling must not change the mu0h_mu0p constant payload"
        );
        assert_eq!(
            lab.store.get(mu0h_mu0p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_alpha = lab
            .exec(Command::Constant {
                name: Some("m_alpha".into()),
            })
            .text()
            .to_string();
        assert!(m_alpha.contains("constant  m_alpha  node "), "{m_alpha}");
        assert!(
            m_alpha.contains(
                "hash     8f3ec14a8381c0b83aba64d6f42a44dcd12b59e65bcf4ff11ab7edb36b4296c4"
            ),
            "{m_alpha}"
        );
        assert!(m_alpha.contains("kind     interval"), "{m_alpha}");
        assert!(m_alpha.contains("table    XXXI"), "{m_alpha}");
        assert!(
            m_alpha.contains("range    m_alpha = 6.6446573357(20)e-27"),
            "{m_alpha}"
        );
        assert!(m_alpha.contains("unit     kg"), "{m_alpha}");
        assert!(
            m_alpha.contains("value    [66446573337/10000000000000000000000000000000000000, 66446573377/10000000000000000000000000000000000000]"),
            "{m_alpha}"
        );
        assert!(m_alpha.contains("rebuild  ok"), "{m_alpha}");
        assert!(m_alpha.contains("not P3N"), "{m_alpha}");
        assert!(!m_alpha.contains("receipt"), "{m_alpha}");
        assert!(!m_alpha.contains("theorem"), "{m_alpha}");
        let m_alpha_id = constant_node_id(&m_alpha);
        assert_eq!(
            m_alpha_id.to_hex(),
            "162b3c933eb5fc4e331a1d3b9a597f0025d94678c7c0a3ea6916c9a8249d554f",
            "journaling must not change the m_alpha constant payload"
        );
        assert_eq!(
            lab.store.get(m_alpha_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_alpha_u = lab
            .exec(Command::Constant {
                name: Some("m_alpha_u".into()),
            })
            .text()
            .to_string();
        assert!(
            m_alpha_u.contains("constant  m_alpha_u  node "),
            "{m_alpha_u}"
        );
        assert!(
            m_alpha_u.contains(
                "hash     69b88fb739109ee090cb94bf699ce2a8b76c941e8b279c5b9f28b6273fa67935"
            ),
            "{m_alpha_u}"
        );
        assert!(m_alpha_u.contains("kind     interval"), "{m_alpha_u}");
        assert!(m_alpha_u.contains("table    XXXI"), "{m_alpha_u}");
        assert!(
            m_alpha_u.contains("range    m_alpha_u = 4.001506179127(63)"),
            "{m_alpha_u}"
        );
        assert!(m_alpha_u.contains("unit     u"), "{m_alpha_u}");
        assert!(
            m_alpha_u.contains("value    [500188272383/125000000000, 400150617919/100000000000]"),
            "{m_alpha_u}"
        );
        assert!(m_alpha_u.contains("rebuild  ok"), "{m_alpha_u}");
        assert!(m_alpha_u.contains("not P3N"), "{m_alpha_u}");
        assert!(!m_alpha_u.contains("receipt"), "{m_alpha_u}");
        assert!(!m_alpha_u.contains("theorem"), "{m_alpha_u}");
        let m_alpha_u_id = constant_node_id(&m_alpha_u);
        assert_eq!(
            m_alpha_u_id.to_hex(),
            "2f8cdcc85902aaf6946137138a2db30503ea12910fbedb15772898d906a7f5df",
            "journaling must not change the m_alpha_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_alpha_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_alpha_c2 = lab
            .exec(Command::Constant {
                name: Some("m_alpha_c2".into()),
            })
            .text()
            .to_string();
        assert!(
            m_alpha_c2.contains("constant  m_alpha_c2  node "),
            "{m_alpha_c2}"
        );
        assert!(
            m_alpha_c2.contains(
                "hash     f3850fe7ecb1fd455ca3422b528eda05a5caf07f149b85c1270102b987fc723e"
            ),
            "{m_alpha_c2}"
        );
        assert!(m_alpha_c2.contains("kind     interval"), "{m_alpha_c2}");
        assert!(m_alpha_c2.contains("table    XXXI"), "{m_alpha_c2}");
        assert!(
            m_alpha_c2.contains("range    m_alpha_c2 = 5.9719201914(18)e-10"),
            "{m_alpha_c2}"
        );
        assert!(m_alpha_c2.contains("unit     J"), "{m_alpha_c2}");
        assert!(
            m_alpha_c2.contains(
                "value    [7464900237/12500000000000000000, 14929800483/25000000000000000000]"
            ),
            "{m_alpha_c2}"
        );
        assert!(m_alpha_c2.contains("rebuild  ok"), "{m_alpha_c2}");
        assert!(m_alpha_c2.contains("not P3N"), "{m_alpha_c2}");
        assert!(!m_alpha_c2.contains("receipt"), "{m_alpha_c2}");
        assert!(!m_alpha_c2.contains("theorem"), "{m_alpha_c2}");
        let m_alpha_c2_id = constant_node_id(&m_alpha_c2);
        assert_eq!(
            m_alpha_c2_id.to_hex(),
            "64a4c00516144551ce261049edb83f3f09431ed32b0da56416dd7f02c51d32ac",
            "journaling must not change the m_alpha_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_alpha_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_alpha_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_alpha_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_alpha_c2_mev.contains("constant  m_alpha_c2_MeV  node "),
            "{m_alpha_c2_mev}"
        );
        assert!(
            m_alpha_c2_mev.contains(
                "hash     5a924d0532dc37f1e318a7dd27ad318160dd7a606d6ddc75fac704b5277a4a42"
            ),
            "{m_alpha_c2_mev}"
        );
        assert!(
            m_alpha_c2_mev.contains("kind     interval"),
            "{m_alpha_c2_mev}"
        );
        assert!(m_alpha_c2_mev.contains("table    XXXI"), "{m_alpha_c2_mev}");
        assert!(
            m_alpha_c2_mev.contains("range    m_alpha_c2_MeV = 3727.3794066(11)"),
            "{m_alpha_c2_mev}"
        );
        assert!(m_alpha_c2_mev.contains("unit     MeV"), "{m_alpha_c2_mev}");
        assert!(
            m_alpha_c2_mev.contains("value    [7454758811/2000000, 37273794077/10000000]"),
            "{m_alpha_c2_mev}"
        );
        assert!(m_alpha_c2_mev.contains("rebuild  ok"), "{m_alpha_c2_mev}");
        assert!(m_alpha_c2_mev.contains("not P3N"), "{m_alpha_c2_mev}");
        assert!(!m_alpha_c2_mev.contains("receipt"), "{m_alpha_c2_mev}");
        assert!(!m_alpha_c2_mev.contains("theorem"), "{m_alpha_c2_mev}");
        let m_alpha_c2_mev_id = constant_node_id(&m_alpha_c2_mev);
        assert_eq!(
            m_alpha_c2_mev_id.to_hex(),
            "cfc774331f8e78a1784d3585d0831969df0d21dc26cd5be819922d0706b33d3b",
            "journaling must not change the m_alpha_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_alpha_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let malpha_me = lab
            .exec(Command::Constant {
                name: Some("malpha_me".into()),
            })
            .text()
            .to_string();
        assert!(
            malpha_me.contains("constant  malpha_me  node "),
            "{malpha_me}"
        );
        assert!(
            malpha_me.contains(
                "hash     72ecb3fbc0d48ab53a5d6f3a08cbe581b7909dc03ece21eb07cb35887e7c68c1"
            ),
            "{malpha_me}"
        );
        assert!(malpha_me.contains("kind     interval"), "{malpha_me}");
        assert!(malpha_me.contains("table    XXXI"), "{malpha_me}");
        assert!(
            malpha_me.contains("range    malpha/me = 7294.29954142(24)"),
            "{malpha_me}"
        );
        assert!(malpha_me.contains("unit     1"), "{malpha_me}");
        assert!(
            malpha_me.contains("value    [364714977059/50000000, 364714977083/50000000]"),
            "{malpha_me}"
        );
        assert!(malpha_me.contains("rebuild  ok"), "{malpha_me}");
        assert!(malpha_me.contains("not P3N"), "{malpha_me}");
        assert!(!malpha_me.contains("receipt"), "{malpha_me}");
        assert!(!malpha_me.contains("theorem"), "{malpha_me}");
        let malpha_me_id = constant_node_id(&malpha_me);
        assert_eq!(
            malpha_me_id.to_hex(),
            "e78c04dd2b05c41c6d7bcd91830d202d14c3dfe104743e86305445a5d0b52937",
            "journaling must not change the malpha_me constant payload"
        );
        assert_eq!(
            lab.store.get(malpha_me_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let malpha_mp = lab
            .exec(Command::Constant {
                name: Some("malpha_mp".into()),
            })
            .text()
            .to_string();
        assert!(
            malpha_mp.contains("constant  malpha_mp  node "),
            "{malpha_mp}"
        );
        assert!(
            malpha_mp.contains(
                "hash     0c31195c0e868eb3e6b4a54c10ed662a1075d58cad2e565ad5ec5f389f7c567d"
            ),
            "{malpha_mp}"
        );
        assert!(malpha_mp.contains("kind     interval"), "{malpha_mp}");
        assert!(malpha_mp.contains("table    XXXI"), "{malpha_mp}");
        assert!(
            malpha_mp.contains("range    malpha/mp = 3.97259969009(22)"),
            "{malpha_mp}"
        );
        assert!(malpha_mp.contains("unit     1"), "{malpha_mp}");
        assert!(
            malpha_mp.contains("value    [397259968987/100000000000, 397259969031/100000000000]"),
            "{malpha_mp}"
        );
        assert!(malpha_mp.contains("rebuild  ok"), "{malpha_mp}");
        assert!(malpha_mp.contains("not P3N"), "{malpha_mp}");
        assert!(!malpha_mp.contains("receipt"), "{malpha_mp}");
        assert!(!malpha_mp.contains("theorem"), "{malpha_mp}");
        let malpha_mp_id = constant_node_id(&malpha_mp);
        assert_eq!(
            malpha_mp_id.to_hex(),
            "172c865937b6a232151daee8e336fcef43c518721c21e67abbdc7bf95b044801",
            "journaling must not change the malpha_mp constant payload"
        );
        assert_eq!(
            lab.store.get(malpha_mp_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_alpha_molar = lab
            .exec(Command::Constant {
                name: Some("M_alpha".into()),
            })
            .text()
            .to_string();
        assert!(
            m_alpha_molar.contains("constant  M_alpha  node "),
            "{m_alpha_molar}"
        );
        assert!(
            m_alpha_molar.contains(
                "hash     247f1e1ac48e536f49b0e9ea42f1233960dc446e599c1177469c80d2b50fddf3"
            ),
            "{m_alpha_molar}"
        );
        assert!(
            m_alpha_molar.contains("kind     interval"),
            "{m_alpha_molar}"
        );
        assert!(m_alpha_molar.contains("table    XXXI"), "{m_alpha_molar}");
        assert!(
            m_alpha_molar.contains("range    Malpha = 4.0015061777(12)e-3"),
            "{m_alpha_molar}"
        );
        assert!(
            m_alpha_molar.contains("unit     kg mol^{-1}"),
            "{m_alpha_molar}"
        );
        assert!(
            m_alpha_molar
                .contains("value    [8003012353/2000000000000, 40015061789/10000000000000]"),
            "{m_alpha_molar}"
        );
        assert!(m_alpha_molar.contains("rebuild  ok"), "{m_alpha_molar}");
        assert!(m_alpha_molar.contains("not P3N"), "{m_alpha_molar}");
        assert!(!m_alpha_molar.contains("receipt"), "{m_alpha_molar}");
        assert!(!m_alpha_molar.contains("theorem"), "{m_alpha_molar}");
        let m_alpha_molar_id = constant_node_id(&m_alpha_molar);
        assert_eq!(
            m_alpha_molar_id.to_hex(),
            "d44a8e073d6b4105db288a959ad5cb0adf3a174239e2753a68e5c7f7981a83ac",
            "journaling must not change the M_alpha constant payload"
        );
        assert_eq!(
            lab.store.get(m_alpha_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_u = lab
            ...
            .text()
            .to_string();
        assert!(m_u.contains("constant  m_u  node "), "{m_u}");
        assert!(
            m_u.contains(
                "hash     fcefc139b85d5be198ab911fed33049d37641b01dcd0b87e12630db6dfd467d3"
            ),
            "{m_u}"
        );
        assert!(m_u.contains("kind     interval"), "{m_u}");
        assert!(m_u.contains("table    XXXI"), "{m_u}");
        assert!(m_u.contains("range    mu = 1.66053906660(50)e-27"), "{m_u}");
        assert!(m_u.contains("unit     kg"), "{m_u}");
        assert!(
            m_u.contains(
                "value    [16605390661/10000000000000000000000000000000000000, 16605390671/10000000000000000000000000000000000000]"
            ),
            "{m_u}"
        );
        assert!(m_u.contains("rebuild  ok"), "{m_u}");
        assert!(m_u.contains("not P3N"), "{m_u}");
        assert!(!m_u.contains("receipt"), "{m_u}");
        assert!(!m_u.contains("theorem"), "{m_u}");
        let m_u_id = constant_node_id(&m_u);
        assert_eq!(
            m_u_id.to_hex(),
            "2771039a06696ad5cd983ff13f28411b8d634e1d890869bb4cf6a6d8e9536b16",
            "journaling must not change the m_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_u_c2 = lab
            .exec(Command::Constant {
                name: Some("m_u_c2".into()),
            })
            .text()
            .to_string();
        assert!(m_u_c2.contains("constant  m_u_c2  node "), "{m_u_c2}");
        assert!(
            m_u_c2.contains(
                "hash     e3e57faf62ef6c5ef37e31fd959eedc747be1444953bb83f39c73590b0454750"
            ),
            "{m_u_c2}"
        );
        assert!(m_u_c2.contains("kind     interval"), "{m_u_c2}");
        assert!(m_u_c2.contains("table    XXXI"), "{m_u_c2}");
        assert!(
            m_u_c2.contains("range    muc2 = 1.49241808560(45)e-10"),
            "{m_u_c2}"
        );
        assert!(m_u_c2.contains("unit     J"), "{m_u_c2}");
        assert!(
            m_u_c2.contains(
                "value    [29848361703/200000000000000000000, 29848361721/200000000000000000000]"
            ),
            "{m_u_c2}"
        );
        assert!(m_u_c2.contains("rebuild  ok"), "{m_u_c2}");
        assert!(m_u_c2.contains("not P3N"), "{m_u_c2}");
        assert!(!m_u_c2.contains("receipt"), "{m_u_c2}");
        assert!(!m_u_c2.contains("theorem"), "{m_u_c2}");
        let m_u_c2_id = constant_node_id(&m_u_c2);
        assert_eq!(
            m_u_c2_id.to_hex(),
            "d6c812670024baec7c1ae41bbb4961d859585d2e1b4c0ee4596ed01d8eaaa684",
            "journaling must not change the m_u_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(m_u_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_u_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_u_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            m_u_c2_mev.contains("constant  m_u_c2_MeV  node "),
            "{m_u_c2_mev}"
        );
        assert!(
            m_u_c2_mev.contains(
                "hash     cbd8cdc0c0c358c2a7204f343ab90d33e1c342820eb596fdb67b330d872c9d3e"
            ),
            "{m_u_c2_mev}"
        );
        assert!(m_u_c2_mev.contains("kind     interval"), "{m_u_c2_mev}");
        assert!(m_u_c2_mev.contains("table    XXXI"), "{m_u_c2_mev}");
        assert!(
            m_u_c2_mev.contains("range    muc2_MeV = 931.49410242(28)"),
            "{m_u_c2_mev}"
        );
        assert!(m_u_c2_mev.contains("unit     MeV"), "{m_u_c2_mev}");
        assert!(
            m_u_c2_mev.contains("value    [46574705107/50000000, 9314941027/10000000]"),
            "{m_u_c2_mev}"
        );
        assert!(m_u_c2_mev.contains("rebuild  ok"), "{m_u_c2_mev}");
        assert!(m_u_c2_mev.contains("not P3N"), "{m_u_c2_mev}");
        assert!(!m_u_c2_mev.contains("receipt"), "{m_u_c2_mev}");
        assert!(!m_u_c2_mev.contains("theorem"), "{m_u_c2_mev}");
        let m_u_c2_mev_id = constant_node_id(&m_u_c2_mev);
        assert_eq!(
            m_u_c2_mev_id.to_hex(),
            "1e05445f79b116401e18e22766ad853e02804628ad5a00e7fd61c000fe93057f",
            "journaling must not change the m_u_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(m_u_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_u_molar = lab
            ...
            .text()
            .to_string();
        assert!(m_u_molar.contains("constant  M_u  node "), "{m_u_molar}");
        assert!(
            m_u_molar.contains(
                "hash     db927829cb6a1d796a00ab6509b3a9faf0e2f09ed7bb3dc5aca6154abb9e388e"
            ),
            "{m_u_molar}"
        );
        assert!(m_u_molar.contains("kind     interval"), "{m_u_molar}");
        assert!(m_u_molar.contains("table    XXXI"), "{m_u_molar}");
        assert!(
            m_u_molar.contains("range    Mu = 0.99999999965(30)e-3"),
            "{m_u_molar}"
        );
        assert!(m_u_molar.contains("unit     kg mol^{-1}"), "{m_u_molar}");
        assert!(
            m_u_molar.contains("value    [19999999987/20000000000000, 19999999999/20000000000000]"),
            "{m_u_molar}"
        );
        assert!(m_u_molar.contains("rebuild  ok"), "{m_u_molar}");
        assert!(m_u_molar.contains("not P3N"), "{m_u_molar}");
        assert!(!m_u_molar.contains("receipt"), "{m_u_molar}");
        assert!(!m_u_molar.contains("theorem"), "{m_u_molar}");
        let m_u_molar_id = constant_node_id(&m_u_molar);
        assert_eq!(
            m_u_molar_id.to_hex(),
            "20a2cd8642b0035f68a9c0487b930415e5472c5bbd393b2f97049772e6bea642",
            "journaling must not change the M_u constant payload"
        );
        assert_eq!(
            lab.store.get(m_u_molar_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let m_12c = lab
            .exec(Command::Constant {
                name: Some("M_12C".into()),
            })
            .text()
            .to_string();
        assert!(m_12c.contains("constant  M_12C  node "), "{m_12c}");
        assert!(
            m_12c.contains(
                "hash     bec80fb1c51bead2000a5ba56e2fd680fd79d5538dd3e0e0cd9ceca1fb983d43"
            ),
            "{m_12c}"
        );
        assert!(m_12c.contains("kind     interval"), "{m_12c}");
        assert!(m_12c.contains("table    XXXI"), "{m_12c}");
        assert!(
            m_12c.contains("range    M12C = 11.9999999958(36)e-3"),
            "{m_12c}"
        );
        assert!(m_12c.contains("unit     kg mol^{-1}"), "{m_12c}");
        assert!(
            m_12c.contains("value    [59999999961/5000000000000, 59999999997/5000000000000]"),
            "{m_12c}"
        );
        assert!(m_12c.contains("rebuild  ok"), "{m_12c}");
        assert!(m_12c.contains("not P3N"), "{m_12c}");
        assert!(!m_12c.contains("receipt"), "{m_12c}");
        assert!(!m_12c.contains("theorem"), "{m_12c}");
        let m_12c_id = constant_node_id(&m_12c);
        assert_eq!(
            m_12c_id.to_hex(),
            "2ad548958aa8e044a9f42f941c3a105621d41ed86c462eb8408fc30b76fdad74",
            "journaling must not change the M_12C constant payload"
        );
        assert_eq!(
            lab.store.get(m_12c_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let n_a_h = lab
            ...
            .text()
            .to_string();
        assert!(n_a_h.contains("constant  NAh  node "), "{n_a_h}");
        assert!(
            n_a_h.contains(
                "hash     9290f6b333a3a26c429b761769bd641d7a642b68d6e34cbea852119c170d6228"
            ),
            "{n_a_h}"
        );
        assert!(n_a_h.contains("kind     ratio"), "{n_a_h}");
        assert!(n_a_h.contains("table    XXXI"), "{n_a_h}");
        assert!(
            n_a_h.contains("range    NAh = 3.990312712e-10 exact"),
            "{n_a_h}"
        );
        assert!(n_a_h.contains("unit     J Hz^{-1} mol^{-1}"), "{n_a_h}");
        assert!(
            n_a_h.contains("value    19951563564467157/50000000000000000000000000"),
            "{n_a_h}"
        );
        assert!(n_a_h.contains("rebuild  ok"), "{n_a_h}");
        assert!(n_a_h.contains("not P3N"), "{n_a_h}");
        assert!(!n_a_h.contains("receipt"), "{n_a_h}");
        assert!(!n_a_h.contains("theorem"), "{n_a_h}");
        let n_a_h_id = constant_node_id(&n_a_h);
        assert_eq!(
            n_a_h_id.to_hex(),
            "2bcc73daab70d1b719c651342dbafc5473a4ee650ce29aacf4e3109bcfa6ebd1",
            "journaling must not change the NAh constant payload"
        );
        assert_eq!(
            lab.store.get(n_a_h_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let n_a_k = lab
            ...
            .text()
            .to_string();
        assert!(n_a_k.contains("constant  NAk  node "), "{n_a_k}");
        assert!(
            n_a_k.contains(
                "hash     28c95a46c67bec666b887658cc44664000bf821eac09b9023cf401b89231efc3"
            ),
            "{n_a_k}"
        );
        assert!(n_a_k.contains("kind     ratio"), "{n_a_k}");
        assert!(n_a_k.contains("table    XXXI"), "{n_a_k}");
        assert!(
            n_a_k.contains("range    NAk = 8.314462618 exact"),
            "{n_a_k}"
        );
        assert!(n_a_k.contains("unit     J mol^{-1} K^{-1}"), "{n_a_k}");
        assert!(
            n_a_k.contains("value    207861565453831/25000000000000"),
            "{n_a_k}"
        );
        assert!(n_a_k.contains("rebuild  ok"), "{n_a_k}");
        assert!(n_a_k.contains("not P3N"), "{n_a_k}");
        assert!(!n_a_k.contains("receipt"), "{n_a_k}");
        assert!(!n_a_k.contains("theorem"), "{n_a_k}");
        let n_a_k_id = constant_node_id(&n_a_k);
        assert_eq!(
            n_a_k_id.to_hex(),
            "ebdfd5d64f47a7b38faa4e4907ac5de394242e039670478b1b8558ffe1a8a853",
            "journaling must not change the NAk constant payload"
        );
        assert_eq!(
            lab.store.get(n_a_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let k_ev = lab
            ...
            .text()
            .to_string();
        assert!(k_ev.contains("constant  k_eV  node "), "{k_ev}");
        assert!(
            k_ev.contains(
                "hash     6af2dc4a70fb23c2c85ff1537e3b6c4c32068d11cbe0a9abca6d651f5cdceed6"
            ),
            "{k_ev}"
        );
        assert!(k_ev.contains("kind     ratio"), "{k_ev}");
        assert!(k_ev.contains("table    XXXI"), "{k_ev}");
        assert!(
            k_ev.contains("range    k_eV = 8.617333262e-5 exact"),
            "{k_ev}"
        );
        assert!(k_ev.contains("unit     eV K^{-1}"), "{k_ev}");
        assert!(k_ev.contains("value    1380649/16021766340"), "{k_ev}");
        assert!(k_ev.contains("rebuild  ok"), "{k_ev}");
        assert!(k_ev.contains("not P3N"), "{k_ev}");
        assert!(!k_ev.contains("receipt"), "{k_ev}");
        assert!(!k_ev.contains("theorem"), "{k_ev}");
        let k_ev_id = constant_node_id(&k_ev);
        assert_eq!(
            k_ev_id.to_hex(),
            "5c6b4f0b437c59c5d406f7bccad046a88f962664f7bceb604cadc09f91d3dcea",
            "journaling must not change the k_eV constant payload"
        );
        assert_eq!(
            lab.store.get(k_ev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let k_hz = lab
            ...
            .text()
            .to_string();
        assert!(k_hz.contains("constant  k_Hz  node "), "{k_hz}");
        assert!(
            k_hz.contains(
                "hash     4e53cf9938c70b39d13f107dc2c90be1486148fd1ebb585505e2e3b8637582bc"
            ),
            "{k_hz}"
        );
        assert!(k_hz.contains("kind     ratio"), "{k_hz}");
        assert!(k_hz.contains("table    XXXI"), "{k_hz}");
        assert!(
            k_hz.contains("range    k_Hz = 2.083661912e10 exact"),
            "{k_hz}"
        );
        assert!(k_hz.contains("unit     Hz K^{-1}"), "{k_hz}");
        assert!(
            k_hz.contains("value    2761298000000000000/132521403"),
            "{k_hz}"
        );
        assert!(k_hz.contains("rebuild  ok"), "{k_hz}");
        assert!(k_hz.contains("not P3N"), "{k_hz}");
        assert!(!k_hz.contains("receipt"), "{k_hz}");
        assert!(!k_hz.contains("theorem"), "{k_hz}");
        let k_hz_id = constant_node_id(&k_hz);
        assert_eq!(
            k_hz_id.to_hex(),
            "f77db081575132d0b379eddeba0f9e4642c54221ffbdcfa29d6147bb30d917c4",
            "journaling must not change the k_Hz constant payload"
        );
        assert_eq!(
            lab.store.get(k_hz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let k_m = lab
            ...
            .text()
            .to_string();
        assert!(k_m.contains("constant  k_m  node "), "{k_m}");
        assert!(
            k_m.contains(
                "hash     533849bdd6300f5e1e48545708d539f94017879558130c41b62dbe7f7742b501"
            ),
            "{k_m}"
        );
        assert!(k_m.contains("kind     ratio"), "{k_m}");
        assert!(k_m.contains("table    XXXI"), "{k_m}");
        assert!(k_m.contains("range    k_m = 69.50348004 exact"), "{k_m}");
        assert!(k_m.contains("unit     m^{-1} K^{-1}"), "{k_m}");
        assert!(
            k_m.contains("value    18913000000000000/272115870842319"),
            "{k_m}"
        );
        assert!(k_m.contains("rebuild  ok"), "{k_m}");
        assert!(k_m.contains("not P3N"), "{k_m}");
        assert!(!k_m.contains("receipt"), "{k_m}");
        assert!(!k_m.contains("theorem"), "{k_m}");
        let k_m_id = constant_node_id(&k_m);
        assert_eq!(
            k_m_id.to_hex(),
            "0af89015e9d3fa2b164a62f44ac054af48855f03338c51a424abbf9a126e0e93",
            "journaling must not change the k_m constant payload"
        );
        assert_eq!(
            lab.store.get(k_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let kg_j = lab
            ...
            .text()
            .to_string();
        assert!(kg_j.contains("constant  kg_J  node "), "{kg_j}");
        assert!(
            kg_j.contains(
                "hash     a54eee6c8f3046f2c68745c29a8040b9b486fd013f72511e1d7372366a34bc7f"
            ),
            "{kg_j}"
        );
        assert!(kg_j.contains("kind     ratio"), "{kg_j}");
        assert!(kg_j.contains("table    XXXV"), "{kg_j}");
        assert!(
            kg_j.contains("range    kg_J = 8.987551787e16 exact"),
            "{kg_j}"
        );
        assert!(kg_j.contains("unit     J"), "{kg_j}");
        assert!(kg_j.contains("value    89875517873681764"), "{kg_j}");
        assert!(kg_j.contains("rebuild  ok"), "{kg_j}");
        assert!(kg_j.contains("not P3N"), "{kg_j}");
        assert!(!kg_j.contains("receipt"), "{kg_j}");
        assert!(!kg_j.contains("theorem"), "{kg_j}");
        let kg_j_id = constant_node_id(&kg_j);
        assert_eq!(
            kg_j_id.to_hex(),
            "c926a11668840ba7f9e461d93be0fed6b3cebea1ee13171ca1675e174bcf58ca",
            "journaling must not change the kg_J constant payload"
        );
        assert_eq!(
            lab.store.get(kg_j_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_j = lab
            ...
            .text()
            .to_string();
        assert!(m_j.contains("constant  m_J  node "), "{m_j}");
        assert!(
            m_j.contains(
                "hash     1b1ae8c0a216320aad8dd8ac91944989de7278e7d89575e0824bcf77e764deeb"
            ),
            "{m_j}"
        );
        assert!(m_j.contains("kind     sci-exact"), "{m_j}");
        assert!(m_j.contains("table    XXXV"), "{m_j}");
        assert!(
            m_j.contains("range    m_J = 1.986445857e-25 exact"),
            "{m_j}"
        );
        assert!(m_j.contains("unit     J"), "{m_j}");
        assert!(m_j.contains("value    19864458571489287e-41"), "{m_j}");
        assert!(m_j.contains("rebuild  ok"), "{m_j}");
        assert!(m_j.contains("not P3N"), "{m_j}");
        assert!(!m_j.contains("receipt"), "{m_j}");
        assert!(!m_j.contains("theorem"), "{m_j}");
        let m_j_id = constant_node_id(&m_j);
        assert_eq!(
            m_j_id.to_hex(),
            "8a296c0621b84a7c7bbc3ad279afa1cb8c353b843806863981fed43ad1e1ffc3",
            "journaling must not change the m_J constant payload"
        );
        assert_eq!(
            lab.store.get(m_j_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_ev = lab
            ...
            .text()
            .to_string();
        assert!(m_ev.contains("constant  m_eV  node "), "{m_ev}");
        assert!(
            m_ev.contains(
                "hash     12c1ae591caa23af86b67134aea0b013f49ede015195013efc865ec6b1340dff"
            ),
            "{m_ev}"
        );
        assert!(m_ev.contains("kind     ratio"), "{m_ev}");
        assert!(m_ev.contains("table    XXXV"), "{m_ev}");
        assert!(
            m_ev.contains("range    m_eV = 1.239841984e-6 exact"),
            "{m_ev}"
        );
        assert!(m_ev.contains("unit     eV"), "{m_ev}");
        assert!(
            m_ev.contains("value    6621486190496429/5340588780000000000000"),
            "{m_ev}"
        );
        assert!(m_ev.contains("rebuild  ok"), "{m_ev}");
        assert!(m_ev.contains("not P3N"), "{m_ev}");
        assert!(!m_ev.contains("receipt"), "{m_ev}");
        assert!(!m_ev.contains("theorem"), "{m_ev}");
        let m_ev_id = constant_node_id(&m_ev);
        assert_eq!(
            m_ev_id.to_hex(),
            "57c79435bf8516f4b2e547b8b221b5dd003f18f701b6d01c44268682fa010d2d",
            "journaling must not change the m_eV constant payload"
        );
        assert_eq!(
            lab.store.get(m_ev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let j_ev = lab
            ...
            .text()
            .to_string();
        assert!(j_ev.contains("constant  J_eV  node "), "{j_ev}");
        assert!(
            j_ev.contains(
                "hash     b775a4c8372acd2d0ba110b108d3971cca85e2fa26a340d9a771522eefdd23e6"
            ),
            "{j_ev}"
        );
        assert!(j_ev.contains("kind     ratio"), "{j_ev}");
        assert!(j_ev.contains("table    XXXV"), "{j_ev}");
        assert!(
            j_ev.contains("range    J_eV = 6.241509074e18 exact"),
            "{j_ev}"
        );
        assert!(j_ev.contains("unit     eV"), "{j_ev}");
        assert!(
            j_ev.contains("value    5000000000000000000000000000/801088317"),
            "{j_ev}"
        );
        assert!(j_ev.contains("rebuild  ok"), "{j_ev}");
        assert!(j_ev.contains("not P3N"), "{j_ev}");
        assert!(!j_ev.contains("receipt"), "{j_ev}");
        assert!(!j_ev.contains("theorem"), "{j_ev}");
        let j_ev_id = constant_node_id(&j_ev);
        assert_eq!(
            j_ev_id.to_hex(),
            "e140741974a7632531ce17777058213576c61bca740b33f5ceb671c5d7924260",
            "journaling must not change the J_eV constant payload"
        );
        assert_eq!(
            lab.store.get(j_ev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let j_k = lab
            ...
            .text()
            .to_string();
        assert!(j_k.contains("constant  J_K  node "), "{j_k}");
        assert!(
            j_k.contains(
                "hash     294b1e620f7a8cacd4c276a74a0f43eba7e28c5969d83a1983a9df046dbb5f26"
            ),
            "{j_k}"
        );
        assert!(j_k.contains("kind     ratio"), "{j_k}");
        assert!(j_k.contains("table    XXXV"), "{j_k}");
        assert!(j_k.contains("range    J_K = 7.242970516e22 exact"), "{j_k}");
        assert!(j_k.contains("unit     K"), "{j_k}");
        assert!(
            j_k.contains("value    100000000000000000000000000000/1380649"),
            "{j_k}"
        );
        assert!(j_k.contains("rebuild  ok"), "{j_k}");
        assert!(j_k.contains("not P3N"), "{j_k}");
        assert!(!j_k.contains("receipt"), "{j_k}");
        assert!(!j_k.contains("theorem"), "{j_k}");
        let j_k_id = constant_node_id(&j_k);
        assert_eq!(
            j_k_id.to_hex(),
            "f830b9ca4ce8383d69a8818628c66a968b039f3f30aa7a22d591a70f629031a4",
            "journaling must not change the J_K constant payload"
        );
        assert_eq!(
            lab.store.get(j_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let ev_hz = lab
            .exec(Command::Constant {
                name: Some("eV_Hz".into()),
            })
            .text()
            .to_string();
        assert!(ev_hz.contains("constant  eV_Hz  node "), "{ev_hz}");
        assert!(
            ev_hz.contains(
                "hash     d323fa85e441848900b49787ac5c058ae78c2bb59d3f46627a5c5ba68ef339d5"
            ),
            "{ev_hz}"
        );
        assert!(ev_hz.contains("kind     ratio"), "{ev_hz}");
        assert!(ev_hz.contains("table    XXXV"), "{ev_hz}");
        assert!(
            ev_hz.contains("range    eV_Hz = 2.417989242e14 exact"),
            "{ev_hz}"
        );
        assert!(ev_hz.contains("unit     Hz"), "{ev_hz}");
        assert!(
            ev_hz.contains("value    10681177560000000000000/44173801"),
            "{ev_hz}"
        );
        assert!(ev_hz.contains("rebuild  ok"), "{ev_hz}");
        assert!(ev_hz.contains("not P3N"), "{ev_hz}");
        assert!(!ev_hz.contains("receipt"), "{ev_hz}");
        assert!(!ev_hz.contains("theorem"), "{ev_hz}");
        let ev_hz_id = constant_node_id(&ev_hz);
        assert_eq!(
            ev_hz_id.to_hex(),
            "d98001e49653a95a3bb1543e45fbb993cdecdfc79d3303e36cd0738fec4b09c6",
            "journaling must not change the eV_Hz constant payload"
        );
        assert_eq!(
            lab.store.get(ev_hz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let ev_k = lab
            ...
            .text()
            .to_string();
        assert!(ev_k.contains("constant  eV_K  node "), "{ev_k}");
        assert!(
            ev_k.contains(
                "hash     a2763a30976f052db834b3260d5399ef0553afaf42a848f22a174c70f0fbdad0"
            ),
            "{ev_k}"
        );
        assert!(ev_k.contains("kind     ratio"), "{ev_k}");
        assert!(ev_k.contains("table    XXXV"), "{ev_k}");
        assert!(
            ev_k.contains("range    eV_K = 1.160451812e4 exact"),
            "{ev_k}"
        );
        assert!(ev_k.contains("unit     K"), "{ev_k}");
        assert!(ev_k.contains("value    16021766340/1380649"), "{ev_k}");
        assert!(ev_k.contains("rebuild  ok"), "{ev_k}");
        assert!(ev_k.contains("not P3N"), "{ev_k}");
        assert!(!ev_k.contains("receipt"), "{ev_k}");
        assert!(!ev_k.contains("theorem"), "{ev_k}");
        let ev_k_id = constant_node_id(&ev_k);
        assert_eq!(
            ev_k_id.to_hex(),
            "e5f5954239201da5cb06da674987a5a1aae6ee343db21317a92a75be82b5424f",
            "journaling must not change the eV_K constant payload"
        );
        assert_eq!(
            lab.store.get(ev_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let hz_m = lab
            ...
            .text()
            .to_string();
        assert!(hz_m.contains("constant  Hz_m  node "), "{hz_m}");
        assert!(
            hz_m.contains(
                "hash     e41772022e94f6c2f45b5a728b61f8258bfac9269c643b84e39d530798bf8421"
            ),
            "{hz_m}"
        );
        assert!(hz_m.contains("kind     ratio"), "{hz_m}");
        assert!(hz_m.contains("table    XXXV"), "{hz_m}");
        assert!(
            hz_m.contains("range    Hz_m = 3.335640951e-9 exact"),
            "{hz_m}"
        );
        assert!(hz_m.contains("unit     m^{-1}"), "{hz_m}");
        assert!(hz_m.contains("value    1/299792458"), "{hz_m}");
        assert!(hz_m.contains("rebuild  ok"), "{hz_m}");
        assert!(hz_m.contains("not P3N"), "{hz_m}");
        assert!(!hz_m.contains("receipt"), "{hz_m}");
        assert!(!hz_m.contains("theorem"), "{hz_m}");
        let hz_m_id = constant_node_id(&hz_m);
        assert_eq!(
            hz_m_id.to_hex(),
            "846bab463c0a51c951864c1258a50764889007111b9f543013c1f060ab63428e",
            "journaling must not change the Hz_m constant payload"
        );
        assert_eq!(
            lab.store.get(hz_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let hz_eh = lab
            .exec(Command::Constant {
                name: Some("Hz_Eh".into()),
            })
            .text()
            .to_string();
        assert!(hz_eh.contains("constant  Hz_Eh  node "), "{hz_eh}");
        assert!(
            hz_eh.contains(
                "hash     1b7fb5c9bc08aea3d58daae4cbb5bb4d59fec7d1ffddea3db40acea388b29473"
            ),
            "{hz_eh}"
        );
        assert!(hz_eh.contains("kind     interval"), "{hz_eh}");
        assert!(hz_eh.contains("table    XXXV"), "{hz_eh}");
        assert!(
            hz_eh.contains("range    Hz_Eh = 1.5198298460570(29)e-16"),
            "{hz_eh}"
        );
        assert!(hz_eh.contains("unit     E_h"), "{hz_eh}");
        assert!(hz_eh.contains("value    [15198298460541/100000000000000000000000000000, 15198298460599/100000000000000000000000000000]"), "{hz_eh}");
        assert!(hz_eh.contains("rebuild  ok"), "{hz_eh}");
        assert!(hz_eh.contains("not P3N"), "{hz_eh}");
        assert!(!hz_eh.contains("receipt"), "{hz_eh}");
        assert!(!hz_eh.contains("theorem"), "{hz_eh}");
        let hz_eh_id = constant_node_id(&hz_eh);
        assert_eq!(
            hz_eh_id.to_hex(),
            "f6ef614b65ab4700b144c8e49be1f26e3706003f3972cee939983d67a5ceade7",
            "journaling must not change the Hz_Eh constant payload"
        );
        assert_eq!(
            lab.store.get(hz_eh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let hz_u = lab
            ...
            .text()
            .to_string();
        assert!(hz_u.contains("constant  Hz_u  node "), "{hz_u}");
        assert!(
            hz_u.contains(
        ...
        assert!(hz_u.contains("kind     interval"), "{hz_u}");
        assert!(hz_u.contains("table    XXXV"), "{hz_u}");
        assert!(
            hz_u.contains("range    Hz_u = 4.4398216652(13)e-24"),
            "{hz_u}"
        );
        assert!(hz_u.contains("unit     u"), "{hz_u}");
        assert!(hz_u.contains("value    [44398216639/10000000000000000000000000000000000, 8879643333/2000000000000000000000000000000000]"), "{hz_u}");
        assert!(hz_u.contains("rebuild  ok"), "{hz_u}");
        assert!(hz_u.contains("not P3N"), "{hz_u}");
        assert!(!hz_u.contains("receipt"), "{hz_u}");
        assert!(!hz_u.contains("theorem"), "{hz_u}");
        let hz_u_id = constant_node_id(&hz_u);
        assert_eq!(
            hz_u_id.to_hex(),
            "54e0629cd6e073f6b5ad822d4120afa624384b2a228a0b46657c5a0e59c52f26",
            "journaling must not change the Hz_u constant payload"
        );
        assert_eq!(
            lab.store.get(hz_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let u_hz = lab
            ...
            .text()
            .to_string();
        assert!(u_hz.contains("constant  u_Hz  node "), "{u_hz}");
        assert!(
            u_hz.contains(
        ...
        assert!(u_hz.contains("kind     interval"), "{u_hz}");
        assert!(u_hz.contains("table    XXXV"), "{u_hz}");
        assert!(
            u_hz.contains("range    u_Hz = 2.25234271871(68)e23"),
            "{u_hz}"
        );
        assert!(u_hz.contains("unit     Hz"), "{u_hz}");
        assert!(
            u_hz.contains("value    [225234271803000000000000, 225234271939000000000000]"),
            "{u_hz}"
        );
        assert!(u_hz.contains("rebuild  ok"), "{u_hz}");
        assert!(u_hz.contains("not P3N"), "{u_hz}");
        assert!(!u_hz.contains("receipt"), "{u_hz}");
        assert!(!u_hz.contains("theorem"), "{u_hz}");
        let u_hz_id = constant_node_id(&u_hz);
        assert_eq!(
            u_hz_id.to_hex(),
            "42e50e73ed21eb10bb761a7ab85ea119d621797520e8138e69d473243c8667b7",
            "journaling must not change the u_Hz constant payload"
        );
        assert_eq!(
            lab.store.get(u_hz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let eh_hz = lab
            .exec(Command::Constant {
                name: Some("Eh_Hz".into()),
            })
            .text()
            .to_string();
        assert!(eh_hz.contains("constant  Eh_Hz  node "), "{eh_hz}");
        assert!(
            eh_hz.contains(
                "hash     9d3d98fd812ac48876d0130c20977f62d5ba838f61c6604868ad6ff050e8358a"
            ),
            "{eh_hz}"
        );
        assert!(eh_hz.contains("kind     interval"), "{eh_hz}");
        assert!(eh_hz.contains("table    XXXV"), "{eh_hz}");
        assert!(
            eh_hz.contains("range    Eh_Hz = 6.579683920502(13)e15"),
            "{eh_hz}"
        );
        assert!(eh_hz.contains("unit     Hz"), "{eh_hz}");
        assert!(
            eh_hz.contains("value    [6579683920489000, 6579683920515000]"),
            "{eh_hz}"
        );
        assert!(eh_hz.contains("rebuild  ok"), "{eh_hz}");
        assert!(eh_hz.contains("not P3N"), "{eh_hz}");
        assert!(!eh_hz.contains("receipt"), "{eh_hz}");
        assert!(!eh_hz.contains("theorem"), "{eh_hz}");
        let eh_hz_id = constant_node_id(&eh_hz);
        assert_eq!(
            eh_hz_id.to_hex(),
            "1e4278900e9f26858816ca32a4ba47cb573ac63172ecd5585a92c455ed1fce70",
            "journaling must not change the Eh_Hz constant payload"
        );
        assert_eq!(
            lab.store.get(eh_hz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let eh_m = lab
            ...
            .text()
            .to_string();
        assert!(eh_m.contains("constant  Eh_m  node "), "{eh_m}");
        assert!(
            eh_m.contains(
                "hash     9696fa523650b61199c2590965df2a7d36be3f0e319f17e3457b126192fb9796"
            ),
            "{eh_m}"
        );
        assert!(eh_m.contains("kind     interval"), "{eh_m}");
        assert!(eh_m.contains("table    XXXV"), "{eh_m}");
        assert!(
            eh_m.contains("range    Eh_m = 2.1947463136320(43)e7"),
            "{eh_m}"
        );
        assert!(eh_m.contains("unit     m^{-1}"), "{eh_m}");
        assert!(
            eh_m.contains("value    [21947463136277/1000000, 21947463136363/1000000]"),
            "{eh_m}"
        );
        assert!(eh_m.contains("rebuild  ok"), "{eh_m}");
        assert!(eh_m.contains("not P3N"), "{eh_m}");
        assert!(!eh_m.contains("receipt"), "{eh_m}");
        assert!(!eh_m.contains("theorem"), "{eh_m}");
        let eh_m_id = constant_node_id(&eh_m);
        assert_eq!(
            eh_m_id.to_hex(),
            "04a36e4495af1759d633be2c96749e3dcbcc6583b6cad1107128eea4746a56c1",
            "journaling must not change the Eh_m constant payload"
        );
        assert_eq!(
            lab.store.get(eh_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let m_eh = lab
            ...
            .text()
            .to_string();
        assert!(m_eh.contains("constant  m_Eh  node "), "{m_eh}");
        assert!(
            m_eh.contains(
                "hash     ec9127261ac8d38bd117a7b563580e40cd7d6e6a893da1d89862739aee98861c"
            ),
            "{m_eh}"
        );
        assert!(m_eh.contains("kind     interval"), "{m_eh}");
        assert!(m_eh.contains("table    XXXV"), "{m_eh}");
        assert!(
            m_eh.contains("range    m_Eh = 4.5563352529120(88)e-8"),
            "{m_eh}"
        );
        assert!(m_eh.contains("unit     E_h"), "{m_eh}");
        assert!(
            m_eh.contains(
                "value    [5695419066129/125000000000000000000, 5695419066151/125000000000000000000]"
            ),
            "{m_eh}"
        );
        assert!(m_eh.contains("rebuild  ok"), "{m_eh}");
        assert!(m_eh.contains("not P3N"), "{m_eh}");
        assert!(!m_eh.contains("receipt"), "{m_eh}");
        assert!(!m_eh.contains("theorem"), "{m_eh}");
        let m_eh_id = constant_node_id(&m_eh);
        assert_eq!(
            m_eh_id.to_hex(),
            "93026de8e32f434e1e60293935443a6cd2f35a77c74adaaf1accd2de43bdf6f3",
            "journaling must not change the m_Eh constant payload"
        );
        assert_eq!(
            lab.store.get(m_eh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let eh_k = lab
            ...
            .text()
            .to_string();
        assert!(eh_k.contains("constant  Eh_K  node "), "{eh_k}");
        assert!(
            eh_k.contains(
                "hash     6f0a8bf07131fca3dec89383496f280b6c1b93086c049e3466e8653c3cc45430"
            ),
            "{eh_k}"
        );
        assert!(eh_k.contains("kind     interval"), "{eh_k}");
        assert!(eh_k.contains("table    XXXV"), "{eh_k}");
        assert!(
            eh_k.contains("range    Eh_K = 3.1577502480407(61)e5"),
            "{eh_k}"
        );
        assert!(eh_k.contains("unit     K"), "{eh_k}");
        assert!(
            eh_k.contains("value    [15788751240173/50000000, 7894375620117/25000000]"),
            "{eh_k}"
        );
        assert!(eh_k.contains("rebuild  ok"), "{eh_k}");
        assert!(eh_k.contains("not P3N"), "{eh_k}");
        assert!(!eh_k.contains("receipt"), "{eh_k}");
        assert!(!eh_k.contains("theorem"), "{eh_k}");
        let eh_k_id = constant_node_id(&eh_k);
        assert_eq!(
            eh_k_id.to_hex(),
            "2dae2b24d50cbbb0e7f5e5d3afac7b5ca4dd1eddb828146c9f245ec4052fa147",
            "journaling must not change the Eh_K constant payload"
        );
        assert_eq!(
            lab.store.get(eh_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let eh_u = lab
            ...
            .text()
            .to_string();
        assert!(eh_u.contains("constant  Eh_u  node "), "{eh_u}");
        assert!(
            eh_u.contains(
                "hash     a3c699716e28cb116c1c86af4014cea3c65677566d268105e86fb13db75a1047"
            ),
            "{eh_u}"
        );
        assert!(eh_u.contains("kind     interval"), "{eh_u}");
        assert!(eh_u.contains("table    XXXV"), "{eh_u}");
        assert!(
            eh_u.contains("range    Eh_u = 2.92126232205(88)e-8"),
            "{eh_u}"
        );
        assert!(eh_u.contains("unit     u"), "{eh_u}");
        assert!(
            eh_u.contains(
                "value    [292126232117/10000000000000000000, 292126232293/10000000000000000000]"
            ),
            "{eh_u}"
        );
        assert!(eh_u.contains("rebuild  ok"), "{eh_u}");
        assert!(eh_u.contains("not P3N"), "{eh_u}");
        assert!(!eh_u.contains("receipt"), "{eh_u}");
        assert!(!eh_u.contains("theorem"), "{eh_u}");
        let eh_u_id = constant_node_id(&eh_u);
        assert_eq!(
            eh_u_id.to_hex(),
            "2c8618557602554a8d101ef34a276b06bcc981cce6ba57d6d93f01de95d3f8a1",
            "journaling must not change the Eh_u constant payload"
        );
        assert_eq!(
            lab.store.get(eh_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let ev_eh = lab
            .exec(Command::Constant {
                name: Some("eV_Eh".into()),
            })
            .text()
            .to_string();
        assert!(ev_eh.contains("constant  eV_Eh  node "), "{ev_eh}");
        assert!(
            ev_eh.contains(
                "hash     8c15bdef7dbec61c106d7df00c024ac4aad0ff46fb280a6e950f11248024a201"
            ),
            "{ev_eh}"
        );
        assert!(ev_eh.contains("kind     interval"), "{ev_eh}");
        assert!(ev_eh.contains("table    XXXV"), "{ev_eh}");
        assert!(
            ev_eh.contains("range    eV_Eh = 3.6749322175655(71)e-2"),
            "{ev_eh}"
        );
        assert!(ev_eh.contains("unit     E_h"), "{ev_eh}");
        assert!(
            ev_eh.contains(
                "value    [1148416317987/31250000000000, 18374661087863/500000000000000]"
            ),
            "{ev_eh}"
        );
        assert!(ev_eh.contains("rebuild  ok"), "{ev_eh}");
        assert!(ev_eh.contains("not P3N"), "{ev_eh}");
        assert!(!ev_eh.contains("receipt"), "{ev_eh}");
        assert!(!ev_eh.contains("theorem"), "{ev_eh}");
        let ev_eh_id = constant_node_id(&ev_eh);
        assert_eq!(
            ev_eh_id.to_hex(),
            "a9e5d2c994add3d47fe7ad539d3417cebf9aa69aee2b2b10154d04190c80172e",
            "journaling must not change the eV_Eh constant payload"
        );
        assert_eq!(
            lab.store.get(ev_eh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let j_eh = lab
            ...
            .text()
            .to_string();
        assert!(j_eh.contains("constant  J_Eh  node "), "{j_eh}");
        assert!(
            j_eh.contains(
                "hash     f922a6b2268fef1d8e0a56c71cfcdd7daee0359c8a1a3ad7e728d05b8e424ddd"
            ),
            "{j_eh}"
        );
        assert!(j_eh.contains("kind     interval"), "{j_eh}");
        assert!(j_eh.contains("table    XXXV"), "{j_eh}");
        assert!(
            j_eh.contains("range    J_Eh = 2.2937122783963(45)e17"),
            "{j_eh}"
        );
        assert!(j_eh.contains("unit     E_h"), "{j_eh}");
        assert!(
            j_eh.contains("value    [229371227839180000, 229371227840080000]"),
            "{j_eh}"
        );
        assert!(j_eh.contains("rebuild  ok"), "{j_eh}");
        assert!(j_eh.contains("not P3N"), "{j_eh}");
        assert!(!j_eh.contains("receipt"), "{j_eh}");
        assert!(!j_eh.contains("theorem"), "{j_eh}");
        let j_eh_id = constant_node_id(&j_eh);
        assert_eq!(
            j_eh_id.to_hex(),
            "897c2126a3ef852a26f86bd5d39162b2d1ea47bb5875232e6ec15909626458b2",
            "journaling must not change the J_Eh constant payload"
        );
        assert_eq!(
            lab.store.get(j_eh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let u_eh = lab
            ...
            .text()
            .to_string();
        assert!(u_eh.contains("constant  u_Eh  node "), "{u_eh}");
        assert!(
            u_eh.contains(
                "hash     15c1d5e49dca9b631334cf8c46bd90a65e8ff6658fb7a64ba43a5a37862c60a1"
            ),
            "{u_eh}"
        );
        assert!(u_eh.contains("kind     interval"), "{u_eh}");
        assert!(u_eh.contains("table    XXXV"), "{u_eh}");
        assert!(
            u_eh.contains("range    u_Eh = 3.4231776874(10)e7"),
            "{u_eh}"
        );
        assert!(u_eh.contains("unit     E_h"), "{u_eh}");
        assert!(
            u_eh.contains("value    [4278972108/125, 8557944221/250]"),
            "{u_eh}"
        );
        assert!(u_eh.contains("rebuild  ok"), "{u_eh}");
        assert!(u_eh.contains("not P3N"), "{u_eh}");
        assert!(!u_eh.contains("receipt"), "{u_eh}");
        assert!(!u_eh.contains("theorem"), "{u_eh}");
        let u_eh_id = constant_node_id(&u_eh);
        assert_eq!(
            u_eh_id.to_hex(),
            "965c12163e67c74d6c8665e15dbef38db6ab88fc42720a075786c7d8af3f6726",
            "journaling must not change the u_Eh constant payload"
        );
        assert_eq!(
            lab.store.get(u_eh_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let kg_u = lab
            ...
            .text()
            .to_string();
        assert!(kg_u.contains("constant  kg_u  node "), "{kg_u}");
        assert!(
            kg_u.contains(
                "hash     8d1f4f91a93aa7b56e3581d371e08e8eb0377c6e892fb24799ab341297d88bb5"
            ),
            "{kg_u}"
        );
        assert!(kg_u.contains("kind     interval"), "{kg_u}");
        assert!(kg_u.contains("table    XXXV"), "{kg_u}");
        assert!(
            kg_u.contains("range    kg_u = 6.0221407621(18)e26"),
            "{kg_u}"
        );
        assert!(kg_u.contains("unit     u"), "{kg_u}");
        assert!(
            kg_u.contains("value    [602214076030000000000000000, 602214076390000000000000000]"),
            "{kg_u}"
        );
        assert!(kg_u.contains("rebuild  ok"), "{kg_u}");
        assert!(kg_u.contains("not P3N"), "{kg_u}");
        assert!(!kg_u.contains("receipt"), "{kg_u}");
        assert!(!kg_u.contains("theorem"), "{kg_u}");
        let kg_u_id = constant_node_id(&kg_u);
        assert_eq!(
            kg_u_id.to_hex(),
            "512d569359b032f207037b2cf2727128ced6dfa813c61a56ca0c59e4db139dff",
            "journaling must not change the kg_u constant payload"
        );
        assert_eq!(
            lab.store.get(kg_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let j_u = lab
            ...
            .text()
            .to_string();
        assert!(j_u.contains("constant  J_u  node "), "{j_u}");
        assert!(
            j_u.contains(
                "hash     b2f6eb6e00f483e99217fb38006ce9d8339ad91f1d6d6d9e0e38c35b03af4d40"
            ),
            "{j_u}"
        );
        assert!(j_u.contains("kind     interval"), "{j_u}");
        assert!(j_u.contains("table    XXXV"), "{j_u}");
        assert!(j_u.contains("range    J_u = 6.7005352565(20)e9"), "{j_u}");
        assert!(j_u.contains("unit     u"), "{j_u}");
        assert!(
            j_u.contains("value    [13401070509/2, 13401070517/2]"),
            "{j_u}"
        );
        assert!(j_u.contains("rebuild  ok"), "{j_u}");
        assert!(j_u.contains("not P3N"), "{j_u}");
        assert!(!j_u.contains("receipt"), "{j_u}");
        assert!(!j_u.contains("theorem"), "{j_u}");
        let j_u_id = constant_node_id(&j_u);
        assert_eq!(
            j_u_id.to_hex(),
            "c52249bcac19ccd303fbe29b8355da38e3ad5a2aef6bc179978f5d8d83563887",
            "journaling must not change the J_u constant payload"
        );
        assert_eq!(
            lab.store.get(j_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let k_u = lab
            ...
            .text()
            .to_string();
        assert!(k_u.contains("constant  K_u  node "), "{k_u}");
        assert!(
            k_u.contains(
                "hash     92a82ff43fedced786dd0efca6fa5cddc57a33ca3cb083217ac936fcd2b72c5e"
            ),
            "{k_u}"
        );
        assert!(k_u.contains("kind     interval"), "{k_u}");
        assert!(k_u.contains("table    XXXV"), "{k_u}");
        assert!(k_u.contains("range    K_u = 9.2510873014(28)e-14"), "{k_u}");
        assert!(k_u.contains("unit     u"), "{k_u}");
        assert!(
            k_u.contains("value    [46255436493/500000000000000000000000, 46255436521/500000000000000000000000]"),
            "{k_u}"
        );
        assert!(k_u.contains("rebuild  ok"), "{k_u}");
        assert!(k_u.contains("not P3N"), "{k_u}");
        assert!(!k_u.contains("receipt"), "{k_u}");
        assert!(!k_u.contains("theorem"), "{k_u}");
        let k_u_id = constant_node_id(&k_u);
        assert_eq!(
            k_u_id.to_hex(),
            "6f2e5d8122826c006e5d64183b9035261fa51e1e6508ff2887a9e5348c33039b",
            "journaling must not change the K_u constant payload"
        );
        assert_eq!(
            lab.store.get(k_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let ev_u = lab
            ...
            .text()
            .to_string();
        assert!(ev_u.contains("constant  eV_u  node "), "{ev_u}");
        assert!(
            ev_u.contains(
                "hash     39dba467459e1b7d4e44cd12512342f420c7f5d8b79b0f975af03b6d361225c6"
            ),
            "{ev_u}"
        );
        assert!(ev_u.contains("kind     interval"), "{ev_u}");
        assert!(ev_u.contains("table    XXXV"), "{ev_u}");
        assert!(
            ev_u.contains("range    eV_u = 1.07354410233(32)e-9"),
            "{ev_u}"
        );
        assert!(ev_u.contains("unit     u"), "{ev_u}");
        assert!(
            ev_u.contains(
                "value    [107354410201/100000000000000000000, 21470882053/20000000000000000000]"
            ),
            "{ev_u}"
        );
        assert!(ev_u.contains("rebuild  ok"), "{ev_u}");
        assert!(ev_u.contains("not P3N"), "{ev_u}");
        assert!(!ev_u.contains("receipt"), "{ev_u}");
        assert!(!ev_u.contains("theorem"), "{ev_u}");
        let ev_u_id = constant_node_id(&ev_u);
        assert_eq!(
            ev_u_id.to_hex(),
            "af9b87a44fa433ffb7b314c4c4aeb3f2736ad27f928d258da05e338fca431b91",
            "journaling must not change the eV_u constant payload"
        );
        assert_eq!(
            lab.store.get(ev_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let u_m = lab
            ...
            .text()
            .to_string();
        assert!(u_m.contains("constant  u_m  node "), "{u_m}");
        assert!(
            u_m.contains(
                "hash     3380878aa06e31c17144288a338b9b4614e21389fa72a9d2c16e1444c6b07035"
            ),
            "{u_m}"
        );
        assert!(u_m.contains("kind     interval"), "{u_m}");
        assert!(u_m.contains("table    XXXV"), "{u_m}");
        assert!(u_m.contains("range    u_m = 7.5130066104(23)e14"), "{u_m}");
        assert!(u_m.contains("unit     m^{-1}"), "{u_m}");
        assert!(
            u_m.contains("value    [751300660810000, 751300661270000]"),
            "{u_m}"
        );
        assert!(u_m.contains("rebuild  ok"), "{u_m}");
        assert!(u_m.contains("not P3N"), "{u_m}");
        assert!(!u_m.contains("receipt"), "{u_m}");
        assert!(!u_m.contains("theorem"), "{u_m}");
        let u_m_id = constant_node_id(&u_m);
        assert_eq!(
            u_m_id.to_hex(),
            "acc94a83eafd38f422f07aa2387373cdfc0b98c33890b5bdce317ff4fe7c8651",
            "journaling must not change the u_m constant payload"
        );
        assert_eq!(
            lab.store.get(u_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let u_k = lab
            ...
            .text()
            .to_string();
        assert!(u_k.contains("constant  u_K  node "), "{u_k}");
        assert!(
            u_k.contains(
                "hash     eceb5956e7b5b435a32c4d8b9b4a8f97cd96a597334919a667d804f13e1a495b"
            ),
            "{u_k}"
        );
        assert!(u_k.contains("kind     interval"), "{u_k}");
        assert!(u_k.contains("table    XXXV"), "{u_k}");
        assert!(u_k.contains("range    u_K = 1.08095401916(33)e13"), "{u_k}");
        assert!(u_k.contains("unit     K"), "{u_k}");
        assert!(
            u_k.contains("value    [10809540188300, 10809540194900]"),
            "{u_k}"
        );
        assert!(u_k.contains("rebuild  ok"), "{u_k}");
        assert!(u_k.contains("not P3N"), "{u_k}");
        assert!(!u_k.contains("receipt"), "{u_k}");
        assert!(!u_k.contains("theorem"), "{u_k}");
        let u_k_id = constant_node_id(&u_k);
        assert_eq!(
            u_k_id.to_hex(),
            "4d6058f8883ec3209ea999ba52721aab5843129d8aa32ca20769e345dc1acbfa",
            "journaling must not change the u_K constant payload"
        );
        assert_eq!(
            lab.store.get(u_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let eh_kg = lab
            .exec(Command::Constant {
                name: Some("Eh_kg".into()),
            })
            .text()
            .to_string();
        assert!(eh_kg.contains("constant  Eh_kg  node "), "{eh_kg}");
        assert!(
            eh_kg.contains(
                "hash     fc95a867392143c42d4006b7b085cf529610c633533ddbe2066b92390535509f"
            ),
            "{eh_kg}"
        );
        assert!(eh_kg.contains("kind     sci-interval"), "{eh_kg}");
        assert!(eh_kg.contains("table    XXXV"), "{eh_kg}");
        assert!(
            eh_kg.contains("range    Eh_kg = 4.8508702095432(94)e-35"),
            "{eh_kg}"
        );
        assert!(eh_kg.contains("unit     kg"), "{eh_kg}");
        assert!(
            eh_kg.contains("value    [48508702095338e-48, 48508702095526e-48]"),
            "{eh_kg}"
        );
        assert!(eh_kg.contains("rebuild  ok"), "{eh_kg}");
        assert!(eh_kg.contains("not P3N"), "{eh_kg}");
        assert!(!eh_kg.contains("receipt"), "{eh_kg}");
        assert!(!eh_kg.contains("theorem"), "{eh_kg}");
        let eh_kg_id = constant_node_id(&eh_kg);
        assert_eq!(
            eh_kg_id.to_hex(),
            "84aed9832f9cce41e32c84f2b27f7c7f7301cb7642e442b9b15f9faf0439211b",
            "journaling must not change the Eh_kg constant payload"
        );
        assert_eq!(
            lab.store.get(eh_kg_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let a_si = lab
            ...
            .text()
            .to_string();
        assert!(a_si.contains("constant  a_Si  node "), "{a_si}");
        assert!(
            a_si.contains(
                "hash     8d9072d01e48779f7404ab918ba023a055f197cd2c8d9f19796431939f568344"
            ),
            "{a_si}"
        );
        assert!(a_si.contains("kind     interval"), "{a_si}");
        assert!(a_si.contains("table    XXXIII"), "{a_si}");
        assert!(
            a_si.contains("range    a_Si = 5.431020511(89)e-10"),
            "{a_si}"
        );
        assert!(a_si.contains("unit     m"), "{a_si}");
        assert!(
            a_si.contains("value    [2715510211/5000000000000000000, 27155103/50000000000000000]"),
            "{a_si}"
        );
        assert!(a_si.contains("rebuild  ok"), "{a_si}");
        assert!(a_si.contains("not P3N"), "{a_si}");
        assert!(!a_si.contains("receipt"), "{a_si}");
        assert!(!a_si.contains("theorem"), "{a_si}");
        let a_si_id = constant_node_id(&a_si);
        assert_eq!(
            a_si_id.to_hex(),
            "786af851dcfebdab86280502344a362caf43e618690b85f0f3c699828c5bec29",
            "journaling must not change the a_Si constant payload"
        );
        assert_eq!(
            lab.store.get(a_si_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let n_a_e = lab
            ...
            .text()
            .to_string();
        assert!(n_a_e.contains("constant  NAe  node "), "{n_a_e}");
        assert!(
            n_a_e.contains(
                "hash     dbc99e6a827156d94029a58f2134e4f2833c556723a089cc2a9e462f3fa76ba4"
            ),
            "{n_a_e}"
        );
        assert!(n_a_e.contains("kind     ratio"), "{n_a_e}");
        assert!(n_a_e.contains("table    XXXI"), "{n_a_e}");
        assert!(
            n_a_e.contains("range    NAe = 96485.33212 exact"),
            "{n_a_e}"
        );
        assert!(n_a_e.contains("unit     C mol^{-1}"), "{n_a_e}");
        assert!(
            n_a_e.contains("value    120606665154137523/1250000000000"),
            "{n_a_e}"
        );
        assert!(n_a_e.contains("rebuild  ok"), "{n_a_e}");
        assert!(n_a_e.contains("not P3N"), "{n_a_e}");
        assert!(!n_a_e.contains("receipt"), "{n_a_e}");
        assert!(!n_a_e.contains("theorem"), "{n_a_e}");
        let n_a_e_id = constant_node_id(&n_a_e);
        assert_eq!(
            n_a_e_id.to_hex(),
            "4146991435f7e1aeca08cafaa6c26cd8f8947a49f7c1ca32df3eac4d36bb060d",
            "journaling must not change the NAe constant payload"
        );
        assert_eq!(
            lab.store.get(n_a_e_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let p_0 = lab
            ...
            .text()
            .to_string();
        assert!(p_0.contains("constant  p0  node "), "{p_0}");
        assert!(
            p_0.contains(
                "hash     21e32b495cec5be6d2655b0f3fc2e6d27541b76c8582ad17562002a30b5a1217"
            ),
            "{p_0}"
        );
        assert!(p_0.contains("kind     ratio"), "{p_0}");
        assert!(p_0.contains("table    XXXI"), "{p_0}");
        assert!(p_0.contains("range    p0 = 100000 exact"), "{p_0}");
        assert!(p_0.contains("unit     Pa"), "{p_0}");
        assert!(p_0.contains("value    100000"), "{p_0}");
        assert!(p_0.contains("rebuild  ok"), "{p_0}");
        assert!(p_0.contains("not P3N"), "{p_0}");
        assert!(!p_0.contains("receipt"), "{p_0}");
        assert!(!p_0.contains("theorem"), "{p_0}");
        let p_0_id = constant_node_id(&p_0);
        assert_eq!(
            p_0_id.to_hex(),
            "85d191df5c952fa73292a6084ae6813f7f99785b624682af2fa135aab85e45f4",
            "journaling must not change the p0 constant payload"
        );
        assert_eq!(
            lab.store.get(p_0_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let v_m = lab
            ...
            .text()
            .to_string();
        assert!(v_m.contains("constant  Vm  node "), "{v_m}");
        assert!(
            v_m.contains(
                "hash     0b56b98a81e8961ca9be8efeb6775ea197dbf0f96a913cae13944d40e71479a5"
            ),
            "{v_m}"
        );
        assert!(v_m.contains("kind     ratio"), "{v_m}");
        assert!(v_m.contains("table    XXXI"), "{v_m}");
        assert!(v_m.contains("range    Vm = 22.71095464e-3 exact"), "{v_m}");
        assert!(v_m.contains("unit     m^{3} mol^{-1}"), "{v_m}");
        assert!(
            v_m.contains("value    1135547732074278753/50000000000000000000"),
            "{v_m}"
        );
        assert!(v_m.contains("rebuild  ok"), "{v_m}");
        assert!(v_m.contains("not P3N"), "{v_m}");
        assert!(!v_m.contains("receipt"), "{v_m}");
        assert!(!v_m.contains("theorem"), "{v_m}");
        let v_m_id = constant_node_id(&v_m);
        assert_eq!(
            v_m_id.to_hex(),
            "e5115026033b6250c155eb66657eeec10fa5f72ddf3ae38e85ade223235f5f6b",
            "journaling must not change the Vm constant payload"
        );
        assert_eq!(
            lab.store.get(v_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let n_0 = lab
            ...
            .text()
            .to_string();
        assert!(n_0.contains("constant  n0  node "), "{n_0}");
        assert!(
            n_0.contains(
                "hash     886c42750e98f22584361f3cba1c202a4b75fbcdb5485e4fa06df7645129e3f3"
            ),
            "{n_0}"
        );
        assert!(n_0.contains("kind     ratio"), "{n_0}");
        assert!(n_0.contains("table    XXXI"), "{n_0}");
        assert!(n_0.contains("range    n0 = 2.651645804e25 exact"), "{n_0}");
        assert!(n_0.contains("unit     m^{-3}"), "{n_0}");
        assert!(
            n_0.contains("value    200000000000000000000000000000000000/7542485487"),
            "{n_0}"
        );
        assert!(n_0.contains("rebuild  ok"), "{n_0}");
        assert!(n_0.contains("not P3N"), "{n_0}");
        assert!(!n_0.contains("receipt"), "{n_0}");
        assert!(!n_0.contains("theorem"), "{n_0}");
        let n_0_id = constant_node_id(&n_0);
        assert_eq!(
            n_0_id.to_hex(),
            "9561ce70ecd5a964b572bf460bed2c6f035b23a4a95815923788dbc075d95eea",
            "journaling must not change the n0 constant payload"
        );
        assert_eq!(
            lab.store.get(n_0_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let v_m_atm = lab
            .exec(Command::Constant {
                name: Some("Vm_atm".into()),
            })
            .text()
            .to_string();
        assert!(v_m_atm.contains("constant  Vm_atm  node "), "{v_m_atm}");
        assert!(
            v_m_atm.contains(
                "hash     ee25d6479dd4102060b836649dc7a84cddec0dab3838c1f79d33b9e19ff11e92"
            ),
            "{v_m_atm}"
        );
        assert!(v_m_atm.contains("kind     ratio"), "{v_m_atm}");
        assert!(v_m_atm.contains("table    XXXI"), "{v_m_atm}");
        assert!(
            v_m_atm.contains("range    Vm_atm = 22.41396954e-3 exact"),
            "{v_m_atm}"
        );
        assert!(v_m_atm.contains("unit     m^{3} mol^{-1}"), "{v_m_atm}");
        assert!(
            v_m_atm.contains("value    378515910691426251/16887500000000000000"),
            "{v_m_atm}"
        );
        assert!(v_m_atm.contains("rebuild  ok"), "{v_m_atm}");
        assert!(v_m_atm.contains("not P3N"), "{v_m_atm}");
        assert!(!v_m_atm.contains("receipt"), "{v_m_atm}");
        assert!(!v_m_atm.contains("theorem"), "{v_m_atm}");
        let v_m_atm_id = constant_node_id(&v_m_atm);
        assert_eq!(
            v_m_atm_id.to_hex(),
            "cf842ed327f1aa6b028b66c6dc6c392ab75f035a01e6dc704ab14702712db094",
            "journaling must not change the Vm_atm constant payload"
        );
        assert_eq!(
            lab.store.get(v_m_atm_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let n_0_atm = lab
            .exec(Command::Constant {
                name: Some("n0_atm".into()),
            })
            .text()
            .to_string();
        assert!(n_0_atm.contains("constant  n0_atm  node "), "{n_0_atm}");
        assert!(
            n_0_atm.contains(
                "hash     040ac164b64d31d949ee7f2b59af9ed649dcd8f6fb69f09546b317b8a9beb14b"
            ),
            "{n_0_atm}"
        );
        assert!(n_0_atm.contains("kind     ratio"), "{n_0_atm}");
        assert!(n_0_atm.contains("table    XXXI"), "{n_0_atm}");
        assert!(
            n_0_atm.contains("range    n0_atm = 2.686780111e25 exact"),
            "{n_0_atm}"
        );
        assert!(n_0_atm.contains("unit     m^{-3}"), "{n_0_atm}");
        assert!(
            n_0_atm.contains("value    67550000000000000000000000000000000/2514161829"),
            "{n_0_atm}"
        );
        assert!(n_0_atm.contains("rebuild  ok"), "{n_0_atm}");
        assert!(n_0_atm.contains("not P3N"), "{n_0_atm}");
        assert!(!n_0_atm.contains("receipt"), "{n_0_atm}");
        assert!(!n_0_atm.contains("theorem"), "{n_0_atm}");
        let n_0_atm_id = constant_node_id(&n_0_atm);
        assert_eq!(
            n_0_atm_id.to_hex(),
            "a57c2a552d28588ebe8cfabbf47a74327805c9e6352f78c46b78be923ee7331e",
            "journaling must not change the n0_atm constant payload"
        );
        assert_eq!(
            lab.store.get(n_0_atm_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let s_0_r = lab
            ...
            .text()
            .to_string();
        assert!(s_0_r.contains("constant  S0_R  node "), "{s_0_r}");
        assert!(
            s_0_r.contains(
                "hash     37bd72c139f411ab023e7a400c4d8b90ef044d0334ab01e29ca31d6bdca08a8e"
            ),
            "{s_0_r}"
        );
        assert!(s_0_r.contains("kind     interval"), "{s_0_r}");
        assert!(s_0_r.contains("table    XXXI"), "{s_0_r}");
        assert!(
            s_0_r.contains("range    S0_R = -1.15170753706(45)"),
            "{s_0_r}"
        );
        assert!(s_0_r.contains("unit     1"), "{s_0_r}");
        assert!(
            s_0_r.contains("value    [-115170753751/100000000000, -115170753661/100000000000]"),
            "{s_0_r}"
        );
        assert!(s_0_r.contains("rebuild  ok"), "{s_0_r}");
        assert!(s_0_r.contains("not P3N"), "{s_0_r}");
        assert!(!s_0_r.contains("receipt"), "{s_0_r}");
        assert!(!s_0_r.contains("theorem"), "{s_0_r}");
        let s_0_r_id = constant_node_id(&s_0_r);
        assert_eq!(
            s_0_r_id.to_hex(),
            "5056a0679ae99314961bcadfe60712a265c4e1b687bf7f28b4cab22f30aa8c56",
            "journaling must not change the S0_R constant payload"
        );
        assert_eq!(
            lab.store.get(s_0_r_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let s_0_r_atm = lab
            .exec(Command::Constant {
                name: Some("S0_R_atm".into()),
            })
            .text()
            .to_string();
        assert!(
            s_0_r_atm.contains("constant  S0_R_atm  node "),
            "{s_0_r_atm}"
        );
        assert!(
            s_0_r_atm.contains(
                "hash     80cbdc3db3e995895b8c311f14beea81756bf55eee0004c45e97efc43d54af2f"
            ),
            "{s_0_r_atm}"
        );
        assert!(s_0_r_atm.contains("kind     interval"), "{s_0_r_atm}");
        assert!(s_0_r_atm.contains("table    XXXI"), "{s_0_r_atm}");
        assert!(
            s_0_r_atm.contains("range    S0_R_atm = -1.16487052358(45)"),
            "{s_0_r_atm}"
        );
        assert!(s_0_r_atm.contains("unit     1"), "{s_0_r_atm}");
        assert!(
            s_0_r_atm.contains("value    [-116487052403/100000000000, -116487052313/100000000000]"),
            "{s_0_r_atm}"
        );
        assert!(s_0_r_atm.contains("rebuild  ok"), "{s_0_r_atm}");
        assert!(s_0_r_atm.contains("not P3N"), "{s_0_r_atm}");
        assert!(!s_0_r_atm.contains("receipt"), "{s_0_r_atm}");
        assert!(!s_0_r_atm.contains("theorem"), "{s_0_r_atm}");
        let s_0_r_atm_id = constant_node_id(&s_0_r_atm);
        assert_eq!(
            s_0_r_atm_id.to_hex(),
            "c3fb74638f4c181d336240b94bd416d3976a63c2720af0497f3a1347f6fab560",
            "journaling must not change the S0_R_atm constant payload"
        );
        assert_eq!(
            lab.store.get(s_0_r_atm_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let c_1_l = lab
            ...
            .text()
            .to_string();
        assert!(c_1_l.contains("constant  c1L  node "), "{c_1_l}");
        assert!(
            c_1_l.contains(
                "hash     bb3b42d41a8d8ebc3191a2aa98d974733538eaba1098eb89a1574d228479249c"
            ),
            "{c_1_l}"
        );
        assert!(c_1_l.contains("kind     sci-exact"), "{c_1_l}");
        assert!(c_1_l.contains("table    XXXI"), "{c_1_l}");
        assert!(
            c_1_l.contains("range    c1L = 1.191042972e-16 exact"),
            "{c_1_l}"
        );
        assert!(c_1_l.contains("unit     W m^{2} sr^{-1}"), "{c_1_l}");
        assert!(
            c_1_l.contains("value    11910429723971884140794892e-41"),
            "{c_1_l}"
        );
        assert!(c_1_l.contains("rebuild  ok"), "{c_1_l}");
        assert!(c_1_l.contains("not P3N"), "{c_1_l}");
        assert!(!c_1_l.contains("receipt"), "{c_1_l}");
        assert!(!c_1_l.contains("theorem"), "{c_1_l}");
        let c_1_l_id = constant_node_id(&c_1_l);
        assert_eq!(
            c_1_l_id.to_hex(),
            "3c68d593569cb32f01daa8afb184f8565dfad4311457c390e6a3d49f665357b3",
            "journaling must not change the c1L constant payload"
        );
        assert_eq!(
            lab.store.get(c_1_l_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let c_2 = lab
            ...
            .text()
            .to_string();
        assert!(c_2.contains("constant  c2  node "), "{c_2}");
        assert!(
            c_2.contains(
                "hash     9b6ced8d9873adf9b03f13f024d13b8c2ebc18e15e9f3d57fadf0eff0ed61cbc"
            ),
            "{c_2}"
        );
        assert!(c_2.contains("kind     ratio"), "{c_2}");
        assert!(c_2.contains("table    XXXI"), "{c_2}");
        assert!(c_2.contains("range    c2 = 1.438776877e-2 exact"), "{c_2}");
        assert!(c_2.contains("unit     m K"), "{c_2}");
        assert!(
            c_2.contains("value    272115870842319/18913000000000000"),
            "{c_2}"
        );
        assert!(c_2.contains("rebuild  ok"), "{c_2}");
        assert!(c_2.contains("not P3N"), "{c_2}");
        assert!(!c_2.contains("receipt"), "{c_2}");
        assert!(!c_2.contains("theorem"), "{c_2}");
        let c_2_id = constant_node_id(&c_2);
        assert_eq!(
            c_2_id.to_hex(),
            "8c098b806e87a28bd2d42c5d7f5662dd61b3b81f9c6d926ecc1cd2c7665ccc87",
            "journaling must not change the c2 constant payload"
        );
        assert_eq!(
            lab.store.get(c_2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let k_j = lab
            ...
            .text()
            .to_string();
        assert!(k_j.contains("constant  KJ  node "), "{k_j}");
        assert!(
            k_j.contains(
                "hash     eb31c5b04ef0823e6e80a2921172c06fa6ef692e5a7700cb25d183b00a0090d2"
            ),
            "{k_j}"
        );
        assert!(k_j.contains("kind     ratio"), "{k_j}");
        assert!(k_j.contains("table    XXXI"), "{k_j}");
        assert!(k_j.contains("range    KJ = 483597.8484e9 exact"), "{k_j}");
        assert!(k_j.contains("unit     Hz V^{-1}"), "{k_j}");
        assert!(
            k_j.contains("value    21362355120000000000000/44173801"),
            "{k_j}"
        );
        assert!(k_j.contains("rebuild  ok"), "{k_j}");
        assert!(k_j.contains("not P3N"), "{k_j}");
        assert!(!k_j.contains("receipt"), "{k_j}");
        assert!(!k_j.contains("theorem"), "{k_j}");
        let k_j_id = constant_node_id(&k_j);
        assert_eq!(
            k_j_id.to_hex(),
            "911ac483d6b64b64f2d44cd808b2242e4445fd5e2eb6b13c0c714cd0aae6527c",
            "journaling must not change the KJ constant payload"
        );
        assert_eq!(
            lab.store.get(k_j_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let r_k = lab
            ...
            .text()
            .to_string();
        assert!(r_k.contains("constant  RK  node "), "{r_k}");
        assert!(
            r_k.contains(
                "hash     2faf6f39986b543d3370bdd5764f0d075fa94a709d3fadd235ed82026fed2d46"
            ),
            "{r_k}"
        );
        assert!(r_k.contains("kind     ratio"), "{r_k}");
        assert!(r_k.contains("table    XXXI"), "{r_k}");
        assert!(r_k.contains("range    RK = 25812.80745 exact"), "{r_k}");
        assert!(r_k.contains("unit     ohm"), "{r_k}");
        assert!(
            r_k.contains("value    5521725125000000000000/213914163877964163"),
            "{r_k}"
        );
        assert!(r_k.contains("rebuild  ok"), "{r_k}");
        assert!(r_k.contains("not P3N"), "{r_k}");
        assert!(!r_k.contains("receipt"), "{r_k}");
        assert!(!r_k.contains("theorem"), "{r_k}");
        let r_k_id = constant_node_id(&r_k);
        assert_eq!(
            r_k_id.to_hex(),
            "9d6fa90a4590f96e4ab850e99847ab06da3d60e90a61218f2c5642a67d174f19",
            "journaling must not change the RK constant payload"
        );
        assert_eq!(
            lab.store.get(r_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_b = lab
            ...
            .text()
            .to_string();
        assert!(mu_b.contains("constant  muB  node "), "{mu_b}");
        assert!(
            mu_b.contains(
                "hash     05bdf64c433e9c8bdf8db2dd7991db310baf2ba41fb0e3cff31d1cd98ef0f9df"
            ),
            "{mu_b}"
        );
        assert!(mu_b.contains("kind     interval"), "{mu_b}");
        assert!(mu_b.contains("table    XXXI"), "{mu_b}");
        assert!(
            mu_b.contains("range    muB = 9.2740100783(28)e-24"),
            "{mu_b}"
        );
        assert!(mu_b.contains("unit     J T^{-1}"), "{mu_b}");
        assert!(
            mu_b.contains("value    [18548020151/2000000000000000000000000000000000, 92740100811/10000000000000000000000000000000000]"),
            "{mu_b}"
        );
        assert!(mu_b.contains("rebuild  ok"), "{mu_b}");
        assert!(mu_b.contains("not P3N"), "{mu_b}");
        assert!(!mu_b.contains("receipt"), "{mu_b}");
        assert!(!mu_b.contains("theorem"), "{mu_b}");
        let mu_b_id = constant_node_id(&mu_b);
        assert_eq!(
            mu_b_id.to_hex(),
            "25548862db56005d5ed865aee7f0ee08a7cd24c478ec0aec2e78c775ca9060ef",
            "journaling must not change the muB constant payload"
        );
        assert_eq!(
            lab.store.get(mu_b_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_b_ev = lab
            .exec(Command::Constant {
                name: Some("muB_eV".into()),
            })
            .text()
            .to_string();
        assert!(mu_b_ev.contains("constant  muB_eV  node "), "{mu_b_ev}");
        assert!(
            mu_b_ev.contains(
                "hash     2eba7e5ba5747c0aabedad751c362d39314d5d212378279e872a2a0c48cdf15b"
            ),
            "{mu_b_ev}"
        );
        assert!(mu_b_ev.contains("kind     interval"), "{mu_b_ev}");
        assert!(mu_b_ev.contains("table    XXXI"), "{mu_b_ev}");
        assert!(
            mu_b_ev.contains("range    muB_eV = 5.7883818060(17)e-5"),
            "{mu_b_ev}"
        );
        assert!(mu_b_ev.contains("unit     eV T^{-1}"), "{mu_b_ev}");
        assert!(
            mu_b_ev
                .contains("value    [57883818043/1000000000000000, 57883818077/1000000000000000]"),
            "{mu_b_ev}"
        );
        assert!(mu_b_ev.contains("rebuild  ok"), "{mu_b_ev}");
        assert!(mu_b_ev.contains("not P3N"), "{mu_b_ev}");
        assert!(!mu_b_ev.contains("receipt"), "{mu_b_ev}");
        assert!(!mu_b_ev.contains("theorem"), "{mu_b_ev}");
        let mu_b_ev_id = constant_node_id(&mu_b_ev);
        assert_eq!(
            mu_b_ev_id.to_hex(),
            "8df4a0984aa398db3d36b391d61ec73784b0a7c72257906a42b9e7425efc6d49",
            "journaling must not change the muB_eV constant payload"
        );
        assert_eq!(
            lab.store.get(mu_b_ev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_b_hz = lab
            .exec(Command::Constant {
                name: Some("muB_Hz".into()),
            })
            .text()
            .to_string();
        assert!(mu_b_hz.contains("constant  muB_Hz  node "), "{mu_b_hz}");
        assert!(
            mu_b_hz.contains(
                "hash     b51f2cbb2761484c081909689471b593a47521359e481cbbe1c6c0083d6cb749"
            ),
            "{mu_b_hz}"
        );
        assert!(mu_b_hz.contains("kind     interval"), "{mu_b_hz}");
        assert!(mu_b_hz.contains("table    XXXI"), "{mu_b_hz}");
        assert!(
            mu_b_hz.contains("range    muB_Hz = 1.39962449361(42)e10"),
            "{mu_b_hz}"
        );
        assert!(mu_b_hz.contains("unit     Hz T^{-1}"), "{mu_b_hz}");
        assert!(
            mu_b_hz.contains("value    [139962449319/10, 139962449403/10]"),
            "{mu_b_hz}"
        );
        assert!(mu_b_hz.contains("rebuild  ok"), "{mu_b_hz}");
        assert!(mu_b_hz.contains("not P3N"), "{mu_b_hz}");
        assert!(!mu_b_hz.contains("receipt"), "{mu_b_hz}");
        assert!(!mu_b_hz.contains("theorem"), "{mu_b_hz}");
        let mu_b_hz_id = constant_node_id(&mu_b_hz);
        assert_eq!(
            mu_b_hz_id.to_hex(),
            "3cf64fd1f060d0379c3d21d17b5ba9bffa0c40ad1817493a084d13cde1cba088",
            "journaling must not change the muB_Hz constant payload"
        );
        assert_eq!(
            lab.store.get(mu_b_hz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_b_m = lab
            .exec(Command::Constant {
                name: Some("muB_m".into()),
            })
            .text()
            .to_string();
        assert!(mu_b_m.contains("constant  muB_m  node "), "{mu_b_m}");
        assert!(
            mu_b_m.contains(
                "hash     5870c955c8af2612ebe76eee32448cabd843ef430611ac3cd01623ae9dfd7bb3"
            ),
            "{mu_b_m}"
        );
        assert!(mu_b_m.contains("kind     interval"), "{mu_b_m}");
        assert!(mu_b_m.contains("table    XXXI"), "{mu_b_m}");
        assert!(
            mu_b_m.contains("range    muB_m = 46.686447783(14)"),
            "{mu_b_m}"
        );
        assert!(mu_b_m.contains("unit     m^{-1} T^{-1}"), "{mu_b_m}");
        assert!(
            mu_b_m.contains("value    [46686447769/1000000000, 46686447797/1000000000]"),
            "{mu_b_m}"
        );
        assert!(mu_b_m.contains("rebuild  ok"), "{mu_b_m}");
        assert!(mu_b_m.contains("not P3N"), "{mu_b_m}");
        assert!(!mu_b_m.contains("receipt"), "{mu_b_m}");
        assert!(!mu_b_m.contains("theorem"), "{mu_b_m}");
        let mu_b_m_id = constant_node_id(&mu_b_m);
        assert_eq!(
            mu_b_m_id.to_hex(),
            "a5c6bc7a371abe6a50398009955f3ff1790cb5323027ddb9f51bc90920664e04",
            "journaling must not change the muB_m constant payload"
        );
        assert_eq!(
            lab.store.get(mu_b_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_b_k = lab
            .exec(Command::Constant {
                name: Some("muB_K".into()),
            })
            .text()
            .to_string();
        assert!(mu_b_k.contains("constant  muB_K  node "), "{mu_b_k}");
        assert!(
            mu_b_k.contains(
                "hash     fd9a45856a999a4b1af21966c2cb3e3cee7f27aa6e8960a3168868dea04aa451"
            ),
            "{mu_b_k}"
        );
        assert!(mu_b_k.contains("kind     interval"), "{mu_b_k}");
        assert!(mu_b_k.contains("table    XXXI"), "{mu_b_k}");
        assert!(
            mu_b_k.contains("range    muB_K = 0.67171381563(20)"),
            "{mu_b_k}"
        );
        assert!(mu_b_k.contains("unit     K T^{-1}"), "{mu_b_k}");
        assert!(
            mu_b_k.contains("value    [67171381543/100000000000, 67171381583/100000000000]"),
            "{mu_b_k}"
        );
        assert!(mu_b_k.contains("rebuild  ok"), "{mu_b_k}");
        assert!(mu_b_k.contains("not P3N"), "{mu_b_k}");
        assert!(!mu_b_k.contains("receipt"), "{mu_b_k}");
        assert!(!mu_b_k.contains("theorem"), "{mu_b_k}");
        let mu_b_k_id = constant_node_id(&mu_b_k);
        assert_eq!(
            mu_b_k_id.to_hex(),
            "0dc9efe8516984f8cd702a37559d258d6073f1e6f2c04544c705b7a1b2a0d083",
            "journaling must not change the muB_K constant payload"
        );
        assert_eq!(
            lab.store.get(mu_b_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let mu_nuc = lab
            ...
            .text()
            .to_string();
        assert!(mu_nuc.contains("constant  muN  node "), "{mu_nuc}");
        assert!(
            mu_nuc.contains(
                "hash     c3185d2ffda0a76ed87ea106513eb25592aec9bbec35f8647a81d367f70f3835"
            ),
            "{mu_nuc}"
        );
        assert!(mu_nuc.contains("kind     interval"), "{mu_nuc}");
        assert!(mu_nuc.contains("table    XXXI"), "{mu_nuc}");
        assert!(
            mu_nuc.contains("range    muN = 5.0507837461(15)e-27"),
            "{mu_nuc}"
        );
        assert!(mu_nuc.contains("unit     J T^{-1}"), "{mu_nuc}");
        assert!(
            mu_nuc.contains("value    [25253918723/5000000000000000000000000000000000000, 12626959369/2500000000000000000000000000000000000]"),
            "{mu_nuc}"
        );
        assert!(mu_nuc.contains("rebuild  ok"), "{mu_nuc}");
        assert!(mu_nuc.contains("not P3N"), "{mu_nuc}");
        assert!(!mu_nuc.contains("receipt"), "{mu_nuc}");
        assert!(!mu_nuc.contains("theorem"), "{mu_nuc}");
        let mu_nuc_id = constant_node_id(&mu_nuc);
        assert_eq!(
            mu_nuc_id.to_hex(),
            "5b78c0f11cbde89d9e2b0fb274c93113715007644fab16975a1c87fea4c79efb",
            "journaling must not change the muN constant payload"
        );
        assert_eq!(
            lab.store.get(mu_nuc_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_nuc_ev = lab
            .exec(Command::Constant {
                name: Some("muN_eV".into()),
            })
            .text()
            .to_string();
        assert!(mu_nuc_ev.contains("constant  muN_eV  node "), "{mu_nuc_ev}");
        assert!(
            mu_nuc_ev.contains(
                "hash     896e79954d3048b9eb9f1f8d0be11c351690c3bd2db4d3be8f1853395bc51291"
            ),
            "{mu_nuc_ev}"
        );
        assert!(mu_nuc_ev.contains("kind     interval"), "{mu_nuc_ev}");
        assert!(mu_nuc_ev.contains("table    XXXI"), "{mu_nuc_ev}");
        assert!(
            mu_nuc_ev.contains("range    muN_eV = 3.15245125844(96)e-8"),
            "{mu_nuc_ev}"
        );
        assert!(mu_nuc_ev.contains("unit     eV T^{-1}"), "{mu_nuc_ev}");
        assert!(
            mu_nuc_ev.contains(
                "value    [78811281437/2500000000000000000, 15762256297/500000000000000000]"
            ),
            "{mu_nuc_ev}"
        );
        assert!(mu_nuc_ev.contains("rebuild  ok"), "{mu_nuc_ev}");
        assert!(mu_nuc_ev.contains("not P3N"), "{mu_nuc_ev}");
        assert!(!mu_nuc_ev.contains("receipt"), "{mu_nuc_ev}");
        assert!(!mu_nuc_ev.contains("theorem"), "{mu_nuc_ev}");
        let mu_nuc_ev_id = constant_node_id(&mu_nuc_ev);
        assert_eq!(
            mu_nuc_ev_id.to_hex(),
            "3499319aa12a29a8123e77d0a0fcbd6cc4a0ee6da708788ef7450ad0453892d0",
            "journaling must not change the muN_eV constant payload"
        );
        assert_eq!(
            lab.store.get(mu_nuc_ev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_nuc_m = lab
            .exec(Command::Constant {
                name: Some("muN_m".into()),
            })
            .text()
            .to_string();
        assert!(mu_nuc_m.contains("constant  muN_m  node "), "{mu_nuc_m}");
        assert!(
            mu_nuc_m.contains(
                "hash     4cbff17f649eaa3b46d7ec75aeeddc8a8becebcddc7c4538bca1e96097800e0f"
            ),
            "{mu_nuc_m}"
        );
        assert!(mu_nuc_m.contains("kind     interval"), "{mu_nuc_m}");
        assert!(mu_nuc_m.contains("table    XXXI"), "{mu_nuc_m}");
        assert!(
            mu_nuc_m.contains("range    muN_m = 2.54262341353(78)e-2"),
            "{mu_nuc_m}"
        );
        assert!(mu_nuc_m.contains("unit     m^{-1} T^{-1}"), "{mu_nuc_m}");
        assert!(
            mu_nuc_m.contains("value    [10170493651/400000000000, 254262341431/10000000000000]"),
            "{mu_nuc_m}"
        );
        assert!(mu_nuc_m.contains("rebuild  ok"), "{mu_nuc_m}");
        assert!(mu_nuc_m.contains("not P3N"), "{mu_nuc_m}");
        assert!(!mu_nuc_m.contains("receipt"), "{mu_nuc_m}");
        assert!(!mu_nuc_m.contains("theorem"), "{mu_nuc_m}");
        let mu_nuc_m_id = constant_node_id(&mu_nuc_m);
        assert_eq!(
            mu_nuc_m_id.to_hex(),
            "1c927bad61aec31ce19c4de08f8331adeba44d15f07c9577d84556b2315f7b9a",
            "journaling must not change the muN_m constant payload"
        );
        assert_eq!(
            lab.store.get(mu_nuc_m_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_nuc_k = lab
            .exec(Command::Constant {
                name: Some("muN_K".into()),
            })
            .text()
            .to_string();
        assert!(mu_nuc_k.contains("constant  muN_K  node "), "{mu_nuc_k}");
        assert!(
            mu_nuc_k.contains(
                "hash     deb575e22e92050ab049888ba327bad752ec6d478cb3384b64ad7b3d2e6592bd"
            ),
            "{mu_nuc_k}"
        );
        assert!(mu_nuc_k.contains("kind     interval"), "{mu_nuc_k}");
        assert!(mu_nuc_k.contains("table    XXXI"), "{mu_nuc_k}");
        assert!(
            mu_nuc_k.contains("range    muN_K = 3.6582677756(11)e-4"),
            "{mu_nuc_k}"
        );
        assert!(mu_nuc_k.contains("unit     K T^{-1}"), "{mu_nuc_k}");
        assert!(
            mu_nuc_k.contains("value    [7316535549/20000000000000, 36582677767/100000000000000]"),
            "{mu_nuc_k}"
        );
        assert!(mu_nuc_k.contains("rebuild  ok"), "{mu_nuc_k}");
        assert!(mu_nuc_k.contains("not P3N"), "{mu_nuc_k}");
        assert!(!mu_nuc_k.contains("receipt"), "{mu_nuc_k}");
        assert!(!mu_nuc_k.contains("theorem"), "{mu_nuc_k}");
        let mu_nuc_k_id = constant_node_id(&mu_nuc_k);
        assert_eq!(
            mu_nuc_k_id.to_hex(),
            "e1076582e911fb1259ed2db8919b357e754aee37032996e48ae58a8759dadd68",
            "journaling must not change the muN_K constant payload"
        );
        assert_eq!(
            lab.store.get(mu_nuc_k_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let mu_nuc_mhz = lab
            .exec(Command::Constant {
                name: Some("muN_MHz".into()),
            })
            .text()
            .to_string();
        assert!(
            mu_nuc_mhz.contains("constant  muN_MHz  node "),
            "{mu_nuc_mhz}"
        );
        assert!(
            mu_nuc_mhz.contains(
                "hash     bd63dae42eacb72e6c76f65457ce03a8abd107dbce9faa68ee04e0ed78c9adf1"
            ),
            "{mu_nuc_mhz}"
        );
        assert!(mu_nuc_mhz.contains("kind     interval"), "{mu_nuc_mhz}");
        assert!(mu_nuc_mhz.contains("table    XXXI"), "{mu_nuc_mhz}");
        assert!(
            mu_nuc_mhz.contains("range    muN_MHz = 7.6225932291(23)"),
            "{mu_nuc_mhz}"
        );
        assert!(mu_nuc_mhz.contains("unit     MHz T^{-1}"), "{mu_nuc_mhz}");
        assert!(
            mu_nuc_mhz.contains("value    [19056483067/2500000000, 38112966157/5000000000]"),
            "{mu_nuc_mhz}"
        );
        assert!(mu_nuc_mhz.contains("rebuild  ok"), "{mu_nuc_mhz}");
        assert!(mu_nuc_mhz.contains("not P3N"), "{mu_nuc_mhz}");
        assert!(!mu_nuc_mhz.contains("receipt"), "{mu_nuc_mhz}");
        assert!(!mu_nuc_mhz.contains("theorem"), "{mu_nuc_mhz}");
        let mu_nuc_mhz_id = constant_node_id(&mu_nuc_mhz);
        assert_eq!(
            mu_nuc_mhz_id.to_hex(),
            "54de34e2ffd258b5a8661cfa82244d91bad42238d9d554602c1eb8a269af583c",
            "journaling must not change the muN_MHz constant payload"
        );
        assert_eq!(
            lab.store.get(mu_nuc_mhz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma_p = lab
            .exec(Command::Constant {
                name: Some("gamma_p".into()),
            })
            .text()
            .to_string();
        assert!(gamma_p.contains("constant  gamma_p  node "), "{gamma_p}");
        assert!(
            gamma_p.contains(
                "hash     116f462ed588536a31ea6e33be0ef8c1e26a5adb78b807dc93a8d5df0204457e"
            ),
            "{gamma_p}"
        );
        assert!(gamma_p.contains("kind     interval"), "{gamma_p}");
        assert!(gamma_p.contains("table    XXXI"), "{gamma_p}");
        assert!(
            gamma_p.contains("range    gamma_p = 2.6752218744(11)e8"),
            "{gamma_p}"
        );
        assert!(gamma_p.contains("unit     s^{-1} T^{-1}"), "{gamma_p}");
        assert!(
            gamma_p.contains("value    [26752218733/100, 5350443751/20]"),
            "{gamma_p}"
        );
        assert!(gamma_p.contains("rebuild  ok"), "{gamma_p}");
        assert!(gamma_p.contains("not P3N"), "{gamma_p}");
        assert!(!gamma_p.contains("receipt"), "{gamma_p}");
        assert!(!gamma_p.contains("theorem"), "{gamma_p}");
        let gamma_p_id = constant_node_id(&gamma_p);
        assert_eq!(
            gamma_p_id.to_hex(),
            "d9f5089e6e55a37dea55e5d8418cf58eba84135d3a03bc0cd1ada40b07db50f3",
            "journaling must not change the gamma_p constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma_p_mhz = lab
            .exec(Command::Constant {
                name: Some("gamma_p_MHz".into()),
            })
            .text()
            .to_string();
        assert!(
            gamma_p_mhz.contains("constant  gamma_p_MHz  node "),
            "{gamma_p_mhz}"
        );
        assert!(
            gamma_p_mhz.contains(
                "hash     0e7b084d03777a4e9e875e48a87702d1d4284d30cb7ea50ba424f103d4660f73"
            ),
            "{gamma_p_mhz}"
        );
        assert!(gamma_p_mhz.contains("kind     interval"), "{gamma_p_mhz}");
        assert!(gamma_p_mhz.contains("table    XXXI"), "{gamma_p_mhz}");
        assert!(
            gamma_p_mhz.contains("range    gamma_p_MHz = 42.577478518(18)"),
            "{gamma_p_mhz}"
        );
        assert!(gamma_p_mhz.contains("unit     MHz T^{-1}"), "{gamma_p_mhz}");
        assert!(
            gamma_p_mhz.contains("value    [85154957/2000000, 5322184817/125000000]"),
            "{gamma_p_mhz}"
        );
        assert!(gamma_p_mhz.contains("rebuild  ok"), "{gamma_p_mhz}");
        assert!(gamma_p_mhz.contains("not P3N"), "{gamma_p_mhz}");
        assert!(!gamma_p_mhz.contains("receipt"), "{gamma_p_mhz}");
        assert!(!gamma_p_mhz.contains("theorem"), "{gamma_p_mhz}");
        let gamma_p_mhz_id = constant_node_id(&gamma_p_mhz);
        assert_eq!(
            gamma_p_mhz_id.to_hex(),
            "89744598268cf707cc8c75c45c9eecd6e9380f4caa2521b9a94633818fc6ebf9",
            "journaling must not change the gamma_p_MHz constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_p_mhz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma0p = lab
            .exec(Command::Constant {
                name: Some("gamma0p".into()),
            })
            .text()
            .to_string();
        assert!(gamma0p.contains("constant  gamma0p  node "), "{gamma0p}");
        assert!(
            gamma0p.contains(
                "hash     ad17e3a83ff5bcbb8f9cb82c9065a16a82caf68df4f48c901c86b4f62a633cbc"
            ),
            "{gamma0p}"
        );
        assert!(gamma0p.contains("kind     interval"), "{gamma0p}");
        assert!(gamma0p.contains("table    XXXI"), "{gamma0p}");
        assert!(
            gamma0p.contains("range    gamma0p = 2.675153151(29)e8"),
            "{gamma0p}"
        );
        assert!(gamma0p.contains("unit     s^{-1} T^{-1}"), "{gamma0p}");
        assert!(
            gamma0p.contains("value    [1337576561/5, 267515318]"),
            "{gamma0p}"
        );
        assert!(gamma0p.contains("rebuild  ok"), "{gamma0p}");
        assert!(gamma0p.contains("not P3N"), "{gamma0p}");
        assert!(!gamma0p.contains("receipt"), "{gamma0p}");
        assert!(!gamma0p.contains("theorem"), "{gamma0p}");
        let gamma0p_id = constant_node_id(&gamma0p);
        assert_eq!(
            gamma0p_id.to_hex(),
            "b5ab615ca36f3799a21c3574ba8d45c12142ec93172797980396ffa02d2f1963",
            "journaling must not change the gamma0p constant payload"
        );
        assert_eq!(
            lab.store.get(gamma0p_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gamma_n = lab
            .exec(Command::Constant {
                name: Some("gamma_n".into()),
            })
            .text()
            .to_string();
        assert!(gamma_n.contains("constant  gamma_n  node "), "{gamma_n}");
        assert!(
            gamma_n.contains(
                "hash     cc9cc1b05368ff576b267d4475acaf2b2fb98fda286dc4a8e1f53556e4841914"
            ),
            "{gamma_n}"
        );
        assert!(gamma_n.contains("kind     interval"), "{gamma_n}");
        assert!(gamma_n.contains("table    XXXI"), "{gamma_n}");
        assert!(
            gamma_n.contains("range    gamma_n = 1.83247171(43)e8"),
            "{gamma_n}"
        );
        assert!(gamma_n.contains("unit     s^{-1} T^{-1}"), "{gamma_n}");
        assert!(
            gamma_n.contains("value    [183247128, 183247214]"),
            "{gamma_n}"
        );
        assert!(gamma_n.contains("rebuild  ok"), "{gamma_n}");
        assert!(gamma_n.contains("not P3N"), "{gamma_n}");
        assert!(!gamma_n.contains("receipt"), "{gamma_n}");
        assert!(!gamma_n.contains("theorem"), "{gamma_n}");
        let gamma_n_id = constant_node_id(&gamma_n);
        assert_eq!(
            gamma_n_id.to_hex(),
            "26593240a293a2a081e5f473eb3f2befbbc559fade13a6761e8b9022c8fae84f",
            "journaling must not change the gamma_n constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_n_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma_n_mhz = lab
            .exec(Command::Constant {
                name: Some("gamma_n_MHz".into()),
            })
            .text()
            .to_string();
        assert!(
            gamma_n_mhz.contains("constant  gamma_n_MHz  node "),
            "{gamma_n_mhz}"
        );
        assert!(
            gamma_n_mhz.contains(
                "hash     afb240ddc4bf6fa45a44eb869268021801762dd70fbaa21d23e5fbc3669049a7"
            ),
            "{gamma_n_mhz}"
        );
        assert!(gamma_n_mhz.contains("kind     interval"), "{gamma_n_mhz}");
        assert!(gamma_n_mhz.contains("table    XXXI"), "{gamma_n_mhz}");
        assert!(
            gamma_n_mhz.contains("range    gamma_n_MHz = 29.1646931(69)"),
            "{gamma_n_mhz}"
        );
        assert!(gamma_n_mhz.contains("unit     MHz T^{-1}"), "{gamma_n_mhz}");
        assert!(
            gamma_n_mhz.contains("value    [145823431/5000000, 291647/10000]"),
            "{gamma_n_mhz}"
        );
        assert!(gamma_n_mhz.contains("rebuild  ok"), "{gamma_n_mhz}");
        assert!(gamma_n_mhz.contains("not P3N"), "{gamma_n_mhz}");
        assert!(!gamma_n_mhz.contains("receipt"), "{gamma_n_mhz}");
        assert!(!gamma_n_mhz.contains("theorem"), "{gamma_n_mhz}");
        let gamma_n_mhz_id = constant_node_id(&gamma_n_mhz);
        assert_eq!(
            gamma_n_mhz_id.to_hex(),
            "2ab81c06d7033e1ee4d83bc510c77355be587dae3c270af8941ba12a568d8608",
            "journaling must not change the gamma_n_MHz constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_n_mhz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma_e = lab
            .exec(Command::Constant {
                name: Some("gamma_e".into()),
            })
            .text()
            .to_string();
        assert!(gamma_e.contains("constant  gamma_e  node "), "{gamma_e}");
        assert!(
            gamma_e.contains(
        ...
        assert!(gamma_e.contains("kind     interval"), "{gamma_e}");
        assert!(gamma_e.contains("table    XXXI"), "{gamma_e}");
        assert!(
            gamma_e.contains("range    gamma_e = 1.76085963023(53)e11"),
            "{gamma_e}"
        );
        assert!(gamma_e.contains("unit     s^{-1} T^{-1}"), "{gamma_e}");
        assert!(
            gamma_e.contains("value    [176085962970, 176085963076]"),
            "{gamma_e}"
        );
        assert!(gamma_e.contains("rebuild  ok"), "{gamma_e}");
        assert!(gamma_e.contains("not P3N"), "{gamma_e}");
        assert!(!gamma_e.contains("receipt"), "{gamma_e}");
        assert!(!gamma_e.contains("theorem"), "{gamma_e}");
        let gamma_e_id = constant_node_id(&gamma_e);
        assert_eq!(
            gamma_e_id.to_hex(),
            "8d3e3aeab146c0e8e7ef31e521dad0b8fba6385f1c496900f93d17d45071c788",
            "journaling must not change the gamma_e constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_e_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let gamma_e_mhz = lab
            .exec(Command::Constant {
                name: Some("gamma_e_MHz".into()),
            })
            .text()
            .to_string();
        assert!(
            gamma_e_mhz.contains("constant  gamma_e_MHz  node "),
            "{gamma_e_mhz}"
        );
        assert!(
            gamma_e_mhz.contains(
                "hash     4467f4343ca683946219dde053497c42e8da08518663051808ffe7529630eda0"
            ),
            "{gamma_e_mhz}"
        );
        assert!(gamma_e_mhz.contains("kind     interval"), "{gamma_e_mhz}");
        assert!(gamma_e_mhz.contains("table    XXXI"), "{gamma_e_mhz}");
        assert!(
            gamma_e_mhz.contains("range    gamma_e_MHz = 28024.9514242(85)"),
            "{gamma_e_mhz}"
        );
        assert!(gamma_e_mhz.contains("unit     MHz T^{-1}"), "{gamma_e_mhz}");
        assert!(
            gamma_e_mhz.contains("value    [280249514157/10000000, 280249514327/10000000]"),
            "{gamma_e_mhz}"
        );
        assert!(gamma_e_mhz.contains("rebuild  ok"), "{gamma_e_mhz}");
        assert!(gamma_e_mhz.contains("not P3N"), "{gamma_e_mhz}");
        assert!(!gamma_e_mhz.contains("receipt"), "{gamma_e_mhz}");
        assert!(!gamma_e_mhz.contains("theorem"), "{gamma_e_mhz}");
        let gamma_e_mhz_id = constant_node_id(&gamma_e_mhz);
        assert_eq!(
            gamma_e_mhz_id.to_hex(),
            "a2a19a2dbfc04e754e457c06a5f84f67a7dc185a5a1dc63634ef97f75700f243",
            "journaling must not change the gamma_e_MHz constant payload"
        );
        assert_eq!(
            lab.store.get(gamma_e_mhz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let gamma0h_mhz = lab
            .exec(Command::Constant {
                name: Some("gamma0h_MHz".into()),
            })
            .text()
            .to_string();
        assert!(
            gamma0h_mhz.contains("constant  gamma0h_MHz  node "),
            "{gamma0h_mhz}"
        );
        assert!(
            gamma0h_mhz.contains(
                "hash     222550d6fcbe1f85109b0d4fb4e6e9d4529a471976a1f24587a3cc29fac5f6ac"
            ),
            "{gamma0h_mhz}"
        );
        assert!(gamma0h_mhz.contains("kind     interval"), "{gamma0h_mhz}");
        assert!(gamma0h_mhz.contains("table    XXXI"), "{gamma0h_mhz}");
        assert!(
            gamma0h_mhz.contains("range    gamma0h_MHz = 32.43409942(38)"),
            "{gamma0h_mhz}"
        );
        assert!(gamma0h_mhz.contains("unit     MHz T^{-1}"), "{gamma0h_mhz}");
        assert!(
            gamma0h_mhz.contains("value    [202713119/6250000, 162170499/5000000]"),
            "{gamma0h_mhz}"
        );
        assert!(gamma0h_mhz.contains("rebuild  ok"), "{gamma0h_mhz}");
        assert!(gamma0h_mhz.contains("not P3N"), "{gamma0h_mhz}");
        assert!(!gamma0h_mhz.contains("receipt"), "{gamma0h_mhz}");
        assert!(!gamma0h_mhz.contains("theorem"), "{gamma0h_mhz}");
        let gamma0h_mhz_id = constant_node_id(&gamma0h_mhz);
        assert_eq!(
            gamma0h_mhz_id.to_hex(),
            "ea16a9b399f93c1d2c29197dba77ad96c1b201cbc041dca6995984f7a0986ec1",
            "journaling must not change the gamma0h_MHz constant payload"
        );
        assert_eq!(
            lab.store.get(gamma0h_mhz_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let me_u = lab
            .exec(Command::Constant {
                name: Some("m_e_u".into()),
            })
            .text()
            .to_string();
        assert!(me_u.contains("constant  m_e_u  node "), "{me_u}");
        assert!(
            me_u.contains(
                "hash     f0f8df112f644065bd01e2b903312927e5ef5d21d7792484aca0e8e7d29934d5"
            ),
            "{me_u}"
        );
        assert!(me_u.contains("kind     interval"), "{me_u}");
        assert!(me_u.contains("table    XXXI"), "{me_u}");
        assert!(
            me_u.contains("range    m_e_u = 5.48579909065(16)e-4"),
            "{me_u}"
        );
        assert!(me_u.contains("unit     u"), "{me_u}");
        assert!(
            me_u.contains(
                "value    [548579909049/1000000000000000, 548579909081/1000000000000000]"
            ),
            "{me_u}"
        );
        assert!(me_u.contains("rebuild  ok"), "{me_u}");
        assert!(me_u.contains("not P3N"), "{me_u}");
        assert!(!me_u.contains("receipt"), "{me_u}");
        assert!(!me_u.contains("theorem"), "{me_u}");
        let me_u_id = constant_node_id(&me_u);
        assert_eq!(
            me_u_id.to_hex(),
            "682e118704f0d556c0c83465b59e59e27c0d59730ddaf82c430d3d5162066741",
            "journaling must not change the m_e_u constant payload"
        );
        assert_eq!(
            lab.store.get(me_u_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let me_c2 = lab
            .exec(Command::Constant {
                name: Some("m_e_c2".into()),
            })
            .text()
            .to_string();
        assert!(me_c2.contains("constant  m_e_c2  node "), "{me_c2}");
        assert!(
            me_c2.contains(
                "hash     935f7db8457024efb853abe9ee42c24e5efec5c4a831a94a308eb46efa6db0d5"
            ),
            "{me_c2}"
        );
        assert!(me_c2.contains("kind     interval"), "{me_c2}");
        assert!(me_c2.contains("table    XXXI"), "{me_c2}");
        assert!(
            me_c2.contains("range    m_e_c2 = 8.1871057769(25)e-14"),
            "{me_c2}"
        );
        assert!(me_c2.contains("unit     J"), "{me_c2}");
        assert!(
            me_c2.contains("value    [5116941109/62500000000000000000000, 40935528897/500000000000000000000000]"),
            "{me_c2}"
        );
        assert!(me_c2.contains("rebuild  ok"), "{me_c2}");
        assert!(me_c2.contains("not P3N"), "{me_c2}");
        assert!(!me_c2.contains("receipt"), "{me_c2}");
        assert!(!me_c2.contains("theorem"), "{me_c2}");
        let me_c2_id = constant_node_id(&me_c2);
        assert_eq!(
            me_c2_id.to_hex(),
            "c78389974fdf97ffc418c12da149e820bc718c7ae103546574696f8d78a13c11",
            "journaling must not change the m_e_c2 constant payload"
        );
        assert_eq!(
            lab.store.get(me_c2_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );

        let me_c2_mev = lab
            .exec(Command::Constant {
                name: Some("m_e_c2_MeV".into()),
            })
            .text()
            .to_string();
        assert!(
            me_c2_mev.contains("constant  m_e_c2_MeV  node "),
            "{me_c2_mev}"
        );
        assert!(
            me_c2_mev.contains(
                "hash     c076e0b56ff109b6f16661d0e7874b9f352cf53c4f862c38203c082a17d87f6b"
            ),
            "{me_c2_mev}"
        );
        assert!(me_c2_mev.contains("kind     interval"), "{me_c2_mev}");
        assert!(me_c2_mev.contains("table    XXXI"), "{me_c2_mev}");
        assert!(
            me_c2_mev.contains("range    m_e_c2_MeV = 0.51099895000(15)"),
            "{me_c2_mev}"
        );
        assert!(me_c2_mev.contains("unit     MeV"), "{me_c2_mev}");
        assert!(
            me_c2_mev.contains("value    [10219978997/20000000000, 10219979003/20000000000]"),
            "{me_c2_mev}"
        );
        assert!(me_c2_mev.contains("rebuild  ok"), "{me_c2_mev}");
        assert!(me_c2_mev.contains("not P3N"), "{me_c2_mev}");
        assert!(!me_c2_mev.contains("receipt"), "{me_c2_mev}");
        assert!(!me_c2_mev.contains("theorem"), "{me_c2_mev}");
        let me_c2_mev_id = constant_node_id(&me_c2_mev);
        assert_eq!(
            me_c2_mev_id.to_hex(),
            "9ac6f08d59173db167d93f15a3d4265d26f966a92584a7053dd0dd8d62f3d33a",
            "journaling must not change the m_e_c2_MeV constant payload"
        );
        assert_eq!(
            lab.store.get(me_c2_mev_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let me_kg = lab
            ...
            .text()
            .to_string();
        assert!(me_kg.contains("constant  m_e  node "), "{me_kg}");
        assert!(
            me_kg.contains(
                "hash     961f48a48d6ca0a563b2ae710ebc3a908369fcfd0bd9d7c59ba2b663a6c275bd"
            ),
            "{me_kg}"
        );
        assert!(me_kg.contains("kind     sci-interval"), "{me_kg}");
        assert!(me_kg.contains("table    XXXI"), "{me_kg}");
        assert!(
            me_kg.contains("range    m_e = 9.1093837015(28)e-31"),
            "{me_kg}"
        );
        assert!(me_kg.contains("unit     kg"), "{me_kg}");
        assert!(
            me_kg.contains("value    [91093836987e-41, 91093837043e-41]"),
            "{me_kg}"
        );
        assert!(me_kg.contains("rebuild  ok"), "{me_kg}");
        assert!(me_kg.contains("not P3N"), "{me_kg}");
        assert!(!me_kg.contains("receipt"), "{me_kg}");
        assert!(!me_kg.contains("theorem"), "{me_kg}");
        let me_kg_id = constant_node_id(&me_kg);
        assert_eq!(
            me_kg_id.to_hex(),
            "25b4aa84438ef207187bb7361b283608c551cf3a992d1bbf9aead9228af322aa",
            "journaling must not change the m_e constant payload"
        );
        assert_eq!(
            lab.store.get(me_kg_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let nup = lab
            ...
            .text()
            .to_string();
        assert!(nup.contains("constant  nu_p  node "), "{nup}");
        assert!(
            nup.contains(
                "hash     e9b6fb4c3612f5c594b48eaa0227c83a9179843e431c0b373c5ef0c25d151daa"
            ),
            "{nup}"
        );
        assert!(nup.contains("kind     interval"), "{nup}");
        assert!(nup.contains("table    XXXIV"), "{nup}");
        assert!(
            nup.contains("range    nu_p = 2.73092453075(82)e-22"),
            "{nup}"
        );
        assert!(nup.contains("unit     kg m s^{-1}"), "{nup}");
        assert!(
            nup.contains("value    [273092452993/1000000000000000000000000000000000, 273092453157/1000000000000000000000000000000000]"),
            "{nup}"
        );
        assert!(nup.contains("rebuild  ok"), "{nup}");
        assert!(nup.contains("not P3N"), "{nup}");
        assert!(!nup.contains("receipt"), "{nup}");
        assert!(!nup.contains("theorem"), "{nup}");
        assert!(!nup.contains("ELECTROMAGNETIC"), "{nup}");
        assert!(!nup.contains("PHYSICOCHEMICAL"), "{nup}");
        assert!(!nup.contains("UNIVERSAL"), "{nup}");
        let nup_id = constant_node_id(&nup);
        assert_eq!(
            nup_id.to_hex(),
            "3b73f5c2000281e562fdf12b64d9bf7b36515105b92a3fee0e31906d0b7cb043",
            "journaling must not change the nu_p constant payload"
        );
        assert_eq!(
            lab.store.get(nup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let auf = lab
            ...
            .text()
            .to_string();
        assert!(auf.contains("constant  au_F  node "), "{auf}");
        assert!(
            auf.contains(
                "hash     c5d5a76de08e86fe094f04e0619baf65676bf1710b24d59a1f6097d180966bc5"
            ),
            "{auf}"
        );
        assert!(auf.contains("kind     interval"), "{auf}");
        assert!(auf.contains("table    XXXIV"), "{auf}");
        assert!(auf.contains("range    au_F = 8.2387234983(12)e-8"), "{auf}");
        assert!(auf.contains("unit     N"), "{auf}");
        assert!(
            auf.contains(
                "value    [82387234971/1000000000000000000, 16477446999/200000000000000000]"
            ),
            "{auf}"
        );
        assert!(auf.contains("rebuild  ok"), "{auf}");
        assert!(auf.contains("not P3N"), "{auf}");
        assert!(!auf.contains("receipt"), "{auf}");
        assert!(!auf.contains("theorem"), "{auf}");
        assert!(!auf.contains("ELECTROMAGNETIC"), "{auf}");
        assert!(!auf.contains("PHYSICOCHEMICAL"), "{auf}");
        assert!(!auf.contains("UNIVERSAL"), "{auf}");
        let auf_id = constant_node_id(&auf);
        assert_eq!(
            auf_id.to_hex(),
            "955545825f8e26d3e1ab94353d171f043cd712a5f4d1be9c3493dda6fb9be701",
            "journaling must not change the au_F constant payload"
        );
        assert_eq!(
            lab.store.get(auf_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let auv = lab
            ...
            .text()
            .to_string();
        assert!(auv.contains("constant  au_v  node "), "{auv}");
        assert!(
            auv.contains(
                "hash     5b8ea3a788076f8159c305e8a97f324a80989c29b6c8fee509a3e15714417ba9"
            ),
            "{auv}"
        );
        assert!(auv.contains("kind     interval"), "{auv}");
        assert!(auv.contains("table    XXXIV"), "{auv}");
        assert!(auv.contains("range    au_v = 2.18769126364(33)e6"), "{auv}");
        assert!(auv.contains("unit     m s^{-1}"), "{auv}");
        assert!(
            auv.contains("value    [218769126331/100000, 218769126397/100000]"),
            "{auv}"
        );
        assert!(auv.contains("rebuild  ok"), "{auv}");
        assert!(auv.contains("not P3N"), "{auv}");
        assert!(!auv.contains("receipt"), "{auv}");
        assert!(!auv.contains("theorem"), "{auv}");
        assert!(!auv.contains("ELECTROMAGNETIC"), "{auv}");
        assert!(!auv.contains("PHYSICOCHEMICAL"), "{auv}");
        assert!(!auv.contains("UNIVERSAL"), "{auv}");
        let auv_id = constant_node_id(&auv);
        assert_eq!(
            auv_id.to_hex(),
            "9593f075fe301bc700285a704f9f5645120b1a9716362313d1bb2c7a2eb743da",
            "journaling must not change the au_v constant payload"
        );
        assert_eq!(
            lab.store.get(auv_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let aup = lab
            ...
            .text()
            .to_string();
        assert!(aup.contains("constant  au_p  node "), "{aup}");
        assert!(
            aup.contains(
                "hash     f12708bfd5d8f16fd214f8c636edb390aa238130bc694a67e131d02cf08953b0"
            ),
            "{aup}"
        );
        assert!(aup.contains("kind     interval"), "{aup}");
        assert!(aup.contains("table    XXXIV"), "{aup}");
        assert!(
            aup.contains("range    au_p = 1.99285191410(30)e-24"),
            "{aup}"
        );
        assert!(aup.contains("unit     kg m s^{-1}"), "{aup}");
        assert!(
            aup.contains("value    [9964259569/5000000000000000000000000000000000, 2491064893/1250000000000000000000000000000000]"),
            "{aup}"
        );
        assert!(aup.contains("rebuild  ok"), "{aup}");
        assert!(aup.contains("not P3N"), "{aup}");
        assert!(!aup.contains("receipt"), "{aup}");
        assert!(!aup.contains("theorem"), "{aup}");
        assert!(!aup.contains("ELECTROMAGNETIC"), "{aup}");
        assert!(!aup.contains("PHYSICOCHEMICAL"), "{aup}");
        assert!(!aup.contains("UNIVERSAL"), "{aup}");
        let aup_id = constant_node_id(&aup);
        assert_eq!(
            aup_id.to_hex(),
            "7fbd9d2b00b5cc4e224ca5519bde5a590cb7aedc0fc0b1020c13d79140adb7fa",
            "journaling must not change the au_p constant payload"
        );
        assert_eq!(
            lab.store.get(aup_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        let nut = lab
            ...
            .text()
            .to_string();
        assert!(nut.contains("constant  nu_t  node "), "{nut}");
        assert!(
            nut.contains(
                "hash     eb796d28a29182f7211327fb07280d4b126e1ea949e19f0fb66575476dad80a5"
            ),
            "{nut}"
        );
        assert!(nut.contains("kind     interval"), "{nut}");
        assert!(nut.contains("table    XXXIV"), "{nut}");
        assert!(
            nut.contains("range    nu_t = 1.28808866819(39)e-21"),
            "{nut}"
        );
        assert!(nut.contains("unit     s"), "{nut}");
        assert!(
            nut.contains("value    [6440443339/5000000000000000000000000000000, 64404433429/50000000000000000000000000000000]"),
            "{nut}"
        );
        assert!(nut.contains("rebuild  ok"), "{nut}");
        assert!(nut.contains("not P3N"), "{nut}");
        assert!(!nut.contains("receipt"), "{nut}");
        assert!(!nut.contains("theorem"), "{nut}");
        assert!(!nut.contains("ELECTROMAGNETIC"), "{nut}");
        assert!(!nut.contains("PHYSICOCHEMICAL"), "{nut}");
        assert!(!nut.contains("UNIVERSAL"), "{nut}");
        let nut_id = constant_node_id(&nut);
        assert_eq!(
            nut_id.to_hex(),
            "2b50e30ecc5e7a5f893e51c261d0be10a8ffb8ec0976622f55c40787d527359b",
            "journaling must not change the nu_t constant payload"
        );
        assert_eq!(
            lab.store.get(nut_id).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        ...
        assert!(
            unknown_me.text().contains("unknown constant 'me'"),
            "{}",
            unknown_me.text()
        );

        let unknown_nup = lab.exec(Command::Constant {
            name: Some("nup".into()),
        });
        assert_eq!(unknown_nup.exit_code(), 1, "{}", unknown_nup.text());
        assert!(
            unknown_nup.text().contains("unknown constant 'nup'"),
            "{}",
            unknown_nup.text()
        );

        let unknown_auf = lab.exec(Command::Constant {
            name: Some("auf".into()),
        });
        assert_eq!(unknown_auf.exit_code(), 1, "{}", unknown_auf.text());
        assert!(
            unknown_auf.text().contains("unknown constant 'auf'"),
            "{}",
            unknown_auf.text()
        );

        let unknown_auv = lab.exec(Command::Constant {
            name: Some("auv".into()),
        });
        assert_eq!(unknown_auv.exit_code(), 1, "{}", unknown_auv.text());
        assert!(
        ...
        let unknown_aup = lab.exec(Command::Constant {
            name: Some("aup".into()),
        });
        assert_eq!(unknown_aup.exit_code(), 1, "{}", unknown_aup.text());
        assert!(
            unknown_aup.text().contains("unknown constant 'aup'"),
            "{}",
            unknown_aup.text()
        );

        let unknown_nut = lab.exec(Command::Constant {
            name: Some("nut".into()),
        });
        assert_eq!(unknown_nut.exit_code(), 1, "{}", unknown_nut.text());
        assert!(
            unknown_nut.text().contains("unknown constant 'nut'"),
            "{}",
            unknown_nut.text()
        );
        ...
        let unknown_g_mu = lab.exec(Command::Constant {
            name: Some("g_mu".into()),
        });
        assert_eq!(unknown_g_mu.exit_code(), 1, "{}", unknown_g_mu.text());
        assert!(
            unknown_g_mu.text().contains("unknown constant 'g_mu'"),
            "{}",
            unknown_g_mu.text()
        );

        let unknown_g_dash_mu = lab.exec(Command::Constant {
            name: Some("g-mu".into()),
        });
        assert_eq!(
            unknown_g_dash_mu.exit_code(),
            1,
            "{}",
            unknown_g_dash_mu.text()
        );
        assert!(
            unknown_g_dash_mu.text().contains("unknown constant 'g-mu'"),
            "{}",
            unknown_g_dash_mu.text()
        );

        let unknown_gmu_e = lab.exec(Command::Constant {
            name: Some("gmu_e".into()),
        });
        assert_eq!(unknown_gmu_e.exit_code(), 1, "{}", unknown_gmu_e.text());
        assert!(
            unknown_gmu_e.text().contains("unknown constant 'gmu_e'"),
            "{}",
            unknown_gmu_e.text()
        );

        let unknown_mumu_mup = lab.exec(Command::Constant {
            name: Some("mumu_mup".into()),
        });
        assert_eq!(
            unknown_mumu_mup.exit_code(),
            1,
            "{}",
            unknown_mumu_mup.text()
        );
        assert!(
            unknown_mumu_mup
                .text()
                .contains("unknown constant 'mumu_mup'"),
            "{}",
            unknown_mumu_mup.text()
        );

        let unknown_mu_mu_slash_mup = lab.exec(Command::Constant {
            name: Some("mu_mu/mup".into()),
        });
        assert_eq!(
            unknown_mu_mu_slash_mup.exit_code(),
            1,
            "{}",
            unknown_mu_mu_slash_mup.text()
        );
        assert!(
            unknown_mu_mu_slash_mup
                .text()
                .contains("unknown constant 'mu_mu/mup'"),
            "{}",
            unknown_mu_mu_slash_mup.text()
        );

        let unknown_mu_dash_mu_mup = lab.exec(Command::Constant {
            name: Some("mu-mu-mup".into()),
        });
        assert_eq!(
            unknown_mu_dash_mu_mup.exit_code(),
            1,
            "{}",
            unknown_mu_dash_mu_mup.text()
        );
        assert!(
            unknown_mu_dash_mu_mup
                .text()
                .contains("unknown constant 'mu-mu-mup'"),
            "{}",
            unknown_mu_dash_mu_mup.text()
        );

        let unknown_mp_u = lab.exec(Command::Constant {
            name: Some("mp_u".into()),
        });
        assert_eq!(unknown_mp_u.exit_code(), 1, "{}", unknown_mp_u.text());
        assert!(
            unknown_mp_u.text().contains("unknown constant 'mp_u'"),
            "{}",
            unknown_mp_u.text()
        );

        let unknown_m_p_slash_u = lab.exec(Command::Constant {
            name: Some("m_p/u".into()),
        });
        assert_eq!(
            unknown_m_p_slash_u.exit_code(),
            1,
            "{}",
            unknown_m_p_slash_u.text()
        );
        assert!(
            unknown_m_p_slash_u
                .text()
                .contains("unknown constant 'm_p/u'"),
            "{}",
            unknown_m_p_slash_u.text()
        );

        let unknown_m_p_hyphen_u = lab.exec(Command::Constant {
            name: Some("m-p-u".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_u.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_u.text()
        );
        assert!(
            unknown_m_p_hyphen_u
                .text()
                .contains("unknown constant 'm-p-u'"),
            "{}",
            unknown_m_p_hyphen_u.text()
        );

        let unknown_ar_p = lab.exec(Command::Constant {
            name: Some("Ar_p".into()),
        });
        assert_eq!(unknown_ar_p.exit_code(), 1, "{}", unknown_ar_p.text());
        assert!(
            unknown_ar_p.text().contains("unknown constant 'Ar_p'"),
            "{}",
            unknown_ar_p.text()
        );

        let unknown_mpc2 = lab.exec(Command::Constant {
            name: Some("mpc2".into()),
        });
        assert_eq!(unknown_mpc2.exit_code(), 1, "{}", unknown_mpc2.text());
        assert!(
            unknown_mpc2.text().contains("unknown constant 'mpc2'"),
            "{}",
            unknown_mpc2.text()
        );

        let unknown_m_p_slash_c2 = lab.exec(Command::Constant {
            name: Some("m_p/c2".into()),
        });
        assert_eq!(
            unknown_m_p_slash_c2.exit_code(),
            1,
            "{}",
            unknown_m_p_slash_c2.text()
        );
        assert!(
            unknown_m_p_slash_c2
                .text()
                .contains("unknown constant 'm_p/c2'"),
            "{}",
            unknown_m_p_slash_c2.text()
        );

        let unknown_m_p_hyphen_c2 = lab.exec(Command::Constant {
            name: Some("m-p-c2".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_c2.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_c2.text()
        );
        assert!(
            unknown_m_p_hyphen_c2
                .text()
                .contains("unknown constant 'm-p-c2'"),
            "{}",
            unknown_m_p_hyphen_c2.text()
        );

        let unknown_mpc2_mev = lab.exec(Command::Constant {
            name: Some("mpc2_MeV".into()),
        });
        assert_eq!(
            unknown_mpc2_mev.exit_code(),
            1,
            "{}",
            unknown_mpc2_mev.text()
        );
        assert!(
            unknown_mpc2_mev
                .text()
                .contains("unknown constant 'mpc2_MeV'"),
            "{}",
            unknown_mpc2_mev.text()
        );

        let unknown_m_p_c2_slash_mev = lab.exec(Command::Constant {
            name: Some("m_p_c2/MeV".into()),
        });
        assert_eq!(
            unknown_m_p_c2_slash_mev.exit_code(),
            1,
            "{}",
            unknown_m_p_c2_slash_mev.text()
        );
        assert!(
            unknown_m_p_c2_slash_mev
                .text()
                .contains("unknown constant 'm_p_c2/MeV'"),
            "{}",
            unknown_m_p_c2_slash_mev.text()
        );

        let unknown_m_p_hyphen_c2_mev = lab.exec(Command::Constant {
            name: Some("m-p-c2-MeV".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_c2_mev.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_c2_mev.text()
        );
        assert!(
            unknown_m_p_hyphen_c2_mev
                .text()
                .contains("unknown constant 'm-p-c2-MeV'"),
            "{}",
            unknown_m_p_hyphen_c2_mev.text()
        );

        let unknown_m_p_me = lab.exec(Command::Constant {
            name: Some("m_p_me".into()),
        });
        assert_eq!(unknown_m_p_me.exit_code(), 1, "{}", unknown_m_p_me.text());
        assert!(
            unknown_m_p_me.text().contains("unknown constant 'm_p_me'"),
            "{}",
            unknown_m_p_me.text()
        );

        let unknown_mp_slash_me = lab.exec(Command::Constant {
            name: Some("mp/me".into()),
        });
        assert_eq!(
            unknown_mp_slash_me.exit_code(),
            1,
            "{}",
            unknown_mp_slash_me.text()
        );
        assert!(
            unknown_mp_slash_me
                .text()
                .contains("unknown constant 'mp/me'"),
            "{}",
            unknown_mp_slash_me.text()
        );

        let unknown_m_p_hyphen_me = lab.exec(Command::Constant {
            name: Some("m-p-me".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_me.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_me.text()
        );
        assert!(
            unknown_m_p_hyphen_me
                .text()
                .contains("unknown constant 'm-p-me'"),
            "{}",
            unknown_m_p_hyphen_me.text()
        );

        let unknown_m_p_mmu = lab.exec(Command::Constant {
            name: Some("m_p_mmu".into()),
        });
        assert_eq!(unknown_m_p_mmu.exit_code(), 1, "{}", unknown_m_p_mmu.text());
        assert!(
            unknown_m_p_mmu
                .text()
                .contains("unknown constant 'm_p_mmu'"),
            "{}",
            unknown_m_p_mmu.text()
        );

        let unknown_mp_slash_mmu = lab.exec(Command::Constant {
            name: Some("mp/mmu".into()),
        });
        assert_eq!(
            unknown_mp_slash_mmu.exit_code(),
            1,
            "{}",
            unknown_mp_slash_mmu.text()
        );
        assert!(
            unknown_mp_slash_mmu
                .text()
                .contains("unknown constant 'mp/mmu'"),
            "{}",
            unknown_mp_slash_mmu.text()
        );

        let unknown_m_p_hyphen_mmu = lab.exec(Command::Constant {
            name: Some("m-p-mmu".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_mmu.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_mmu.text()
        );
        assert!(
            unknown_m_p_hyphen_mmu
                .text()
                .contains("unknown constant 'm-p-mmu'"),
            "{}",
            unknown_m_p_hyphen_mmu.text()
        );

        let unknown_mp_mtau = lab.exec(Command::Constant {
            name: Some("mp_mtau".into()),
        });
        assert_eq!(unknown_mp_mtau.exit_code(), 1, "{}", unknown_mp_mtau.text());
        assert!(
            unknown_mp_mtau
                .text()
                .contains("unknown constant 'mp_mtau'"),
            "{}",
            unknown_mp_mtau.text()
        );

        let unknown_m_p_mn = lab.exec(Command::Constant {
            name: Some("m_p_mn".into()),
        });
        assert_eq!(unknown_m_p_mn.exit_code(), 1, "{}", unknown_m_p_mn.text());
        assert!(
            unknown_m_p_mn.text().contains("unknown constant 'm_p_mn'"),
            "{}",
            unknown_m_p_mn.text()
        );

        let unknown_mp_slash_mn = lab.exec(Command::Constant {
            name: Some("mp/mn".into()),
        });
        assert_eq!(
            unknown_mp_slash_mn.exit_code(),
            1,
            "{}",
            unknown_mp_slash_mn.text()
        );
        assert!(
            unknown_mp_slash_mn
                .text()
                .contains("unknown constant 'mp/mn'"),
            "{}",
            unknown_mp_slash_mn.text()
        );

        let unknown_m_p_hyphen_mn = lab.exec(Command::Constant {
            name: Some("m-p-mn".into()),
        });
        assert_eq!(
            unknown_m_p_hyphen_mn.exit_code(),
            1,
            "{}",
            unknown_m_p_hyphen_mn.text()
        );
        assert!(
            unknown_m_p_hyphen_mn
                .text()
                .contains("unknown constant 'm-p-mn'"),
            "{}",
            unknown_m_p_hyphen_mn.text()
        );

        let unknown_e_slash_mp = lab.exec(Command::Constant {
            name: Some("e/mp".into()),
        });
        assert_eq!(
            unknown_e_slash_mp.exit_code(),
            1,
            "{}",
            unknown_e_slash_mp.text()
        );
        assert!(
            unknown_e_slash_mp
                .text()
                .contains("unknown constant 'e/mp'"),
            "{}",
            unknown_e_slash_mp.text()
        );

        let unknown_e_hyphen_mp = lab.exec(Command::Constant {
            name: Some("e-mp".into()),
        });
        assert_eq!(
            unknown_e_hyphen_mp.exit_code(),
            1,
            "{}",
            unknown_e_hyphen_mp.text()
        );
        assert!(
            unknown_e_hyphen_mp
                .text()
        ...
        let unknown_minus_e_mp = lab.exec(Command::Constant {
            name: Some("-e/mp".into()),
        });
        assert_eq!(
            unknown_minus_e_mp.exit_code(),
            1,
            "{}",
            unknown_minus_e_mp.text()
        );
        assert!(
            unknown_minus_e_mp
                .text()
        ...
        let unknown_mp_plain = lab.exec(Command::Constant {
            name: Some("Mp".into()),
        });
        assert_eq!(
            unknown_mp_plain.exit_code(),
            1,
            "{}",
            unknown_mp_plain.text()
        );
        assert!(
            unknown_mp_plain.text().contains("unknown constant 'Mp'"),
            "{}",
            unknown_mp_plain.text()
        );

        let unknown_m_hyphen_p = lab.exec(Command::Constant {
            name: Some("M-p".into()),
        });
        assert_eq!(
            unknown_m_hyphen_p.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_p.text()
        );
        assert!(
            unknown_m_hyphen_p.text().contains("unknown constant 'M-p'"),
            "{}",
            unknown_m_hyphen_p.text()
        );

        let unknown_m_p_mol = lab.exec(Command::Constant {
            name: Some("M_p/mol".into()),
        });
        assert_eq!(unknown_m_p_mol.exit_code(), 1, "{}", unknown_m_p_mol.text());
        assert!(
            unknown_m_p_mol
                .text()
                .contains("unknown constant 'M_p/mol'"),
            "{}",
            unknown_m_p_mol.text()
        );

        let unknown_lambdabar_c_p = lab.exec(Command::Constant {
            name: Some("lambdabar_C_p".into()),
        });
        assert_eq!(
            unknown_lambdabar_c_p.exit_code(),
            1,
            "{}",
            unknown_lambdabar_c_p.text()
        );
        assert!(
            unknown_lambdabar_c_p
                .text()
                .contains("unknown constant 'lambdabar_C_p'"),
            "{}",
            unknown_lambdabar_c_p.text()
        );

        let unknown_lambdacp = lab.exec(Command::Constant {
            name: Some("lambdaCp".into()),
        });
        assert_eq!(
            unknown_lambdacp.exit_code(),
            1,
            "{}",
            unknown_lambdacp.text()
        );
        assert!(
            unknown_lambdacp
                .text()
                .contains("unknown constant 'lambdaCp'"),
            "{}",
            unknown_lambdacp.text()
        );

        let unknown_lambda_c_comma_p = lab.exec(Command::Constant {
            name: Some("lambda_C,p".into()),
        });
        assert_eq!(
            unknown_lambda_c_comma_p.exit_code(),
            1,
            "{}",
            unknown_lambda_c_comma_p.text()
        );
        assert!(
            unknown_lambda_c_comma_p
                .text()
                .contains("unknown constant 'lambda_C,p'"),
            "{}",
            unknown_lambda_c_comma_p.text()
        );

        let unknown_r_p = lab.exec(Command::Constant {
            name: Some("r_p".into()),
        });
        assert_eq!(unknown_r_p.exit_code(), 1, "{}", unknown_r_p.text());
        assert!(
            unknown_r_p.text().contains("unknown constant 'r_p'"),
            "{}",
            unknown_r_p.text()
        );

        let unknown_r_hyphen_p = lab.exec(Command::Constant {
            name: Some("r-p".into()),
        });
        assert_eq!(
            unknown_r_hyphen_p.exit_code(),
            1,
            "{}",
            unknown_r_hyphen_p.text()
        );
        assert!(
            unknown_r_hyphen_p.text().contains("unknown constant 'r-p'"),
            "{}",
            unknown_r_hyphen_p.text()
        );

        let unknown_rp_plain = lab.exec(Command::Constant {
            name: Some("Rp".into()),
        });
        assert_eq!(
            unknown_rp_plain.exit_code(),
            1,
            "{}",
            unknown_rp_plain.text()
        );
        assert!(
            unknown_rp_plain.text().contains("unknown constant 'Rp'"),
            "{}",
            unknown_rp_plain.text()
        );

        let unknown_mup = lab.exec(Command::Constant {
            name: Some("mup".into()),
        });
        assert_eq!(unknown_mup.exit_code(), 1, "{}", unknown_mup.text());
        assert!(
            unknown_mup.text().contains("unknown constant 'mup'"),
            "{}",
            unknown_mup.text()
        );

        let unknown_mu_hyphen_p = lab.exec(Command::Constant {
            name: Some("mu-p".into()),
        });
        assert_eq!(
            unknown_mu_hyphen_p.exit_code(),
            1,
            "{}",
            unknown_mu_hyphen_p.text()
        );
        assert!(
            unknown_mu_hyphen_p
                .text()
                .contains("unknown constant 'mu-p'"),
            "{}",
            unknown_mu_hyphen_p.text()
        );

        let unknown_mu_p_ = lab.exec(Command::Constant {
            name: Some("mu_p_".into()),
        });
        assert_eq!(unknown_mu_p_.exit_code(), 1, "{}", unknown_mu_p_.text());
        assert!(
            unknown_mu_p_.text().contains("unknown constant 'mu_p_'"),
            "{}",
            unknown_mu_p_.text()
        );

        let unknown_mup_mu_b = lab.exec(Command::Constant {
            name: Some("mup_muB".into()),
        });
        assert_eq!(
            unknown_mup_mu_b.exit_code(),
            1,
            "{}",
            unknown_mup_mu_b.text()
        );
        assert!(
            unknown_mup_mu_b
                .text()
                .contains("unknown constant 'mup_muB'"),
            "{}",
            unknown_mup_mu_b.text()
        );

        let unknown_mu_p_slash = lab.exec(Command::Constant {
            name: Some("mu_p/muB".into()),
        });
        assert_eq!(
            unknown_mu_p_slash.exit_code(),
            1,
            "{}",
            unknown_mu_p_slash.text()
        );
        assert!(
            unknown_mu_p_slash
                .text()
                .contains("unknown constant 'mu_p/muB'"),
            "{}",
            unknown_mu_p_slash.text()
        );

        let unknown_mu_p_hyphen = lab.exec(Command::Constant {
            name: Some("mu-p-muB".into()),
        });
        assert_eq!(
            unknown_mu_p_hyphen.exit_code(),
            1,
            "{}",
            unknown_mu_p_hyphen.text()
        );
        assert!(
            unknown_mu_p_hyphen
                .text()
                .contains("unknown constant 'mu-p-muB'"),
            "{}",
            unknown_mu_p_hyphen.text()
        );

        let unknown_mup_mu_n = lab.exec(Command::Constant {
            name: Some("mup_muN".into()),
        });
        assert_eq!(
            unknown_mup_mu_n.exit_code(),
            1,
            "{}",
            unknown_mup_mu_n.text()
        );
        assert!(
            unknown_mup_mu_n
                .text()
                .contains("unknown constant 'mup_muN'"),
            "{}",
            unknown_mup_mu_n.text()
        );

        let unknown_mu_p_slash_n = lab.exec(Command::Constant {
            name: Some("mu_p/muN".into()),
        });
        assert_eq!(
            unknown_mu_p_slash_n.exit_code(),
            1,
            "{}",
            unknown_mu_p_slash_n.text()
        );
        assert!(
            unknown_mu_p_slash_n
                .text()
                .contains("unknown constant 'mu_p/muN'"),
            "{}",
            unknown_mu_p_slash_n.text()
        );

        let unknown_mu_p_hyphen_n = lab.exec(Command::Constant {
            name: Some("mu-p-muN".into()),
        });
        assert_eq!(
            unknown_mu_p_hyphen_n.exit_code(),
            1,
            "{}",
            unknown_mu_p_hyphen_n.text()
        );
        assert!(
            unknown_mu_p_hyphen_n
                .text()
                .contains("unknown constant 'mu-p-muN'"),
            "{}",
            unknown_mu_p_hyphen_n.text()
        );

        let unknown_g_p = lab.exec(Command::Constant {
            name: Some("g_p".into()),
        });
        assert_eq!(unknown_g_p.exit_code(), 1, "{}", unknown_g_p.text());
        assert!(
            unknown_g_p.text().contains("unknown constant 'g_p'"),
            "{}",
            unknown_g_p.text()
        );

        let unknown_g_hyphen_p = lab.exec(Command::Constant {
            name: Some("g-p".into()),
        });
        assert_eq!(
            unknown_g_hyphen_p.exit_code(),
            1,
            "{}",
            unknown_g_hyphen_p.text()
        );
        assert!(
            unknown_g_hyphen_p.text().contains("unknown constant 'g-p'"),
            "{}",
            unknown_g_hyphen_p.text()
        );

        let unknown_gp_plain = lab.exec(Command::Constant {
            name: Some("Gp".into()),
        });
        assert_eq!(
            unknown_gp_plain.exit_code(),
            1,
            "{}",
            unknown_gp_plain.text()
        );
        assert!(
            unknown_gp_plain.text().contains("unknown constant 'Gp'"),
            "{}",
            unknown_gp_plain.text()
        );

        let unknown_mup_mun = lab.exec(Command::Constant {
            name: Some("mup_mun".into()),
        });
        assert_eq!(unknown_mup_mun.exit_code(), 1, "{}", unknown_mup_mun.text());
        assert!(
            unknown_mup_mun
                .text()
                .contains("unknown constant 'mup_mun'"),
            "{}",
            unknown_mup_mun.text()
        );
        ...
        let unknown_mu_p_hyphen_mun = lab.exec(Command::Constant {
            name: Some("mu-p-mun".into()),
        });
        assert_eq!(
            unknown_mu_p_hyphen_mun.exit_code(),
            1,
            "{}",
            unknown_mu_p_hyphen_mun.text()
        );
        assert!(
            unknown_mu_p_hyphen_mun
                .text()
                .contains("unknown constant 'mu-p-mun'"),
            "{}",
            unknown_mu_p_hyphen_mun.text()
        ...
        let unknown_mu_0p = lab.exec(Command::Constant {
            name: Some("mu_0p".into()),
        });
        assert_eq!(unknown_mu_0p.exit_code(), 1, "{}", unknown_mu_0p.text());
        assert!(
            unknown_mu_0p.text().contains("unknown constant 'mu_0p'"),
            "{}",
            unknown_mu_0p.text()
        );

        let unknown_mu_hyphen_0p = lab.exec(Command::Constant {
            name: Some("mu-0p".into()),
        });
        assert_eq!(
            unknown_mu_hyphen_0p.exit_code(),
            1,
            "{}",
            unknown_mu_hyphen_0p.text()
        );
        assert!(
            unknown_mu_hyphen_0p
                .text()
                .contains("unknown constant 'mu-0p'"),
            "{}",
            unknown_mu_hyphen_0p.text()
        );

        let unknown_mu0_p = lab.exec(Command::Constant {
            name: Some("mu0_p".into()),
        });
        assert_eq!(unknown_mu0_p.exit_code(), 1, "{}", unknown_mu0_p.text());
        assert!(
            unknown_mu0_p.text().contains("unknown constant 'mu0_p'"),
            "{}",
            unknown_mu0_p.text()
        );

        let unknown_mu0p_slash_mub = lab.exec(Command::Constant {
            name: Some("mu0p/muB".into()),
        });
        assert_eq!(
            unknown_mu0p_slash_mub.exit_code(),
            1,
            "{}",
            unknown_mu0p_slash_mub.text()
        );
        assert!(
            unknown_mu0p_slash_mub
                .text()
                .contains("unknown constant 'mu0p/muB'"),
            "{}",
            unknown_mu0p_slash_mub.text()
        );

        let unknown_mu_hyphen_0p_mub = lab.exec(Command::Constant {
            name: Some("mu-0p-muB".into()),
        });
        assert_eq!(
            unknown_mu_hyphen_0p_mub.exit_code(),
            1,
            "{}",
            unknown_mu_hyphen_0p_mub.text()
        );
        assert!(
            unknown_mu_hyphen_0p_mub
                .text()
                .contains("unknown constant 'mu-0p-muB'"),
            "{}",
            unknown_mu_hyphen_0p_mub.text()
        );

        let unknown_mu0p_mub = lab.exec(Command::Constant {
            name: Some("mu0p_mub".into()),
        });
        assert_eq!(
            unknown_mu0p_mub.exit_code(),
            1,
            "{}",
            unknown_mu0p_mub.text()
        );
        assert!(
            unknown_mu0p_mub
                .text()
                .contains("unknown constant 'mu0p_mub'"),
            "{}",
            unknown_mu0p_mub.text()
        );

        let unknown_mu0p_slash_mun = lab.exec(Command::Constant {
            name: Some("mu0p/muN".into()),
        });
        assert_eq!(
            unknown_mu0p_slash_mun.exit_code(),
            1,
            "{}",
            unknown_mu0p_slash_mun.text()
        );
        assert!(
            unknown_mu0p_slash_mun
                .text()
                .contains("unknown constant 'mu0p/muN'"),
            "{}",
            unknown_mu0p_slash_mun.text()
        );

        let unknown_mu_hyphen_0p_mun = lab.exec(Command::Constant {
            name: Some("mu-0p-muN".into()),
        });
        assert_eq!(
            unknown_mu_hyphen_0p_mun.exit_code(),
            1,
            "{}",
            unknown_mu_hyphen_0p_mun.text()
        );
        assert!(
            unknown_mu_hyphen_0p_mun
                .text()
                .contains("unknown constant 'mu-0p-muN'"),
            "{}",
            unknown_mu_hyphen_0p_mun.text()
        );

        let unknown_mu0p_mun = lab.exec(Command::Constant {
            name: Some("mu0p_mun".into()),
        });
        assert_eq!(
            unknown_mu0p_mun.exit_code(),
            1,
            "{}",
            unknown_mu0p_mun.text()
        );
        assert!(
            unknown_mu0p_mun
                .text()
                .contains("unknown constant 'mu0p_mun'"),
            "{}",
            unknown_mu0p_mun.text()
        );

        let unknown_sigma_p = lab.exec(Command::Constant {
            name: Some("sigma_p".into()),
        });
        assert_eq!(unknown_sigma_p.exit_code(), 1, "{}", unknown_sigma_p.text());
        assert!(
            unknown_sigma_p
                .text()
                .contains("unknown constant 'sigma_p'"),
            "{}",
            unknown_sigma_p.text()
        );

        let unknown_sigma_hyphen_0p = lab.exec(Command::Constant {
            name: Some("sigma-0p".into()),
        });
        assert_eq!(
            unknown_sigma_hyphen_0p.exit_code(),
            1,
            "{}",
            unknown_sigma_hyphen_0p.text()
        );
        assert!(
            unknown_sigma_hyphen_0p
                .text()
                .contains("unknown constant 'sigma-0p'"),
            "{}",
            unknown_sigma_hyphen_0p.text()
        );

        let unknown_sigma0_p = lab.exec(Command::Constant {
            name: Some("sigma0_p".into()),
        });
        assert_eq!(
            unknown_sigma0_p.exit_code(),
            1,
            "{}",
            unknown_sigma0_p.text()
        );
        assert!(
            unknown_sigma0_p
                .text()
                .contains("unknown constant 'sigma0_p'"),
            "{}",
            unknown_sigma0_p.text()
        );

        let unknown_sigmap = lab.exec(Command::Constant {
            name: Some("sigmap".into()),
        });
        assert_eq!(unknown_sigmap.exit_code(), 1, "{}", unknown_sigmap.text());
        assert!(
            unknown_sigmap.text().contains("unknown constant 'sigmap'"),
            "{}",
            unknown_sigmap.text()
        );

        let unknown_mn = lab.exec(Command::Constant {
            name: Some("mn".into()),
        });
        assert_eq!(unknown_mn.exit_code(), 1, "{}", unknown_mn.text());
        assert!(
            unknown_mn.text().contains("unknown constant 'mn'"),
            "{}",
            unknown_mn.text()
        );

        let unknown_m_hyphen_n = lab.exec(Command::Constant {
            name: Some("m-n".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n.text()
        );
        assert!(
            unknown_m_hyphen_n.text().contains("unknown constant 'm-n'"),
            "{}",
            unknown_m_hyphen_n.text()
        );

        let unknown_neutron_mass = lab.exec(Command::Constant {
            name: Some("neutron-mass".into()),
        });
        assert_eq!(
            unknown_neutron_mass.exit_code(),
            1,
            "{}",
            unknown_neutron_mass.text()
        );
        assert!(
            unknown_neutron_mass
                .text()
                .contains("unknown constant 'neutron-mass'"),
            "{}",
            unknown_neutron_mass.text()
        );

        let unknown_mn_u = lab.exec(Command::Constant {
            name: Some("mn_u".into()),
        });
        assert_eq!(unknown_mn_u.exit_code(), 1, "{}", unknown_mn_u.text());
        assert!(
            unknown_mn_u.text().contains("unknown constant 'mn_u'"),
            "{}",
            unknown_mn_u.text()
        );

        let unknown_m_n_slash_u = lab.exec(Command::Constant {
            name: Some("m_n/u".into()),
        });
        assert_eq!(
            unknown_m_n_slash_u.exit_code(),
            1,
            "{}",
            unknown_m_n_slash_u.text()
        );
        assert!(
            unknown_m_n_slash_u
                .text()
                .contains("unknown constant 'm_n/u'"),
            "{}",
            unknown_m_n_slash_u.text()
        );

        let unknown_m_hyphen_n_u = lab.exec(Command::Constant {
            name: Some("m-n-u".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_u.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_u.text()
        );
        assert!(
            unknown_m_hyphen_n_u
                .text()
                .contains("unknown constant 'm-n-u'"),
            "{}",
            unknown_m_hyphen_n_u.text()
        );

        let unknown_ar_n = lab.exec(Command::Constant {
            name: Some("Ar_n".into()),
        });
        assert_eq!(unknown_ar_n.exit_code(), 1, "{}", unknown_ar_n.text());
        assert!(
            unknown_ar_n.text().contains("unknown constant 'Ar_n'"),
            "{}",
            unknown_ar_n.text()
        );

        let unknown_mnc2 = lab.exec(Command::Constant {
            name: Some("mnc2".into()),
        });
        assert_eq!(unknown_mnc2.exit_code(), 1, "{}", unknown_mnc2.text());
        assert!(
            unknown_mnc2.text().contains("unknown constant 'mnc2'"),
            "{}",
            unknown_mnc2.text()
        );

        let unknown_m_n_slash_c2 = lab.exec(Command::Constant {
            name: Some("m_n/c2".into()),
        });
        assert_eq!(
            unknown_m_n_slash_c2.exit_code(),
            1,
            "{}",
            unknown_m_n_slash_c2.text()
        );
        assert!(
            unknown_m_n_slash_c2
                .text()
                .contains("unknown constant 'm_n/c2'"),
            "{}",
            unknown_m_n_slash_c2.text()
        );

        let unknown_m_hyphen_n_c2 = lab.exec(Command::Constant {
            name: Some("m-n-c2".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_c2.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_c2.text()
        );
        assert!(
            unknown_m_hyphen_n_c2
                .text()
                .contains("unknown constant 'm-n-c2'"),
            "{}",
            unknown_m_hyphen_n_c2.text()
        );

        let unknown_mnc2_mev = lab.exec(Command::Constant {
            name: Some("mnc2_MeV".into()),
        });
        assert_eq!(
            unknown_mnc2_mev.exit_code(),
            1,
            "{}",
            unknown_mnc2_mev.text()
        );
        assert!(
            unknown_mnc2_mev
                .text()
                .contains("unknown constant 'mnc2_MeV'"),
            "{}",
            unknown_mnc2_mev.text()
        );

        let unknown_m_n_c2_slash_mev = lab.exec(Command::Constant {
            name: Some("m_n_c2/MeV".into()),
        });
        assert_eq!(
            unknown_m_n_c2_slash_mev.exit_code(),
            1,
            "{}",
            unknown_m_n_c2_slash_mev.text()
        );
        assert!(
            unknown_m_n_c2_slash_mev
                .text()
                .contains("unknown constant 'm_n_c2/MeV'"),
            "{}",
            unknown_m_n_c2_slash_mev.text()
        );

        let unknown_m_hyphen_n_c2_mev = lab.exec(Command::Constant {
            name: Some("m-n-c2-MeV".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_c2_mev.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_c2_mev.text()
        );
        assert!(
            unknown_m_hyphen_n_c2_mev
                .text()
                .contains("unknown constant 'm-n-c2-MeV'"),
            "{}",
            unknown_m_hyphen_n_c2_mev.text()
        );

        let unknown_mn_slash_me = lab.exec(Command::Constant {
            name: Some("mn/me".into()),
        });
        assert_eq!(
            unknown_mn_slash_me.exit_code(),
            1,
            "{}",
            unknown_mn_slash_me.text()
        );
        assert!(
            unknown_mn_slash_me
                .text()
                .contains("unknown constant 'mn/me'"),
            "{}",
            unknown_mn_slash_me.text()
        );

        let unknown_m_n_me = lab.exec(Command::Constant {
            name: Some("m_n_me".into()),
        });
        assert_eq!(unknown_m_n_me.exit_code(), 1, "{}", unknown_m_n_me.text());
        assert!(
            unknown_m_n_me.text().contains("unknown constant 'm_n_me'"),
            "{}",
            unknown_m_n_me.text()
        );

        let unknown_m_hyphen_n_me = lab.exec(Command::Constant {
            name: Some("m-n-me".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_me.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_me.text()
        );
        assert!(
            unknown_m_hyphen_n_me
                .text()
                .contains("unknown constant 'm-n-me'"),
            "{}",
            unknown_m_hyphen_n_me.text()
        );

        let unknown_mn_slash_mmu = lab.exec(Command::Constant {
            name: Some("mn/mmu".into()),
        });
        assert_eq!(
            unknown_mn_slash_mmu.exit_code(),
            1,
            "{}",
            unknown_mn_slash_mmu.text()
        );
        assert!(
            unknown_mn_slash_mmu
                .text()
                .contains("unknown constant 'mn/mmu'"),
            "{}",
            unknown_mn_slash_mmu.text()
        );

        let unknown_m_n_mmu = lab.exec(Command::Constant {
            name: Some("m_n_mmu".into()),
        });
        assert_eq!(unknown_m_n_mmu.exit_code(), 1, "{}", unknown_m_n_mmu.text());
        assert!(
            unknown_m_n_mmu
                .text()
                .contains("unknown constant 'm_n_mmu'"),
            "{}",
            unknown_m_n_mmu.text()
        );

        let unknown_m_hyphen_n_mmu = lab.exec(Command::Constant {
            name: Some("m-n-mmu".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_mmu.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_mmu.text()
        );
        assert!(
            unknown_m_hyphen_n_mmu
                .text()
                .contains("unknown constant 'm-n-mmu'"),
            "{}",
            unknown_m_hyphen_n_mmu.text()
        );

        let unknown_mn_slash_mp = lab.exec(Command::Constant {
            name: Some("mn/mp".into()),
        });
        assert_eq!(
            unknown_mn_slash_mp.exit_code(),
            1,
            "{}",
            unknown_mn_slash_mp.text()
        );
        assert!(
            unknown_mn_slash_mp
                .text()
                .contains("unknown constant 'mn/mp'"),
            "{}",
            unknown_mn_slash_mp.text()
        );

        let unknown_m_n_mp = lab.exec(Command::Constant {
            name: Some("m_n_mp".into()),
        });
        assert_eq!(unknown_m_n_mp.exit_code(), 1, "{}", unknown_m_n_mp.text());
        assert!(
            unknown_m_n_mp.text().contains("unknown constant 'm_n_mp'"),
            "{}",
            unknown_m_n_mp.text()
        );

        let unknown_m_hyphen_n_mp = lab.exec(Command::Constant {
            name: Some("m-n-mp".into()),
        });
        assert_eq!(
            unknown_m_hyphen_n_mp.exit_code(),
            1,
            "{}",
            unknown_m_hyphen_n_mp.text()
        );
        assert!(
            unknown_m_hyphen_n_mp
                .text()
                .contains("unknown constant 'm-n-mp'"),
            "{}",
            unknown_m_hyphen_n_mp.text()
        );

        let unknown_mn_hyphen_mp = lab.exec(Command::Constant {
            name: Some("mn-mp".into()),
        });
        assert_eq!(
            unknown_mn_hyphen_mp.exit_code(),
            1,
            "{}",
            unknown_mn_hyphen_mp.text()
        );
        assert!(
            unknown_mn_hyphen_mp
                .text()
                .contains("unknown constant 'mn-mp'"),
            "{}",
            unknown_mn_hyphen_mp.text()
        ...
        let unknown_m_n_minus_m_p = lab.exec(Command::Constant {
            name: Some("m_n-m_p".into()),
        });
        assert_eq!(
            unknown_m_n_minus_m_p.exit_code(),
            1,
            "{}",
            unknown_m_n_minus_m_p.text()
        );
        assert!(
            unknown_m_n_minus_m_p
                .text()
                .contains("unknown constant 'm_n-m_p'"),
            "{}",
            unknown_m_n_minus_m_p.text()
        );

        let unknown_m_n_minus_m_p_words = lab.exec(Command::Constant {
            name: Some("m_n_minus_m_p".into()),
        });
        assert_eq!(
            unknown_m_n_minus_m_p_words.exit_code(),
            1,
            "{}",
            unknown_m_n_minus_m_p_words.text()
        );
        assert!(
            unknown_m_n_minus_m_p_words
                .text()
                .contains("unknown constant 'm_n_minus_m_p'"),
            "{}",
            unknown_m_n_minus_m_p_words.text()
        );

        let unknown_mn_hyphen_mp_u = lab.exec(Command::Constant {
            name: Some("mn-mp_u".into()),
        });
        assert_eq!(
            unknown_mn_hyphen_mp_u.exit_code(),
            1,
            "{}",
            unknown_mn_hyphen_mp_u.text()
        );
        assert!(
            unknown_mn_hyphen_mp_u
                .text()
                .contains("unknown constant 'mn-mp_u'"),
            "{}",
            unknown_mn_hyphen_mp_u.text()
        );

        let unknown_mn_minus_mp_slash_u = lab.exec(Command::Constant {
            name: Some("mn_minus_mp/u".into()),
        });
        assert_eq!(
            unknown_mn_minus_mp_slash_u.exit_code(),
            1,
            "{}",
            unknown_mn_minus_mp_slash_u.text()
        );
        assert!(
            unknown_mn_minus_mp_slash_u
                .text()
                .contains("unknown constant 'mn_minus_mp/u'"),
            "{}",
            unknown_mn_minus_mp_slash_u.text()
        );

        let unknown_m_n_minus_m_p_u = lab.exec(Command::Constant {
            name: Some("m_n_minus_m_p_u".into()),
        });
        assert_eq!(
            unknown_m_n_minus_m_p_u.exit_code(),
            1,
            "{}",
            unknown_m_n_minus_m_p_u.text()
        );
        assert!(
            unknown_m_n_minus_m_p_u
                .text()
                .contains("unknown constant 'm_n_minus_m_p_u'"),
            "{}",
            unknown_m_n_minus_m_p_u.text()
        );

        let unknown_mn_hyphen_mp_c2 = lab.exec(Command::Constant {
            name: Some("mn-mp_c2".into()),
        });
        assert_eq!(
            unknown_mn_hyphen_mp_c2.exit_code(),
            1,
            "{}",
            unknown_mn_hyphen_mp_c2.text()
        );
        assert!(
            unknown_mn_hyphen_mp_c2
                .text()
                .contains("unknown constant 'mn-mp_c2'"),
            "{}",
            unknown_mn_hyphen_mp_c2.text()
        );

        let unknown_mn_minus_mp_slash_c2 = lab.exec(Command::Constant {
            name: Some("mn_minus_mp/c2".into()),
        });
        assert_eq!(
            unknown_mn_minus_mp_slash_c2.exit_code(),
            1,
            "{}",
            unknown_mn_minus_mp_slash_c2.text()
        );
        assert!(
            unknown_mn_minus_mp_slash_c2
                .text()
                .contains("unknown constant 'mn_minus_mp/c2'"),
            "{}",
            unknown_mn_minus_mp_slash_c2.text()
        );

        let unknown_m_n_minus_m_p_c2 = lab.exec(Command::Constant {
            name: Some("m_n_minus_m_p_c2".into()),
        });
        assert_eq!(
            unknown_m_n_minus_m_p_c2.exit_code(),
            1,
            "{}",
            unknown_m_n_minus_m_p_c2.text()
        );
        assert!(
            unknown_m_n_minus_m_p_c2
                .text()
                .contains("unknown constant 'm_n_minus_m_p_c2'"),
            "{}",
            unknown_m_n_minus_m_p_c2.text()
        );
        ...
        assert!(
            ledger.contains("range    gmu = -2.0023318418(13)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu_mu/mup = -3.183345142(71)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mp_u = 1.007276466621(53)"),
            "{ledger}"
        );
        assert!(
            ledger.contains("range    mpc2 = 1.50327761598(46)e-10"),
            "{ledger}"
        );
        assert!(
            ledger.contains("range    mpc2_MeV = 938.27208816(29)"),
            "{ledger}"
        );
        assert!(
            ledger.contains("range    mp/me = 1836.15267343(11)"),
            "{ledger}"
        );
        assert!(
            ledger.contains("range    mp/mmu = 8.88024337(20)"),
            "{ledger}"
        );
        assert!(
            ledger.contains("range    mp/mn = 0.99862347812(49)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    e/mp = 9.5788331560(29)e7"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    Mp = 1.00727646627(31)e-3"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    lambda_C_p = 1.32140985539(40)e-15"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(ledger.contains("range    rp = 8.414(19)e-16"), "{ledger}");
        assert!(
            ledger.contains(
                "hash     bf987c5fccc4ef40691f126024092320bc335b7942323fa90675a28a250e304c"
            ),
        ...
        assert!(
            ledger.contains("range    mu_p = 1.41060679736(60)e-26"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu_p/muB = 1.52103220230(46)e-3"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu_p/muN = 2.79284734463(82)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    gp = 5.5856946893(16)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu_p/mun = -1.45989805(34)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu0p = 1.410570560(15)e-26"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu0p/muB = 1.520993128(17)e-3"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mu0p/muN = 2.792775599(30)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    sigma0p = 2.5689(11)e-5"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn = 1.67492749804(95)e-27"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn_u = 1.00866491595(49)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mnc2 = 1.50534976287(86)e-10"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mnc2_MeV = 939.56542052(54)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn/me = 1838.68366173(89)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn/mmu = 8.89248406(20)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn/mp = 1.00137841931(49)"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn-mp = 2.30557435(82)e-30"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn-mp_u = 1.38844933(49)e-3"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
        ...
        assert!(
            ledger.contains("range    mn-mp_c2 = 2.07214689(74)e-13"),
            "{ledger}"
        );
        assert!(
            ledger.contains(
    ...
    fn versioned_constant_ledger_restores_by_rebuild_not_deserialize() {
        ...
        *lab2.journal_mut() = Journal::from_jsonl(&jsonl);
        ...
        let gmu = physis_core::artifact::ArtifactId::from_hex(
            "a810ed959ec82be1763f525714739871f182303850a02507d163f4e73fcc5996",
        )
        .expect("pinned gmu node");
        assert_eq!(
            lab2.store.get(gmu).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_mu_mup = physis_core::artifact::ArtifactId::from_hex(
            "6468f3033806b2fdcf78e2fbc7dfc7921b14f98b7a61b88127e4302c4d7fabff",
        )
        .expect("pinned mu_mu_mup node");
        assert_eq!(
            lab2.store.get(mu_mu_mup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_p_u = physis_core::artifact::ArtifactId::from_hex(
            "b7d7c12458d2829801236ee857190822650a55625bc19c759e683874869f9b6c",
        )
        .expect("pinned m_p_u node");
        assert_eq!(
            lab2.store.get(m_p_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_p_c2 = physis_core::artifact::ArtifactId::from_hex(
            "e242f8c1e541d797801325311916546d420c7a71b42303c94a0a7d060b8adde6",
        )
        .expect("pinned m_p_c2 node");
        assert_eq!(
            lab2.store.get(m_p_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_p_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "c44790b16220afd067ff78b6174045801f94d8412541b829412bccf223891629",
        )
        .expect("pinned m_p_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_p_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mp_me = physis_core::artifact::ArtifactId::from_hex(
            "b26d54036d2d6b10bf50cda5ef46746fba7a7a0edeb71569428e895ee0a81dc0",
        )
        .expect("pinned mp_me node");
        assert_eq!(
            lab2.store.get(mp_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mp_mmu = physis_core::artifact::ArtifactId::from_hex(
            "25ce5aab6c87db67ab59a27319ed6dc0e5958ade65afad70a977c9fa43049634",
        )
        .expect("pinned mp_mmu node");
        assert_eq!(
            lab2.store.get(mp_mmu).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mp_mn = physis_core::artifact::ArtifactId::from_hex(
            "569fded02422616274c374100f8e423cedece58684b6bd0521c559301a74577e",
        )
        .expect("pinned mp_mn node");
        assert_eq!(
            lab2.store.get(mp_mn).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let e_mp = physis_core::artifact::ArtifactId::from_hex(
            "297019b19da4904272c51a8e04b72e02d384fc15dd567e0d888d9edbc3ac5097",
        )
        .expect("pinned e_mp node");
        assert_eq!(
            lab2.store.get(e_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_p_molar = physis_core::artifact::ArtifactId::from_hex(
            "aa03e8a73d0ec0d8805ac00559d7c6db336f50fc102dc2d7f295adadfb291162",
        )
        .expect("pinned M_p node");
        assert_eq!(
            lab2.store.get(m_p_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let lambda_c_p = physis_core::artifact::ArtifactId::from_hex(
            "7c05dfef72a29e8b2ede826332febcd0331436b3be2f6b97f83f2489d1ee6840",
        )
        .expect("pinned lambda_C_p node");
        assert_eq!(
            lab2.store.get(lambda_c_p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let rp = physis_core::artifact::ArtifactId::from_hex(
            "1cae30455d7ad0da2a7cee5aea88b53749b93936a1d209c3e4bb514f1d9c3058",
        )
        .expect("pinned rp node");
        assert_eq!(
            lab2.store.get(rp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_p = physis_core::artifact::ArtifactId::from_hex(
            "05785ed691b20e86ee5246df2dad1d6a0f90fe6bb85bd35f06c8561a6194622e",
        )
        .expect("pinned mu_p node");
        assert_eq!(
            lab2.store.get(mu_p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_p_mu_b = physis_core::artifact::ArtifactId::from_hex(
            "c5a6302bb2a038bac321db0f3ab362024dfed4b37cf3bc42c1a2850ec255af2a",
        )
        .expect("pinned mu_p_muB node");
        assert_eq!(
            lab2.store.get(mu_p_mu_b).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_p_mu_n = physis_core::artifact::ArtifactId::from_hex(
            "4a808f05f3bd65df18072a9c679ee49155dd2f621824f039a0b83990ae14cc65",
        )
        .expect("pinned mu_p_muN node");
        assert_eq!(
            lab2.store.get(mu_p_mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gp = physis_core::artifact::ArtifactId::from_hex(
            "ae15d6d01a97e5ff6a9e8fdab3c5e3e3e262e41aab803a85ee3a8f8f7e471169",
        )
        .expect("pinned gp node");
        assert_eq!(
            lab2.store.get(gp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_p_mun = physis_core::artifact::ArtifactId::from_hex(
            "865524beff711d79dbcb0c655d6494882c2a63f8988d1476a15c03d34daa29ce",
        )
        .expect("pinned mu_p_mun node");
        assert_eq!(
            lab2.store.get(mu_p_mun).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0p = physis_core::artifact::ArtifactId::from_hex(
            "06e2160c2ed44fd38fe9766274f2de71e2ec60d4e4c705763ba3446f07164739",
        )
        .expect("pinned mu0p node");
        assert_eq!(
            lab2.store.get(mu0p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0p_mu_b = physis_core::artifact::ArtifactId::from_hex(
            "6204531acecb1bcba0d4e881586e346d94f771bf66c19c8c43148c5de99cbbb6",
        )
        .expect("pinned mu0p_muB node");
        assert_eq!(
            lab2.store.get(mu0p_mu_b).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0p_mu_n = physis_core::artifact::ArtifactId::from_hex(
            "842df9c30e0e80c22044f56801b4c741110707db286770da4a3f2d3ff25bf24a",
        )
        .expect("pinned mu0p_muN node");
        assert_eq!(
            lab2.store.get(mu0p_mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let sigma0p = physis_core::artifact::ArtifactId::from_hex(
            "5750e14f1d96e1d0a29d15f26fab1bc146ff8fbcc345daaea980d499198050b3",
        )
        .expect("pinned sigma0p node");
        assert_eq!(
            lab2.store.get(sigma0p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_n = physis_core::artifact::ArtifactId::from_hex(
            "226446ffad910bf91f0ec6f6a4ea8861e515ea82b319cbf2e077995edc051147",
        )
        .expect("pinned m_n node");
        assert_eq!(
            lab2.store.get(m_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_n_u = physis_core::artifact::ArtifactId::from_hex(
            "97d71ba92203fee5acad8633da5489a51a8339cf70a47379866690bce701e6e9",
        )
        .expect("pinned m_n_u node");
        assert_eq!(
            lab2.store.get(m_n_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_n_c2 = physis_core::artifact::ArtifactId::from_hex(
            "d11ab471f5ca712d524882673709646b0e6480bd019d01af2813441bb1389825",
        )
        .expect("pinned m_n_c2 node");
        assert_eq!(
            lab2.store.get(m_n_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_n_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "8e291e13da25fdb5817f4ce46a62918d72e82a1a023efbac440e986768ccc034",
        )
        .expect("pinned m_n_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_n_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_me = physis_core::artifact::ArtifactId::from_hex(
            "785b3da762cf12d86766853b74f872bc4c328111748be893a1ca4b39fef77fbb",
        )
        .expect("pinned mn_me node");
        assert_eq!(
            lab2.store.get(mn_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_mmu = physis_core::artifact::ArtifactId::from_hex(
            "cf2fb2b1ded7aec6b2512de342860fb4bc3a9f7be2f2d8ce37fd0ef180047f12",
        )
        .expect("pinned mn_mmu node");
        assert_eq!(
            lab2.store.get(mn_mmu).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_mp = physis_core::artifact::ArtifactId::from_hex(
            "945a9d7cd8a8baf4c852e73d2b86cb4cca450eeee92775737e6b50877abce21d",
        )
        .expect("pinned mn_mp node");
        assert_eq!(
            lab2.store.get(mn_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_minus_mp = physis_core::artifact::ArtifactId::from_hex(
            "c02b5d5688fa618f7e226612267689b8a0f1428e6b1f7ead92f7b5a5959789da",
        )
        .expect("pinned mn_minus_mp node");
        assert_eq!(
            lab2.store.get(mn_minus_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_minus_mp_u = physis_core::artifact::ArtifactId::from_hex(
            "393944c68f266ffe577f831ae5c03ecbccfc9f2ee336efd281ca912b89fbdb3c",
        )
        .expect("pinned mn_minus_mp_u node");
        assert_eq!(
            lab2.store.get(mn_minus_mp_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_minus_mp_c2 = physis_core::artifact::ArtifactId::from_hex(
            "d80066ce06701b9bfe83a44cfb9aa8a34bacc23e1d1a3860666ec0b317be0e08",
        )
        .expect("pinned mn_minus_mp_c2 node");
        assert_eq!(
            lab2.store.get(mn_minus_mp_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mn_minus_mp_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "3755ecb6520d54be37dbc22d659304b2504065b84d4a3b41422db7efe9a2ec6d",
        )
        .expect("pinned mn_minus_mp_c2_MeV node");
        assert_eq!(
            lab2.store.get(mn_minus_mp_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_n_molar = physis_core::artifact::ArtifactId::from_hex(
            "8351e225509b669ee96bf0e8e63baff12e020575065270ef2d54cc77877ed1c0",
        )
        .expect("pinned M_n node");
        assert_eq!(
            lab2.store.get(m_n_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let lambda_c_n = physis_core::artifact::ArtifactId::from_hex(
            "7e23a68e4206617cddff21229e106f6cb57a4a7f1eb41316ee4e2c251a2ac85a",
        )
        .expect("pinned lambda_C_n node");
        assert_eq!(
            lab2.store.get(lambda_c_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n = physis_core::artifact::ArtifactId::from_hex(
            "f1a990d6d1f6c6abdae0834a0153935da5673e4979a29d6988e4ba563c90aa1d",
        )
        .expect("pinned mu_n node");
        assert_eq!(
            lab2.store.get(mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n_mub = physis_core::artifact::ArtifactId::from_hex(
            "c4f9861a3802f769b5127e1c0e6308c19920c5baacb3eae95802b855984c9d1e",
        )
        .expect("pinned mu_n_muB node");
        assert_eq!(
            lab2.store.get(mu_n_mub).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n_mun = physis_core::artifact::ArtifactId::from_hex(
            "32dc6a7e69b86f074f85c31946033fa8267e4d48df5b511a2bd371f813c383ed",
        )
        .expect("pinned mu_n_muN node");
        assert_eq!(
            lab2.store.get(mu_n_mun).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gn = physis_core::artifact::ArtifactId::from_hex(
            "5e266c7126aeed691a87f41410dd736298097a6a81760313b2d47517c7a3ebad",
        )
        .expect("pinned gn node");
        assert_eq!(
            lab2.store.get(gn).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n_mue = physis_core::artifact::ArtifactId::from_hex(
            "2116e88a6a333c70afbbac409f03add69be63c505641359e0311ddb16f98bff0",
        )
        .expect("pinned mu_n_mue node");
        assert_eq!(
            lab2.store.get(mu_n_mue).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n_mup = physis_core::artifact::ArtifactId::from_hex(
            "677f9e0d701a23bb4a86c03666f3e9af0850affa168ef263f4170843cb91f2ad",
        )
        .expect("pinned mu_n_mup node");
        assert_eq!(
            lab2.store.get(mu_n_mup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_n_mu0p = physis_core::artifact::ArtifactId::from_hex(
            "eb647725813c11763a6c554d40c2e6cb7c085fe4383c225ebe809024b9904620",
        )
        .expect("pinned mu_n_mu0p node");
        assert_eq!(
            lab2.store.get(mu_n_mu0p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_d = physis_core::artifact::ArtifactId::from_hex(
            "f4182bcd2231bc769f766746198bf60aa496cf0ae1b9ca7726cfc78c775eeaab",
        )
        .expect("pinned m_d node");
        assert_eq!(
            lab2.store.get(m_d).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_d_u = physis_core::artifact::ArtifactId::from_hex(
            "d5f940659bb086ee0fe58faea4e01c168cf873d12a762de08d154892ce348373",
        )
        .expect("pinned m_d_u node");
        assert_eq!(
            lab2.store.get(m_d_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_d_c2 = physis_core::artifact::ArtifactId::from_hex(
            "98f021436a9b05709710fb44e4d4302f2559a000c80632c589d0ce35a60a310f",
        )
        .expect("pinned m_d_c2 node");
        assert_eq!(
            lab2.store.get(m_d_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_d_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "1fd451a0f9e39fa6f32963caf5375259d716af30de6dc462c39eadfac63a644a",
        )
        .expect("pinned m_d_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_d_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let md_me = physis_core::artifact::ArtifactId::from_hex(
            "861d04500921d545595f7454950d748d589778b8e15b0633af2e6b2b89af6641",
        )
        .expect("pinned md_me node");
        assert_eq!(
            lab2.store.get(md_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let md_mp = physis_core::artifact::ArtifactId::from_hex(
            "76438ea152e9bce3c9733ba0635291b65794e143615119234ed8777508b0f61d",
        )
        .expect("pinned md_mp node");
        assert_eq!(
            lab2.store.get(md_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_d_molar = physis_core::artifact::ArtifactId::from_hex(
            "29960ff017c63127f01463751240e48b884a25734f16f442f05e4b42806d3a70",
        )
        .expect("pinned M_d node");
        assert_eq!(
            lab2.store.get(m_d_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let rd = physis_core::artifact::ArtifactId::from_hex(
            "0203790131852cb3c7f69d05f48d29347c4955cfbbd096d3906aab205127ede5",
        )
        .expect("pinned rd node");
        assert_eq!(
            lab2.store.get(rd).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d = physis_core::artifact::ArtifactId::from_hex(
            "27fa5e1fad0d9768c160ff694e634905908467dc52156c39fb642d7406045835",
        )
        .expect("pinned mu_d node");
        assert_eq!(
            lab2.store.get(mu_d).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d_mub = physis_core::artifact::ArtifactId::from_hex(
            "1cb11f5a26cf543d181459b2c769e9a610d5573825e81d6bfcebd41085521ae0",
        )
        .expect("pinned mu_d_muB node");
        assert_eq!(
            lab2.store.get(mu_d_mub).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d_to_mu_n = physis_core::artifact::ArtifactId::from_hex(
            "20dd35057706c81f9f847cfe546030855559e0d520dee37b415d14357eab1d0e",
        )
        .expect("pinned mu_d_muN node");
        assert_eq!(
            lab2.store.get(mu_d_to_mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gd = physis_core::artifact::ArtifactId::from_hex(
            "db4f985bb0b58a1e8c23a9d093544983d9b1a9b7d7e29f04b25bb16be0c75f97",
        )
        .expect("pinned gd node");
        assert_eq!(
            lab2.store.get(gd).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d_mue = physis_core::artifact::ArtifactId::from_hex(
            "b7e3f8d483fffdd0e0214b0066e04342242e5d6b9e15c6833c7946aaf8153ca8",
        )
        .expect("pinned mu_d_mue node");
        assert_eq!(
            lab2.store.get(mu_d_mue).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d_mup = physis_core::artifact::ArtifactId::from_hex(
            "5d500b4b42060fbd1225eb945e6d95322456a99148415d18d11e04039c43be7a",
        )
        .expect("pinned mu_d_mup node");
        assert_eq!(
            lab2.store.get(mu_d_mup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_d_mun = physis_core::artifact::ArtifactId::from_hex(
            "1b59d33d5a68a24febbefa7ab88b5800bee2a0d0994ec769f9d147f6d1f6a184",
        )
        .expect("pinned mu_d_mun node");
        assert_eq!(
            lab2.store.get(mu_d_mun).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_t = physis_core::artifact::ArtifactId::from_hex(
            "1f3c547a59756969902848bb496c7d19fddbdb577efca55e9e22074dd3f9ccf7",
        )
        .expect("pinned m_t node");
        assert_eq!(
            lab2.store.get(m_t).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_t_u = physis_core::artifact::ArtifactId::from_hex(
            "40b6aff75bbd01dd42f219fff535af9cd774cf38082fa837bcec84ede61ae9a4",
        )
        ...
        let m_t_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "1332df022f8b2d564b2ec9f8760debb91a5260fe0c1691c4919ccffc9c410c99",
        )
        .expect("pinned m_t_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_t_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mt_me = physis_core::artifact::ArtifactId::from_hex(
            "e204ed8ecdc5afb1b11914f6ba7980f0931aaabbf7ff74d0b8d2fca75025bd3a",
        )
        .expect("pinned mt_me node");
        assert_eq!(
            lab2.store.get(mt_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mt_mp = physis_core::artifact::ArtifactId::from_hex(
            "41ffc89be1c1822959ce00cee4dc8def69aa243b60dc61a9191747d600a7548f",
        )
        .expect("pinned mt_mp node");
        assert_eq!(
            lab2.store.get(mt_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_t_molar = physis_core::artifact::ArtifactId::from_hex(
            "98820fbeb58e46a4d5e31bc12f19bec979553c95500ffd5303811a5c7dd1edb0",
        )
        .expect("pinned M_t node");
        assert_eq!(
            lab2.store.get(m_t_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_t = physis_core::artifact::ArtifactId::from_hex(
            "cf24b32cc388f7e447b4fd094f607dcb10e8982bd2251a0538b5a65d9fdf039c",
        )
        .expect("pinned mu_t node");
        assert_eq!(
            lab2.store.get(mu_t).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_t_mub = physis_core::artifact::ArtifactId::from_hex(
            "0165f8433e9823ce96f4b3758754f555006d0a3d074d2311d5b2424cb0101ec5",
        )
        .expect("pinned mu_t_muB node");
        assert_eq!(
            lab2.store.get(mu_t_mub).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_t_to_mu_n = physis_core::artifact::ArtifactId::from_hex(
            "499abd51eb8397c380382aaa5cdf4e713c71f2feb1ef36b0c8d3faa31a268266",
        )
        .expect("pinned mu_t_muN node");
        assert_eq!(
            lab2.store.get(mu_t_to_mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gt = physis_core::artifact::ArtifactId::from_hex(
            "78102b0ca8a56bb2e5a99294daa66bce2d2899a0527102b56ffc0add1b2348f8",
        )
        .expect("pinned gt node");
        assert_eq!(
            lab2.store.get(gt).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_h = physis_core::artifact::ArtifactId::from_hex(
            "dd7fb3d8d0f506e48199cd8b575f47d9668d7968a53a4d82a9d40c0f62e4cb9a",
        )
        .expect("pinned m_h node");
        assert_eq!(
            lab2.store.get(m_h).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_h_u = physis_core::artifact::ArtifactId::from_hex(
            "78d82dde163ee7a883b7e0748ee24f0af600a8d8feb762eaec4dccbce750e07a",
        )
        .expect("pinned m_h_u node");
        assert_eq!(
            lab2.store.get(m_h_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_h_c2 = physis_core::artifact::ArtifactId::from_hex(
            "ca342c9a437d6127ac6cc8fa5219e366912504aadbd7d9c13ebea57feda513a2",
        )
        .expect("pinned m_h_c2 node");
        assert_eq!(
            lab2.store.get(m_h_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_h_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "7b477dc07fbc9093483a14d474da85361c3f1b41df0815caf12c3c3c4c6750f8",
        )
        .expect("pinned m_h_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_h_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mh_me = physis_core::artifact::ArtifactId::from_hex(
            "ae4ffe1f19def746094bea82f05535eb48236670eb61d5eeaed0c821c42ce1eb",
        )
        .expect("pinned mh_me node");
        assert_eq!(
            lab2.store.get(mh_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mh_mp = physis_core::artifact::ArtifactId::from_hex(
            "cd44c6dd61faf6bab9fc807f39873493ef1f0608501f15958cf18df3900b8e0e",
        )
        .expect("pinned mh_mp node");
        assert_eq!(
            lab2.store.get(mh_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_h_molar = physis_core::artifact::ArtifactId::from_hex(
            "846bd5680670d73586466f859106f679a20e316bc8f379fd20a0f0198de56c89",
        )
        .expect("pinned M_h node");
        assert_eq!(
            lab2.store.get(m_h_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_h = physis_core::artifact::ArtifactId::from_hex(
            "d915995be231b6c53b6d71929708c0b6316d07b165cc70da16e043b6acc04ff5",
        )
        .expect("pinned mu_h node");
        assert_eq!(
            lab2.store.get(mu_h).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_h_mub = physis_core::artifact::ArtifactId::from_hex(
            "3bb0b020d185e2a5a5fa6fd0448fca4a0c41a3cafa42e2e347e4f86d0dfacb32",
        )
        .expect("pinned mu_h_muB node");
        assert_eq!(
            lab2.store.get(mu_h_mub).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_h_to_mu_n = physis_core::artifact::ArtifactId::from_hex(
            "d5ec8eaa8842a4d6a63449e9c864686d2fe786bb47186ec5be6d6fe86c398e2e",
        )
        .expect("pinned mu_h_muN node");
        assert_eq!(
            lab2.store.get(mu_h_to_mu_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gh = physis_core::artifact::ArtifactId::from_hex(
            "89d787cea08d18f680faa8fa0e4e2bc9e25ed8d4c4f649ba7bfe47ed4096e90f",
        )
        .expect("pinned gh node");
        assert_eq!(
            lab2.store.get(gh).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0h = physis_core::artifact::ArtifactId::from_hex(
            "1292784df9b775d6115c93d72a320c32327fccef404f1e628513353025312cc3",
        )
        .expect("pinned mu0h node");
        assert_eq!(
            lab2.store.get(mu0h).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0h_mub = physis_core::artifact::ArtifactId::from_hex(
            "418169685f940c3a11d1df4c4a98825558ff51c7aa59ff97872428511b1db9e2",
        )
        .expect("pinned mu0h_muB node");
        assert_eq!(
            lab2.store.get(mu0h_mub).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0h_mun = physis_core::artifact::ArtifactId::from_hex(
            "b9f0dcbb3f7f4a7d7a965e00f7d25cf6f8229b62aaa1ac187f07c85387046b47",
        )
        .expect("pinned mu0h_muN node");
        assert_eq!(
            lab2.store.get(mu0h_mun).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0h_mup = physis_core::artifact::ArtifactId::from_hex(
            "12d5e03dfa5101a9f92901c642bd9870cbe5340890fab192a12a0e18a36d9d97",
        )
        .expect("pinned mu0h_mup node");
        assert_eq!(
            lab2.store.get(mu0h_mup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu0h_mu0p = physis_core::artifact::ArtifactId::from_hex(
            "b92f8c9af39f5e7287562555b7531498bab3028db3f4bf11816d29ec3b323ef6",
        )
        .expect("pinned mu0h_mu0p node");
        assert_eq!(
            lab2.store.get(mu0h_mu0p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_alpha = physis_core::artifact::ArtifactId::from_hex(
            "162b3c933eb5fc4e331a1d3b9a597f0025d94678c7c0a3ea6916c9a8249d554f",
        )
        .expect("pinned m_alpha node");
        assert_eq!(
            lab2.store.get(m_alpha).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_alpha_u = physis_core::artifact::ArtifactId::from_hex(
            "2f8cdcc85902aaf6946137138a2db30503ea12910fbedb15772898d906a7f5df",
        )
        .expect("pinned m_alpha_u node");
        assert_eq!(
            lab2.store.get(m_alpha_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_alpha_c2 = physis_core::artifact::ArtifactId::from_hex(
            "64a4c00516144551ce261049edb83f3f09431ed32b0da56416dd7f02c51d32ac",
        )
        .expect("pinned m_alpha_c2 node");
        assert_eq!(
            lab2.store.get(m_alpha_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_alpha_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "cfc774331f8e78a1784d3585d0831969df0d21dc26cd5be819922d0706b33d3b",
        )
        .expect("pinned m_alpha_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_alpha_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let malpha_me = physis_core::artifact::ArtifactId::from_hex(
            "e78c04dd2b05c41c6d7bcd91830d202d14c3dfe104743e86305445a5d0b52937",
        )
        .expect("pinned malpha_me node");
        assert_eq!(
            lab2.store.get(malpha_me).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let malpha_mp = physis_core::artifact::ArtifactId::from_hex(
            "172c865937b6a232151daee8e336fcef43c518721c21e67abbdc7bf95b044801",
        )
        .expect("pinned malpha_mp node");
        assert_eq!(
            lab2.store.get(malpha_mp).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_alpha_molar = physis_core::artifact::ArtifactId::from_hex(
            "d44a8e073d6b4105db288a959ad5cb0adf3a174239e2753a68e5c7f7981a83ac",
        )
        .expect("pinned M_alpha node");
        assert_eq!(
            lab2.store.get(m_alpha_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_u = physis_core::artifact::ArtifactId::from_hex(
            "2771039a06696ad5cd983ff13f28411b8d634e1d890869bb4cf6a6d8e9536b16",
        )
        .expect("pinned m_u node");
        assert_eq!(
            lab2.store.get(m_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_u_c2 = physis_core::artifact::ArtifactId::from_hex(
            "d6c812670024baec7c1ae41bbb4961d859585d2e1b4c0ee4596ed01d8eaaa684",
        )
        .expect("pinned m_u_c2 node");
        assert_eq!(
            lab2.store.get(m_u_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_u_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "1e05445f79b116401e18e22766ad853e02804628ad5a00e7fd61c000fe93057f",
        )
        .expect("pinned m_u_c2_MeV node");
        assert_eq!(
            lab2.store.get(m_u_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_u_molar = physis_core::artifact::ArtifactId::from_hex(
            "20a2cd8642b0035f68a9c0487b930415e5472c5bbd393b2f97049772e6bea642",
        )
        .expect("pinned M_u node");
        assert_eq!(
            lab2.store.get(m_u_molar).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_12c = physis_core::artifact::ArtifactId::from_hex(
            "2ad548958aa8e044a9f42f941c3a105621d41ed86c462eb8408fc30b76fdad74",
        )
        .expect("pinned M_12C node");
        assert_eq!(
            lab2.store.get(m_12c).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let n_a_h = physis_core::artifact::ArtifactId::from_hex(
            "2bcc73daab70d1b719c651342dbafc5473a4ee650ce29aacf4e3109bcfa6ebd1",
        )
        .expect("pinned NAh node");
        assert_eq!(
            lab2.store.get(n_a_h).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let n_a_k = physis_core::artifact::ArtifactId::from_hex(
            "ebdfd5d64f47a7b38faa4e4907ac5de394242e039670478b1b8558ffe1a8a853",
        )
        .expect("pinned NAk node");
        assert_eq!(
            lab2.store.get(n_a_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let k_ev = physis_core::artifact::ArtifactId::from_hex(
            "5c6b4f0b437c59c5d406f7bccad046a88f962664f7bceb604cadc09f91d3dcea",
        )
        .expect("pinned k_eV node");
        assert_eq!(
            lab2.store.get(k_ev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let k_hz = physis_core::artifact::ArtifactId::from_hex(
            "f77db081575132d0b379eddeba0f9e4642c54221ffbdcfa29d6147bb30d917c4",
        )
        .expect("pinned k_Hz node");
        assert_eq!(
            lab2.store.get(k_hz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let k_m = physis_core::artifact::ArtifactId::from_hex(
            "0af89015e9d3fa2b164a62f44ac054af48855f03338c51a424abbf9a126e0e93",
        )
        ...
        let kg_j = physis_core::artifact::ArtifactId::from_hex(
            "c926a11668840ba7f9e461d93be0fed6b3cebea1ee13171ca1675e174bcf58ca",
        )
        .expect("pinned kg_J node");
        assert_eq!(
            lab2.store.get(kg_j).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_j = physis_core::artifact::ArtifactId::from_hex(
            "8a296c0621b84a7c7bbc3ad279afa1cb8c353b843806863981fed43ad1e1ffc3",
        )
        .expect("pinned m_J node");
        assert_eq!(
            lab2.store.get(m_j).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_ev = physis_core::artifact::ArtifactId::from_hex(
            "57c79435bf8516f4b2e547b8b221b5dd003f18f701b6d01c44268682fa010d2d",
        )
        .expect("pinned m_eV node");
        assert_eq!(
            lab2.store.get(m_ev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let j_ev = physis_core::artifact::ArtifactId::from_hex(
            "e140741974a7632531ce17777058213576c61bca740b33f5ceb671c5d7924260",
        )
        .expect("pinned J_eV node");
        assert_eq!(
            lab2.store.get(j_ev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let j_k = physis_core::artifact::ArtifactId::from_hex(
            "f830b9ca4ce8383d69a8818628c66a968b039f3f30aa7a22d591a70f629031a4",
        )
        .expect("pinned J_K node");
        assert_eq!(
            lab2.store.get(j_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let ev_hz = physis_core::artifact::ArtifactId::from_hex(
            "d98001e49653a95a3bb1543e45fbb993cdecdfc79d3303e36cd0738fec4b09c6",
        )
        .expect("pinned eV_Hz node");
        assert_eq!(
            lab2.store.get(ev_hz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let ev_k = physis_core::artifact::ArtifactId::from_hex(
            "e5f5954239201da5cb06da674987a5a1aae6ee343db21317a92a75be82b5424f",
        )
        ...
        let hz_m = physis_core::artifact::ArtifactId::from_hex(
            "846bab463c0a51c951864c1258a50764889007111b9f543013c1f060ab63428e",
        )
        .expect("pinned Hz_m node");
        assert_eq!(
            lab2.store.get(hz_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let hz_eh = physis_core::artifact::ArtifactId::from_hex(
            "f6ef614b65ab4700b144c8e49be1f26e3706003f3972cee939983d67a5ceade7",
        )
        .expect("pinned Hz_Eh node");
        assert_eq!(
            lab2.store.get(hz_eh).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let hz_u = physis_core::artifact::ArtifactId::from_hex(
            "54e0629cd6e073f6b5ad822d4120afa624384b2a228a0b46657c5a0e59c52f26",
        )
        .expect("pinned Hz_u node");
        assert_eq!(
            lab2.store.get(hz_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let u_hz = physis_core::artifact::ArtifactId::from_hex(
            "42e50e73ed21eb10bb761a7ab85ea119d621797520e8138e69d473243c8667b7",
        )
        .expect("pinned u_Hz node");
        assert_eq!(
            lab2.store.get(u_hz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let eh_hz = physis_core::artifact::ArtifactId::from_hex(
            "1e4278900e9f26858816ca32a4ba47cb573ac63172ecd5585a92c455ed1fce70",
        )
        .expect("pinned Eh_Hz node");
        assert_eq!(
            lab2.store.get(eh_hz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let eh_m = physis_core::artifact::ArtifactId::from_hex(
            "04a36e4495af1759d633be2c96749e3dcbcc6583b6cad1107128eea4746a56c1",
        )
        .expect("pinned Eh_m node");
        assert_eq!(
            lab2.store.get(eh_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let m_eh = physis_core::artifact::ArtifactId::from_hex(
            "93026de8e32f434e1e60293935443a6cd2f35a77c74adaaf1accd2de43bdf6f3",
        )
        .expect("pinned m_Eh node");
        assert_eq!(
            lab2.store.get(m_eh).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let eh_k = physis_core::artifact::ArtifactId::from_hex(
            "2dae2b24d50cbbb0e7f5e5d3afac7b5ca4dd1eddb828146c9f245ec4052fa147",
        )
        ...
        let eh_u = physis_core::artifact::ArtifactId::from_hex(
            "2c8618557602554a8d101ef34a276b06bcc981cce6ba57d6d93f01de95d3f8a1",
        )
        .expect("pinned Eh_u node");
        assert_eq!(
            lab2.store.get(eh_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let ev_eh = physis_core::artifact::ArtifactId::from_hex(
            "a9e5d2c994add3d47fe7ad539d3417cebf9aa69aee2b2b10154d04190c80172e",
        )
        .expect("pinned eV_Eh node");
        assert_eq!(
            lab2.store.get(ev_eh).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let j_eh = physis_core::artifact::ArtifactId::from_hex(
            "897c2126a3ef852a26f86bd5d39162b2d1ea47bb5875232e6ec15909626458b2",
        )
        .expect("pinned J_Eh node");
        assert_eq!(
            lab2.store.get(j_eh).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let u_eh = physis_core::artifact::ArtifactId::from_hex(
            "965c12163e67c74d6c8665e15dbef38db6ab88fc42720a075786c7d8af3f6726",
        )
        ...
        let kg_u = physis_core::artifact::ArtifactId::from_hex(
            "512d569359b032f207037b2cf2727128ced6dfa813c61a56ca0c59e4db139dff",
        )
        .expect("pinned kg_u node");
        assert_eq!(
            lab2.store.get(kg_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let j_u = physis_core::artifact::ArtifactId::from_hex(
            "c52249bcac19ccd303fbe29b8355da38e3ad5a2aef6bc179978f5d8d83563887",
        )
        .expect("pinned J_u node");
        assert_eq!(
            lab2.store.get(j_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let k_u = physis_core::artifact::ArtifactId::from_hex(
            "6f2e5d8122826c006e5d64183b9035261fa51e1e6508ff2887a9e5348c33039b",
        )
        .expect("pinned K_u node");
        assert_eq!(
            lab2.store.get(k_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let ev_u = physis_core::artifact::ArtifactId::from_hex(
            "af9b87a44fa433ffb7b314c4c4aeb3f2736ad27f928d258da05e338fca431b91",
        )
        .expect("pinned eV_u node");
        assert_eq!(
            lab2.store.get(ev_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let u_m = physis_core::artifact::ArtifactId::from_hex(
            "acc94a83eafd38f422f07aa2387373cdfc0b98c33890b5bdce317ff4fe7c8651",
        )
        .expect("pinned u_m node");
        assert_eq!(
            lab2.store.get(u_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let u_k = physis_core::artifact::ArtifactId::from_hex(
            "4d6058f8883ec3209ea999ba52721aab5843129d8aa32ca20769e345dc1acbfa",
        )
        .expect("pinned u_K node");
        assert_eq!(
            lab2.store.get(u_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let eh_kg = physis_core::artifact::ArtifactId::from_hex(
            "84aed9832f9cce41e32c84f2b27f7c7f7301cb7642e442b9b15f9faf0439211b",
        )
        .expect("pinned Eh_kg node");
        assert_eq!(
            lab2.store.get(eh_kg).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let a_si = physis_core::artifact::ArtifactId::from_hex(
            "786af851dcfebdab86280502344a362caf43e618690b85f0f3c699828c5bec29",
        )
        .expect("pinned a_Si node");
        assert_eq!(
            lab2.store.get(a_si).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let n_a_e = physis_core::artifact::ArtifactId::from_hex(
            "4146991435f7e1aeca08cafaa6c26cd8f8947a49f7c1ca32df3eac4d36bb060d",
        )
        .expect("pinned NAe node");
        assert_eq!(
            lab2.store.get(n_a_e).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let p_0 = physis_core::artifact::ArtifactId::from_hex(
            "85d191df5c952fa73292a6084ae6813f7f99785b624682af2fa135aab85e45f4",
        )
        ...
        let v_m = physis_core::artifact::ArtifactId::from_hex(
            "e5115026033b6250c155eb66657eeec10fa5f72ddf3ae38e85ade223235f5f6b",
        )
        .expect("pinned Vm node");
        assert_eq!(
            lab2.store.get(v_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let n_0 = physis_core::artifact::ArtifactId::from_hex(
            "9561ce70ecd5a964b572bf460bed2c6f035b23a4a95815923788dbc075d95eea",
        )
        .expect("pinned n0 node");
        assert_eq!(
            lab2.store.get(n_0).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let v_m_atm = physis_core::artifact::ArtifactId::from_hex(
            "cf842ed327f1aa6b028b66c6dc6c392ab75f035a01e6dc704ab14702712db094",
        )
        .expect("pinned Vm_atm node");
        assert_eq!(
            lab2.store.get(v_m_atm).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let n_0_atm = physis_core::artifact::ArtifactId::from_hex(
            "a57c2a552d28588ebe8cfabbf47a74327805c9e6352f78c46b78be923ee7331e",
        )
        .expect("pinned n0_atm node");
        assert_eq!(
            lab2.store.get(n_0_atm).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let s_0_r = physis_core::artifact::ArtifactId::from_hex(
            "5056a0679ae99314961bcadfe60712a265c4e1b687bf7f28b4cab22f30aa8c56",
        )
        .expect("pinned S0_R node");
        assert_eq!(
            lab2.store.get(s_0_r).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let s_0_r_atm = physis_core::artifact::ArtifactId::from_hex(
            "c3fb74638f4c181d336240b94bd416d3976a63c2720af0497f3a1347f6fab560",
        )
        .expect("pinned S0_R_atm node");
        assert_eq!(
            lab2.store.get(s_0_r_atm).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let c_1_l = physis_core::artifact::ArtifactId::from_hex(
            "3c68d593569cb32f01daa8afb184f8565dfad4311457c390e6a3d49f665357b3",
        )
        .expect("pinned c1L node");
        assert_eq!(
            lab2.store.get(c_1_l).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let c_2 = physis_core::artifact::ArtifactId::from_hex(
            "8c098b806e87a28bd2d42c5d7f5662dd61b3b81f9c6d926ecc1cd2c7665ccc87",
        )
        .expect("pinned c2 node");
        assert_eq!(
            lab2.store.get(c_2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let k_j = physis_core::artifact::ArtifactId::from_hex(
            "911ac483d6b64b64f2d44cd808b2242e4445fd5e2eb6b13c0c714cd0aae6527c",
        )
        .expect("pinned KJ node");
        assert_eq!(
            lab2.store.get(k_j).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let r_k = physis_core::artifact::ArtifactId::from_hex(
            "9d6fa90a4590f96e4ab850e99847ab06da3d60e90a61218f2c5642a67d174f19",
        )
        .expect("pinned RK node");
        assert_eq!(
            lab2.store.get(r_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_b = physis_core::artifact::ArtifactId::from_hex(
            "25548862db56005d5ed865aee7f0ee08a7cd24c478ec0aec2e78c775ca9060ef",
        )
        .expect("pinned muB node");
        assert_eq!(
            lab2.store.get(mu_b).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_b_ev = physis_core::artifact::ArtifactId::from_hex(
            "8df4a0984aa398db3d36b391d61ec73784b0a7c72257906a42b9e7425efc6d49",
        )
        .expect("pinned muB_eV node");
        assert_eq!(
            lab2.store.get(mu_b_ev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_b_hz = physis_core::artifact::ArtifactId::from_hex(
            "3cf64fd1f060d0379c3d21d17b5ba9bffa0c40ad1817493a084d13cde1cba088",
        )
        .expect("pinned muB_Hz node");
        assert_eq!(
            lab2.store.get(mu_b_hz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_b_m = physis_core::artifact::ArtifactId::from_hex(
            "a5c6bc7a371abe6a50398009955f3ff1790cb5323027ddb9f51bc90920664e04",
        )
        .expect("pinned muB_m node");
        assert_eq!(
            lab2.store.get(mu_b_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_b_k = physis_core::artifact::ArtifactId::from_hex(
            "0dc9efe8516984f8cd702a37559d258d6073f1e6f2c04544c705b7a1b2a0d083",
        )
        .expect("pinned muB_K node");
        assert_eq!(
            lab2.store.get(mu_b_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_nuc = physis_core::artifact::ArtifactId::from_hex(
            "5b78c0f11cbde89d9e2b0fb274c93113715007644fab16975a1c87fea4c79efb",
        )
        .expect("pinned muN node");
        assert_eq!(
            lab2.store.get(mu_nuc).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_nuc_ev = physis_core::artifact::ArtifactId::from_hex(
            "3499319aa12a29a8123e77d0a0fcbd6cc4a0ee6da708788ef7450ad0453892d0",
        )
        .expect("pinned muN_eV node");
        assert_eq!(
            lab2.store.get(mu_nuc_ev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_nuc_m = physis_core::artifact::ArtifactId::from_hex(
            "1c927bad61aec31ce19c4de08f8331adeba44d15f07c9577d84556b2315f7b9a",
        )
        .expect("pinned muN_m node");
        assert_eq!(
            lab2.store.get(mu_nuc_m).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_nuc_k = physis_core::artifact::ArtifactId::from_hex(
            "e1076582e911fb1259ed2db8919b357e754aee37032996e48ae58a8759dadd68",
        )
        .expect("pinned muN_K node");
        assert_eq!(
            lab2.store.get(mu_nuc_k).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let mu_nuc_mhz = physis_core::artifact::ArtifactId::from_hex(
            "54de34e2ffd258b5a8661cfa82244d91bad42238d9d554602c1eb8a269af583c",
        )
        .expect("pinned muN_MHz node");
        assert_eq!(
            lab2.store.get(mu_nuc_mhz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma_p = physis_core::artifact::ArtifactId::from_hex(
            "d9f5089e6e55a37dea55e5d8418cf58eba84135d3a03bc0cd1ada40b07db50f3",
        )
        .expect("pinned gamma_p node");
        assert_eq!(
            lab2.store.get(gamma_p).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma_p_mhz = physis_core::artifact::ArtifactId::from_hex(
            "89744598268cf707cc8c75c45c9eecd6e9380f4caa2521b9a94633818fc6ebf9",
        )
        .expect("pinned gamma_p_MHz node");
        assert_eq!(
            lab2.store.get(gamma_p_mhz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma0p = physis_core::artifact::ArtifactId::from_hex(
            "b5ab615ca36f3799a21c3574ba8d45c12142ec93172797980396ffa02d2f1963",
        )
        ...
        let gamma_n = physis_core::artifact::ArtifactId::from_hex(
            "26593240a293a2a081e5f473eb3f2befbbc559fade13a6761e8b9022c8fae84f",
        )
        .expect("pinned gamma_n node");
        assert_eq!(
            lab2.store.get(gamma_n).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma_n_mhz = physis_core::artifact::ArtifactId::from_hex(
            "2ab81c06d7033e1ee4d83bc510c77355be587dae3c270af8941ba12a568d8608",
        )
        .expect("pinned gamma_n_MHz node");
        assert_eq!(
            lab2.store.get(gamma_n_mhz).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma_e = physis_core::artifact::ArtifactId::from_hex(
            "8d3e3aeab146c0e8e7ef31e521dad0b8fba6385f1c496900f93d17d45071c788",
        )
        .expect("pinned gamma_e node");
        assert_eq!(
            lab2.store.get(gamma_e).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let gamma_e_mhz = physis_core::artifact::ArtifactId::from_hex(
            "a2a19a2dbfc04e754e457c06a5f84f67a7dc185a5a1dc63634ef97f75700f243",
        )
        ...
        let gamma0h_mhz = physis_core::artifact::ArtifactId::from_hex(
            "ea16a9b399f93c1d2c29197dba77ad96c1b201cbc041dca6995984f7a0986ec1",
        )
        ...
        let me_u = physis_core::artifact::ArtifactId::from_hex(
            "682e118704f0d556c0c83465b59e59e27c0d59730ddaf82c430d3d5162066741",
        )
        .expect("pinned m_e_u node");
        assert_eq!(
            lab2.store.get(me_u).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let me_c2 = physis_core::artifact::ArtifactId::from_hex(
            "c78389974fdf97ffc418c12da149e820bc718c7ae103546574696f8d78a13c11",
        )
        .expect("pinned m_e_c2 node");
        assert_eq!(
            lab2.store.get(me_c2).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let me_c2_mev = physis_core::artifact::ArtifactId::from_hex(
            "9ac6f08d59173db167d93f15a3d4265d26f966a92584a7053dd0dd8d62f3d33a",
        )
        .expect("pinned m_e_c2_MeV node");
        assert_eq!(
            lab2.store.get(me_c2_mev).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let me_kg = physis_core::artifact::ArtifactId::from_hex(
            "25b4aa84438ef207187bb7361b283608c551cf3a992d1bbf9aead9228af322aa",
        )
        .expect("pinned m_e node");
        assert_eq!(
            lab2.store.get(me_kg).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let nup = physis_core::artifact::ArtifactId::from_hex(
            "3b73f5c2000281e562fdf12b64d9bf7b36515105b92a3fee0e31906d0b7cb043",
        )
        .expect("pinned nu_p node");
        assert_eq!(
            lab2.store.get(nup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let auf = physis_core::artifact::ArtifactId::from_hex(
            "955545825f8e26d3e1ab94353d171f043cd712a5f4d1be9c3493dda6fb9be701",
        )
        .expect("pinned au_F node");
        assert_eq!(
            lab2.store.get(auf).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let auv = physis_core::artifact::ArtifactId::from_hex(
            "9593f075fe301bc700285a704f9f5645120b1a9716362313d1bb2c7a2eb743da",
        )
        .expect("pinned au_v node");
        assert_eq!(
        ...
        let aup = physis_core::artifact::ArtifactId::from_hex(
            "7fbd9d2b00b5cc4e224ca5519bde5a590cb7aedc0fc0b1020c13d79140adb7fa",
        )
        .expect("pinned au_p node");
        assert_eq!(
            lab2.store.get(aup).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
        let nut = physis_core::artifact::ArtifactId::from_hex(
            "2b50e30ecc5e7a5f893e51c261d0be10a8ffb8ec0976622f55c40787d527359b",
        )
        .expect("pinned nu_t node");
        assert_eq!(
            lab2.store.get(nut).map(|n| n.kind),
            Some(NodeKind::VersionedConstant)
        );
