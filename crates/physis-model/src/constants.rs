...
/// kg is not a second name. The inverse listing is J_kg.
pub fn kilogram_in_joule() -> Qty<Dimensionless> {
    Qty::new(8.987_551_787_368_176e16)
}

/// Joule-kilogram relationship, SI 2019 exact.
///
/// This is the exact table XXXV energy conversion listed as the
/// joule-kilogram relationship, not kilogram-joule kg_J inverted as a
/// second name, not SI metre-per-second c, not hertz-inverse meter
/// Hz_m, and not a FormalClaim that reconstructs 1/c^2 from live
/// lookups. The table prints an ellipsis; the ledger stores the exact
/// Ratio. This is not a terminating SciExact (7, 73, and 293339 remain
/// in the denominator). The versioned ledger stores the exact Ratio;
/// this Qty is the IEEE rounding of that Ratio. Ledger unit is kg; this
/// Qty is dimensionless, not SI kilogram. Jkg, J-kg, and 1/kg_J are not
/// second names. Faraday constant is NAe.
pub fn joule_in_kilogram() -> Qty<Dimensionless> {
    Qty::new(1.112_650_056_053_618_5e-17)
}
...
/// Molybdenum x unit xu(Mo) (m), CODATA 2018 centre.
///
/// This is the recommended printed table XXXIII x-ray-related centre
/// defined as λ(MoKα1)/707.831, not Bohr radius a0, not copper x unit
/// xu_Cu, not Angstrom star Astar, not lattice parameter a_Si, not
/// classical electron radius re, and not a FormalClaim that reconstructs
...
/// The atomic-unit first-hyperpolarizability listing is au_hyp.
...
/// Atomic unit of 1st hyperpolarizability e^3 a0^3 / Eh^2 (C^3 m^3 J^{-2}),
/// CODATA 2018 centre.
///
/// This is the recommended printed table XXXIV Atomic units centre,
...
/// Atomic unit of magnetic dipole moment 2 muB (J T^{-1} = A m^2),
/// CODATA 2018 centre.
///
/// This is the recommended printed table XXXIV Atomic units centre,
/// not Bohr magneton muB, not electron magnetic moment mu_e, not nuclear
/// magneton muN, not au_hyp2, and not a FormalClaim that reconstructs
/// twice Bohr magneton or hbar e / m_e from a live lookup. The printed
/// formula cites hbar and is unused; the versioned ledger stores the
/// one-sigma hull; this Qty is that centre. Ledger unit is J T^{-1};
/// this Qty is magnetic dipole moment (L^2 I). aumu, au-mu, au_mdm,
/// mag_dipole, magnetic_dipole, 2muB, two_muB, and ehbar_me are not
/// second names.
pub fn atomic_unit_of_magnetic_dipole_moment(
...
/// Shielding difference of d and p in HD σ_dp, CODATA 2018.
///
/// This is the recommended centre from the NIST 2018 complete listing,
/// not proton magnetic shielding correction and not the HT sibling.
/// This Qty is not a certificate that it equals a reconstructed σ_dp
/// from sibling shieldings and not Table XXI D42 as a second name. The
/// versioned ledger stores the one-sigma hull; this Qty is that centre.
/// This is not the CODATA 2022 last-digit 8770. Faraday constant is NAe.
pub fn shielding_difference_d_p_in_hd() -> Qty<Dimensionless> {
    Qty::new(2.020_0e-8)
}
...
///
/// This is the recommended printed UNIVERSAL centre, not Planck
/// temperature TP, not atomic-unit time au_t, not natural-unit time
/// nu_t, and not a reconstructed time formula. The versioned ledger
/// stores a SciInterval because Ratio scale 10^50 overflows i128;
/// this Qty is that centre. t_P, tp, Planck_time, planck-time, and
/// tPlanck are not second names.
...
mod tests {
    ...
    fn overlapping_qty_floats_lockstep_the_versioned_ledger() {
        ...
        assert_eq!(
            tp_centre.to_ratio(),
            None,
            "tP Qty lockstep cannot be a Ratio: 10^50 overflows i128"
        );
        assert!(
            tptime.value.contains(SciInterval::point(tp_centre)),
            "tP Qty centre must lie in the versioned one-sigma hull"
        );
        assert_ne!(
            tptime.value.lo, tptime.value.hi,
            "ledger tP stays a SciInterval; the Qty is not that hull"
        );
        assert_ne!(
            tptime.hash,
            physis_constants::planck_length().hash,
            "tP Qty lockstep is not Planck length"
        );
        assert_ne!(
            tptime.hash,
            physis_constants::planck_temperature().hash,
            "tP Qty lockstep is not TP"
        );
        assert_ne!(
            tptime.hash,
            physis_constants::atomic_unit_of_time().hash,
            "tP Qty lockstep is not au_t"
        );
        assert_ne!(
            tptime.hash,
            physis_constants::natural_unit_of_time().hash,
            "tP Qty lockstep is not nu_t"
        );
        ...
        assert_eq!(
            ...
            sigma_dp_centre.to_f64(),
            "sigma_dp Qty is the CODATA centre inside the hull"
        );
        assert_eq!(
            shielding_difference_d_p_in_hd().value(),
            2.020_0e-8,
            "sigma_dp Qty locksteps to Ratio::to_f64 on the integer 10^12 centre"
        );
        assert!(
            sigma_dp.value.contains(Interval::point(sigma_dp_centre)),
            "sigma_dp Qty centre must lie in the versioned one-sigma hull"
        );
        assert_ne!(
            sigma_dp.value.lo, sigma_dp.value.hi,
            "ledger sigma_dp stays an Interval; the Qty is not that Interval"
        );
        assert_ne!(
            physis_constants::shielding_difference_d_p_in_hd().hash,
            physis_constants::proton_magnetic_shielding_correction().hash,
            "sigma_dp is not sigma0p"
        );
        assert_ne!(
            physis_constants::shielding_difference_d_p_in_hd().hash,
            physis_constants::triton_proton_magnetic_moment_ratio().hash,
            "sigma_dp is not mu_t_mup"
        );
        assert_eq!(
            physis_constants::lookup("sigma_dp").unwrap().kind,
            "interval"
        );
        assert!(
            physis_constants::lookup("sigmadp").is_none(),
            "sigmadp is not a ledger name; the live name is sigma_dp"
        );
        assert!(
        ...
        assert_eq!(
            joule_in_kilogram().value(),
            j_kg_value.to_f64(),
            "J_kg Qty is the IEEE rounding of the exact Ratio"
        );
        assert_eq!(
            joule_in_kilogram().value(),
            1.112_650_056_053_618_5e-17,
            "J_kg Qty locksteps to Ratio::to_f64 of the exact Ratio"
        );
        assert!(
            j_kg.value > Ratio::int(0),
            "ledger J_kg stays a positive exact Ratio"
        );
        assert_ne!(
            physis_constants::joule_in_kilogram().hash,
            physis_constants::kilogram_in_joule().hash,
            "J_kg is not kg_J"
        );
        assert_ne!(
        ...
        assert_eq!(physis_constants::lookup("J_kg").unwrap().kind, "ratio");
        assert!(
            physis_constants::lookup("Jkg").is_none(),
            "Jkg is not a ledger name; the live name is J_kg"
        );
        assert!(
        ...
        assert_eq!(
            ...
            xu_mo_centre.to_f64(),
            "xu_Mo Qty is the CODATA centre inside the hull"
        );
        assert_eq!(
            molybdenum_x_unit().value(),
            1.002_099_52e-13,
            "xu_Mo Qty locksteps to Ratio::to_f64 on the integer 10^21 centre"
        );
        assert!(
            xu_mo.value.contains(Interval::point(xu_mo_centre)),
            "xu_Mo Qty centre must lie in the versioned one-sigma hull"
        );
        assert_ne!(
            xu_mo.value.lo, xu_mo.value.hi,
            "ledger xu_Mo stays an Interval; the Qty is not that Interval"
        );
        assert_ne!(
        ...
        assert_eq!(physis_constants::lookup("xu_Mo").unwrap().kind, "interval");
        assert!(
            physis_constants::lookup("xuMo").is_none(),
            "xuMo is not a ledger name; the live name is xu_Mo"
        );
        assert!(
            physis_constants::lookup("molybdenum_xu").is_none(),
            "molybdenum_xu is not a second name for xu_Mo"
        );
        assert!(
            physis_constants::lookup("d220").is_none(),
        ...
        assert_eq!(
            ...
            au_hyp_centre.to_f64(),
            "au_hyp Qty is the CODATA centre inside the hull"
        );
        assert_eq!(
            atomic_unit_of_first_hyperpolarizability().value(),
            3.206_361_306_1e-53,
            "au_hyp Qty locksteps to SciExact::to_f64 on the 10^-63 centre"
        );
        assert!(
            au_hyp.value.contains(SciInterval::point(au_hyp_centre)),
            "au_hyp Qty centre must lie in the versioned one-sigma hull"
        );
        assert_ne!(
            au_hyp.value.lo, au_hyp.value.hi,
            "ledger au_hyp stays a SciInterval; the Qty is not that Interval"
        );
        assert_ne!(
            physis_constants::atomic_unit_of_first_hyperpolarizability().hash,
            physis_constants::atomic_unit_of_electric_polarizability().hash,
            "au_hyp is not au_pol"
        );
        assert_ne!(
            physis_constants::atomic_unit_of_first_hyperpolarizability().hash,
            physis_constants::elementary_charge().hash,
        ...
        assert_eq!(
            physis_constants::lookup("au_hyp").unwrap().kind,
            "sci-interval"
        );
        assert!(
            physis_constants::lookup("auhypol").is_none(),
            "auhypol is not a ledger name; the live name is au_hyp"
        );
        assert!(
            physis_constants::lookup("hyperpolarizability").is_none(),
            "hyperpolarizability is not a second name for au_hyp"
        );
        assert!(
            physis_constants::lookup("d220").is_none(),
        ...
        assert_eq!(
            ...
            au_mu_centre.to_f64(),
            "au_mu Qty is the CODATA centre inside the hull"
        );
        assert_eq!(
            atomic_unit_of_magnetic_dipole_moment().value(),
            1.854_802_015_66e-23,
            "au_mu Qty locksteps to Ratio::to_f64 on the integer 10^34 centre"
        );
        assert!(
            au_mu.value.contains(Interval::point(au_mu_centre)),
            "au_mu Qty centre must lie in the versioned one-sigma hull"
        );
        assert_ne!(
            au_mu.value.lo, au_mu.value.hi,
            "ledger au_mu stays an Interval; the Qty is not that Interval"
        );
        assert_ne!(
        ...
        assert_ne!(
            physis_constants::atomic_unit_of_magnetic_dipole_moment().hash,
            physis_constants::atomic_unit_of_second_hyperpolarizability().hash,
            "au_mu is not au_hyp2"
        );
        assert_eq!(physis_constants::lookup("au_mu").unwrap().kind, "interval");
        assert!(
            physis_constants::lookup("aumu").is_none(),
            "aumu is not a ledger name; the live name is au_mu"
        );
        assert!(
            physis_constants::lookup("2muB").is_none(),
            "2muB is not a second name for au_mu"
        );
        assert!(
            physis_constants::lookup("d220").is_none(),
