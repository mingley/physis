...
2. Run the CLI (`physis layers|theories|knobs|run|set|experiment|journal|epistemics|why|prove|formalize|reproduce|falsify|sweep|branch|audit|review|inspect|loop`). `--role explorer` (and the other named roles) can observe but cannot mint. `--budget prove=N,review=N,set=N` is a research cap, not a proof. `physis reproduce` remints a stored receipt in-process and is **not** P4.
