...
- Versioned constants: SI 2019 defining `c`, `Δν_Cs`, `e`, `k`, `N_A`,
  ...
  measured solar luminosity. IAU 2015 `S_☉^N` is an exact `Ratio` `1361` W m^{-2} from the same table: a conversion ruler, not a measured solar irradiance. IAU 2015 `T_☉^N` is an exact `Ratio` `5772` K from the same table: a conversion ruler, not a measured solar effective temperature. IAU 2015 `R_Ee^N` is an exact `Ratio` `6378100` m from the same table: a conversion ruler, not a measured terrestrial equatorial radius. IAU 2015 `R_Ep^N` is an exact `Ratio` `6356800` m from the same table: a conversion ruler, not a measured terrestrial polar radius. IAU 2015 `R_Je^N` is an exact `Ratio` `71492000` m from the same table: a conversion ruler, not a measured Jovian equatorial radius. IAU 2015 `R_Jp^N` is an exact `Ratio` `66854000` m from the same table: a conversion ruler, not a measured Jovian polar radius. IAU 2015 `(GM)_E^N` is an exact `Ratio` `3.986004×10^14` m³ s⁻² from the same table: a conversion ruler, not a measured terrestrial mass. `physis constant [name]`
  ...
  `c`, `au`, `p0`, `atm`, `GM_sun`, `R_sun`, `L_sun`, `S_sun`, `T_sun`, `R_earth`, `R_earth_p`, `R_jup`, `R_jup_p`, and `GM_earth` via integer `to_f64`; `e`/`k`/`eV`/`NAh`/`NAk`/`NAe`/`Vm` via IEEE rounding of the SI
