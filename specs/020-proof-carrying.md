...
| roles | `Role`, `ResearchBudget`, `physis formalize` | Named processes propose; only `verify` mints. Explorer cannot prove or score. Proof-searcher cannot remint. Replication-agent remints (not P4). Empirical-analyst scores. Numerical-verifier encloses (`CertifiedNumeric` Ratio cells and an independent `Interval` parse of the GQW and one-loop α_3 input-interval overlays; those are not P3N). Provenance-auditor cites and independently rebuilds versioned constants (`physis constant`). Encoding-auditor round-trips live IR packages. Judge rebuilds `from_lab` (JSON cannot mint `logical proved`). Budget is a cap, not a proof |
...
- Trust tiers do not gate observation or standalone encoding-review.
  ...
  `CertifiedNumeric` enclosure or a GQW / one-loop α_3 input-interval overlay; that is
  `numerical-verifier` (`physis enclose`, a content-addressed
  NumericCertificate; Ratio parse of the four P3N cells; independent
  `Interval` parse of `gut.weinberg-angle-mz-interval` and
  `gut.coupling-unification-interval` is not
...
| C. Interval-certified numeric | Independent `Interval::parse_display` of the GQW overlay on `gut.weinberg-angle-mz-interval` and the one-loop `α_3` overlay on `gut.coupling-unification-interval` (not P3N, not `CertifiedNumeric`). GUT-scale `3/8` remains the `Ratio` / P3N identity, disjoint from the PDG `0.23122` hull |
