...
| constants | `physis-constants` | Versioned SI 2019 defining `c`, `Δν_Cs`, `e`, `k`, `N_A`, `K_cd` as exact `Ratio`. Planck `h` is SI-exact but not a `Ratio` (`i128` denominator overflow). Theories still use `physis_model` `f64` Qty |
...
- Versioned constants: SI 2019 defining `c`, `Δν_Cs`, `e`, `k`, `N_A`,
  `K_cd` are exact `Ratio`. Planck `h` is SI-exact but not a `Ratio`
  (`i128` denominator overflow). CODATA measured `G` is not an interval
  here. Theories still use `physis_model` `f64` Qty constants
