...
- Versioned constants: SI 2019 defining `c`, `Δν_Cs`, `e`, `k`, `N_A`,
  ...
  `SciExact`: 3, 7, and 6310543 remain in the denominator; `k/hc` is not stored).
  ...
  decimal (`SciExact::to_f64`, not reduced `Ratio::to_f64`); `n0`/`Vm_atm`/`n0_atm`/`c2`/`KJ`/`RK`/`k_eV`/`k_Hz` via IEEE rounding of the exact `Ratio`; `h`/`c1L` via
