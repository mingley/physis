...
- Versioned constants: SI 2019 defining `c`, `Δν_Cs`, `e`, `k`, `N_A`,
  ...
  `Ratio` `149597870700` m (BIPM table 8). BIPM table 8 `barn` is an exact `Ratio` `1/10^{28}` square metres from the same table (not Thomson, not `Astar`). BIPM table 8 `angstrom` is an exact `Ratio` `1/10^{10}` m from the same table (not Angstrom star `Astar`). BIPM table 8 `gal` is an exact `Ratio` `1/100` m s^{-2} from the same table (not the CGPM 1901 standard gravity listing). The electronvolt is an exact
