...
| `inspect <axis> <value>` | inverse query over `trust`, `class`, knob `origin`, knowledge `gap`, or projected `judgment` (`statistical-computed`, `empirical-excluded`, `logical-proved`, …). Super-K is `empirical-excluded`; the PDG GQW and one-loop `α_3` cells are `statistical-computed`; `logical-proved` is empty until a dual-checked receipt exists |
