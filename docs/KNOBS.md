...
- `kind` — chosen. A missing E8 is not this knob: `add-missing-e8` is an IR mutation on `heterotic-e8e8`.
- `total_dim` — chosen (the critical-dimension theorem constrains it). A missing E8 is not this knob: `add-missing-e8` is an IR mutation on `heterotic-e8e8`.
